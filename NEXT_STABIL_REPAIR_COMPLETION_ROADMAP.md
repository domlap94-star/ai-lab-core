...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B `PARTIAL_AFTER_FAILURE / HOST_TASK_FAILED_22 / SAFE_INACTIVE_PARTIAL_UNKNOWN / READY_FOR_OWNER_REVIEW`; payload i exact installed manifest są zainstalowane, Host `Disabled/no-trigger`, warm runs `0/2`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260919T212059Z-D21-P4B-RUN01-PARTIAL` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-19T21:20:59Z` |
| Aktualny wykonawca / sesja | Codex / jednorazowy RunAs/UAC wykonany; instalacja przerwana na Host result `22`; jeden SAFE_INACTIVE zakończony `PARTIAL_UNKNOWN`; pooperacyjne odczyty bez dalszych mutacji; audyt CRM, Gmail/Sheets/n8n, modele i dane biznesowe nietknięte |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — faktyczny wynik częściowej instalacji do owner review; bez retry, drugiego UAC, dodatkowego rollbacku lub przejścia do P5 |
...
| Ostatnia faktycznie zakończona czynność | Po jednym UAC wykonano exact run01. Instalator zainstalował trzy przypięte pliki i manifest oraz definicje docelowe, uruchomił Host raz, otrzymał `22`, wykonał jeden SAFE_INACTIVE i zatrzymał się `PARTIAL_UNKNOWN`; następnie utrwalono read-only stan tasków, surface i sześciu kontenerów |
| Potwierdzone testy bieżącego wykonania | Installer result `PARTIAL_AFTER_FAILURE`, mutation workers `50/50`, pending mutation `false`; Host `Disabled/no-trigger`, LastTaskResult `22`; post task definitions `6/6` zgodne z recipe after; backend/Public Gateway `200`, public `/control*` `404`; sześć kontenerów unchanged/running, restarts `0`, PostgreSQL healthy. Warm runs `0/2`, Private starts `0` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY `C:\ai-lab-core-staging\recovery\P4B-FIN-01`; run01 result SHA-256 `3EA0038D...764C6`, events `F915A12C...11CC2`, post task `B3D1EDB0...8A005`, definitions `97352F8F...1C825`, surface `AE93C524...649FE`, containers `8865AC7C...69E9` |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260919T212059Z-R04-D21-P4B-RUN01-PARTIAL.md` |
| Zakres aktualnej zgody | Jednorazowa zgoda operacyjna, jeden UAC i jeden SAFE_INACTIVE są skonsumowane. Dokumentacja wyniku jest dozwolona; brak zgody na retry Host/Install, drugi UAC, dalszy rollback, logon/reboot, P5 lub R06. D-22 pozostaje `NOT_RUN` |
| Blokada / wymagana decyzja | `HOST_TASK_FAILED_22`; szczegół wyniku launchera nie został utrwalony (`LAUNCHER_RESULT_DETAIL_NOT_CAPTURED`). Private Gateway nie wystartował. Nowa diagnostyka lub operacja mutująca wymaga owner review i osobnej decyzji |
| Jeden następny bezpieczny krok | Owner review faktycznego run01 i wybór ograniczonego zakresu diagnostycznego dla kodu `22`; bez ponowienia Host/instalatora w ramach zużytej zgody |
...
| R04 | IN_PROGRESS | `R04-20260919T212059Z-D21-P4B-RUN01-PARTIAL` / P4-A OWNER ACCEPTED + P4/B PARTIAL AFTER FAILURE | P3 backend switch pozostaje `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A zachowuje swój wcześniejszy odbiór. Exact short-output resume zużyło jeden UAC. Trzy pliki i exact installed manifest są zainstalowane, task definitions są w docelowym/on-demand albo bezpiecznie wyłączonym wariancie, lecz Host zakończył pierwszą próbę kodem `22`; SAFE_INACTIVE `PARTIAL_UNKNOWN`, warm `0/2`, Private start `0`, Host disabled/no-trigger, brak logon triggera. Sześć kontenerów unchanged; Supervisor `INTENTIONALLY_STOPPED`. Qdrant/VHD/profile, UI, backup proof, live logon/reboot i P4-B/P5 pozostają otwarte |
