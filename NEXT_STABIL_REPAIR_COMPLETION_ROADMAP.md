...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard i ograniczony P3 zachowują wcześniejsze odbiory. P4/A pozostaje `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B nadal jest `PARTIAL_SAFE_INACTIVE`. Dokładna LOCAL_ONLY recepta `F6D3A8CC...C883F0E`, indeks `FDF9FE7A...A977F` i review ZIP `D2B3263B...65DA1C` zostały przyjęte wyłącznie jako `EXACT_ROLLBACK_SAFE_RECIPE_ACCEPTED / SOURCE_AND_OFFLINE_SCOPE / NOT_INSTALLED`; 7/7 plików ZIP, 6/6 wpisów pakietu, 29/29 wejść oraz `VerifyInputsOnly` są zgodne. Świeży read-only preflight zatrzymał się na błędzie zapisu pierwszej projekcji taska do zbyt długiej ścieżki dowodowej; wynik taska nie został utrwalony i nie był ponawiany. Status operacyjny to `PRE_UAC_BLOCKED / PREFLIGHT_EVIDENCE_NOT_PERSISTED_PATH_LENGTH / NO_MUTATION`; UAC, Install, Docker/HTTP readback i warm runs nie wystąpiły. Globalny manifest pozostaje `NOT_APPROVED_FOR_START`, Supervisor `INTENTIONALLY_STOPPED`, a P4-B/P5 i R06–R24 nie zostały uruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B zachowuje `PARTIAL_SAFE_INACTIVE`; recepta ma `EXACT_ROLLBACK_SAFE_RECIPE_ACCEPTED / SOURCE_AND_OFFLINE_SCOPE / NOT_INSTALLED`, a bieżąca próba `PRE_UAC_BLOCKED / PREFLIGHT_EVIDENCE_NOT_PERSISTED_PATH_LENGTH`; globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260919T094728Z-D21-P4B-EXACT-RECIPE-PREFLIGHT-BLOCKED` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-19T09:47:28Z` |
| Aktualny wykonawca / sesja | Codex / exact recipe review zakończony; świeży read-only preflight zatrzymany na nieutrwalonym pierwszym odczycie taska. Bez Docker/HTTP/UAC/Install i bez mutacji. STOP przed replacement preflight wymagającym nowej decyzji |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — exact recipe odebrana w zakresie source/offline; replacement preflight wymaga nowej decyzji; cały R04 nadal `IN_PROGRESS` |
...
| Ostatnia faktycznie zakończona czynność | Potwierdzono dokładne bajty pakietu i 29 wejść oraz wykonano `VerifyInputsOnly` z exit 0. Następny świeży preflight zatrzymał się po pierwszym odczycie taska, gdy collector nie utrwalił XML z powodu limitu długości ścieżki; odczytu nie ponowiono, UAC ani Install nie uruchomiono |
| Potwierdzone testy bieżącego wykonania | Zachowane Windows PowerShell `5.1.26100.8894`: preimage 4/4/16, input 14/14/110, orchestration 15/15/96, workers 437/437, parser PASS i ZIP roundtrip PASS. Bieżące: package 6/6, ZIP 7/7, external inputs 29/29, `VerifyInputsOnly` exit 0. Fresh task preflight: exit 1, projection not persisted; Docker/HTTP/UAC/Install/host mutations `0`; warm runs `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY root `p4b-rollback-dependencies-20260919T101500Z`; recipe 91,889 B SHA-256 `F6D3A8CC...C883F0E`, index 2,970 B SHA-256 `FDF9FE7A...A977F`, review ZIP 47,891 B SHA-256 `D2B3263B...65DA1C`. Historyczne recipe/index/ZIP pozostają bez zmian. Payload/manifest pozostają NOT_INSTALLED |
| Niezakończone procesy i skutki operacyjne | Brak własnego instalatora i UAC. Collector zakończony exit 1; nowy katalog dowodowy pozostał pusty. Pierwszy task read/export nie ma utrwalonej projekcji i nie został ponowiony. Docker/HTTP/Install/mutations/rollback/warm runs 0. Zachowany wcześniejszy stan hosta pozostaje historyczny |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260919T094728Z-R04-D21-P4B-EXACT-RECIPE-PREFLIGHT-BLOCKED.md` |
| Zakres aktualnej zgody | Exact recipe ma odbiór wyłącznie source/offline. Pierwotna jednorazowa kampania read-only została zużyta przez nieutrwalony pierwszy task read. Żadna zgoda na replacement preflight, UAC lub Install nie pozostaje aktywna |
| Blokada / wymagana decyzja | `PREFLIGHT_EVIDENCE_NOT_PERSISTED_PATH_LENGTH`. Nowa decyzja musi dopuścić jedną zastępczą kampanię read-only z krótką ścieżką dowodową. Dopiero osobny późniejszy PASS i bieżące potwierdzenie właściciela mogą otworzyć UAC/Install; Docker/WSL pool availability i swap-used pozostają `UNKNOWN`; Qdrant/VHD/profile, backup proof i UI są otwarte |
| Jeden następny bezpieczny krok | Review checkpointu i osobna zgoda wyłącznie na jeden replacement bounded read-only preflight z wcześniej wskazanym krótkim evidence rootem; bez UAC i Install w tej samej decyzji |
...
| R04 | IN_PROGRESS | `R04-20260919T094728Z-D21-P4B-EXACT-RECIPE-PREFLIGHT-BLOCKED` / P4-A OWNER ACCEPTED + P4/B PARTIAL SAFE INACTIVE | P3 backend switch pozostaje `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A pozostaje owner-accepted jako `NOT_INSTALLED`. Preimage potwierdził RV-P4B-FULL-03B/02B/02C. LOCAL_ONLY recipe `F6D3A8CC...C883F0E`, package index `FDF9FE7A...A977F` i review ZIP `D2B3263B...65DA1C` przeszły końcowe PowerShell 5.1 offline: input 14/110, orchestration 15/96, workers 437/437. Real Docker/Task/CIM/TCP/HTTP/UAC/mutacje `0`; historycznie wrapper pozostaje w rollbacku, Host disabled/no-trigger, istniejące taski na preimage, payload/manifest nie są zainstalowane, warm runs `0/2`, draft/global manifest `NOT_APPROVED`. Dokładne bajty są owner-accepted jako `EXACT_ROLLBACK_SAFE_RECIPE_ACCEPTED / SOURCE_AND_OFFLINE_SCOPE / NOT_INSTALLED`; package 6/6, ZIP 7/7, inputs 29/29 i VerifyInputsOnly exit 0. Fresh preflight ma `PRE_UAC_BLOCKED / PREFLIGHT_EVIDENCE_NOT_PERSISTED_PATH_LENGTH`: pierwszy task read nie został utrwalony ani ponowiony; UAC/Install/Docker/HTTP/warm/mutations 0. Następny krok to osobna decyzja na replacement read-only preflight, a nie instalacja. Qdrant/VHD/profile, UI, backup proof, live logon/reboot/rollback i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
