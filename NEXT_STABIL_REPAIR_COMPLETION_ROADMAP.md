...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`, P2 `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`, a DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0` pozostaje zaakceptowany jako `OFFLINE_TEST_ONLY / NOT_DEPLOYED`. Guard source `0ee0ea50943578e6e552aae23ce1688595ddc262` jest teraz `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. D21-P3 pozostaje `PREPARATION_PARTIAL / CURRENT_DB_METADATA_OBSERVED_READ_ONLY / ROLLBACK_EVIDENCE_REVIEW_READY / NO_CUTOVER`: exact runtime read potwierdził backend/PostgreSQL i mounty bez recovery/WIP, a jedna transakcja PostgreSQL READ ONLY potwierdziła revision, Administrator readiness, PGDATA, brak zewnętrznych WAL/tablespaces oraz małe agregaty kolejek. Zastany pending work to 18 document-preparation, 16 analysis i jeden Assistant. Wyniki backup tasków `267014/1/1` nie potwierdzają świeżej kopii; accepted full point `8F20A784...` z 2026-09-08 oraz DB/n8n z 2026-09-11 wymagają jednej decyzji właściciela o świeżości przed cutoverem. Produkcyjny manifest pozostaje `NOT_APPROVED_FOR_START`. Cutover, relokacja, task install, live startup, reboot, cleanup i operacyjne P3–P5 są `NOT_RUN / NOT_AUTHORIZED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`; P2 jest `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`; DATA_ONLY source jest `ACCEPTED / OFFLINE_TEST_ONLY / NOT_DEPLOYED`. Guard startupu jest `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. D21-P3 pozostaje `PREPARATION_PARTIAL / CURRENT_DB_METADATA_OBSERVED_READ_ONLY / ROLLBACK_EVIDENCE_REVIEW_READY / NO_CUTOVER`; draft manifestu nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260916T200609Z-D21-P3-CURRENT-DB-ROLLBACK` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-16T20:06:09Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 current DB + rollback evidence READ ONLY; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P3 preparation` — `PREPARATION_PARTIAL / CURRENT_DB_METADATA_OBSERVED_READ_ONLY / ROLLBACK_EVIDENCE_REVIEW_READY / NO_CUTOVER`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `c52f453513dc96bd750dd9e6c0a0a836f134d7e6`; accepted guard source `0ee0ea50943578e6e552aae23ce1688595ddc262`; accepted DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0`. Oryginalny preservation `203/203` i zastane runtime/data/junction pozostały nietknięte |
...
| Źródła runtime / release / DB | Exact read `2026-09-16T19:59:54Z–20:05:34Z` potwierdził backend `9d9b46c5...615a`, image `sha256:6342b36f...d63702`, `/app=deploy-main`, `/data=C:/ai-lab-core/data` oraz PostgreSQL `240343eb...1226`, image `sha256:a426e44b...d8508d`, bind `C:/ai-lab-core/data/postgres:/var/lib/postgresql/data`; recovery/WIP nie jest runtime. SQL READ ONLY potwierdził PostgreSQL `17.10`, schema `followup_assistant_chat_history_20260829`, Administrator `1/1`, PGDATA pod bindem D:, brak zewnętrznych WAL/tablespaces. Web P2 pozostaje TEST_ONLY, Supervisor nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | Jedna ograniczona sesja PostgreSQL READ ONLY zakończyła się exit `0` po 1424 ms i jawnym rollbacku; przejrzano aktualne taski i istniejące manifesty rollbacku bez backupu/restore/rehashu dużych artefaktów |
| Potwierdzone testy bieżącego wykonania | Testy aplikacji `NOT_RUN`. Odczyt: `psql -X`, `ON_ERROR_STOP`, `default_transaction_read_only=on`, `BEGIN READ ONLY`, timeouty i `ROLLBACK`, exit `0`. Historyczne testy niezmienionego guarda pozostają: preimage `2 passed / 9 failed`; guard `11 passed`; auth 8x401; version+A4 API `26 passed`; compileall exit `0` |
...
| Niezakończone procesy i skutki operacyjne | Nie utworzono procesu, kontenera ani zasobu testowego. Żądane zapisy danych biznesowych: brak. Restart/start/stop/diagnostic upload = `0`. Supervisor pozostaje `INTENTIONALLY_STOPPED / NOT_STARTED_BY_THIS_SESSION`. Nie zmieniono kontenerów, junctionu, tasków, mountów, flag, kolejek, harmonogramów, danych, backupu lub restore |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260916T200609Z-R04-D21-P3-CURRENT-DB-ROLLBACK.md`; poprzedni handoff: `docs/recovery/checkpoints/20260916T183757Z-R04-D21-P3-STARTUP-GUARD.md` |
| Zakres aktualnej zgody | D21-P3: odczyt runtime, jedna sesja PostgreSQL READ ONLY i przegląd istniejących dowodów rollbacku; bez cutoveru, instalacji, startu/restartu produktu, zmian runtime/config/data/mount/task/flag/queue, backupu/restore/escrow lub P4/P5 |
| Blokada / wymagana decyzja | Rollback ma status `REVIEW_READY / DATA_FRESHNESS_OWNER_DECISION_REQUIRED`; taski `267014/1/1` nie potwierdzają świeżej kopii po accepted full 2026-09-08 i DB/n8n 2026-09-11. Produkcyjny manifest nie jest zatwierdzony, a zastany pending work musi zostać zachowany |
| Jeden następny bezpieczny krok | Właściciel decyduje, czy P3 może użyć full 2026-09-08 + DB/n8n 2026-09-11 mimo nieobjętych zmian do 2026-09-16, czy przed cutoverem wymagany jest nowy zweryfikowany punkt rollbacku |
| Warunek STOP | Bez zapisu/instalacji pod `C:\ai-lab-core`, uruchomienia launchera lub aplikacji, zatwierdzenia manifestu startu, zmian tasków/shortcutów/mountów/config/flag, usług/kolejek/modeli, backupu/restore, deploy/release i P3–P5/R05-A5/R06–R24 |
...
| R04 | IN_PROGRESS | `R04-20260916T200609Z-D21-P3-CURRENT-DB-ROLLBACK` / PREPARATION PARTIAL, CURRENT DB OBSERVED, NO CUTOVER | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 P1 i P2 zachowują odbiory; DATA_ONLY source `cb6e225...` jest `ACCEPTED / OFFLINE_TEST_ONLY / NOT_DEPLOYED`, guard `0ee0ea5...` jest `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. Exact runtime i SQL READ ONLY potwierdziły backend/PostgreSQL, revision, Administrator readiness, PGDATA D: oraz brak zewnętrznych WAL/tablespaces; pending work 18/16/1 pozostaje do zachowania. Rollback evidence jest review-ready, lecz świeżość wymaga decyzji właściciela. Manifest nadal `NOT_APPROVED_FOR_START`; cutover, relokacja, task install, live startup/reboot/rollback, cleanup i runtime acceptance są `NOT_RUN` |
