...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują dotychczasowe odbiory. Run01 pozostaje historycznie PARTIAL_AFTER_FAILURE, Host disabled/no-trigger, warm 0/2 i Supervisor INTENTIONALLY_STOPPED. Exact VerifyOnly jest przyjęty wyłącznie READ_ONLY; późniejszy preflight package/container/HTTP/Windows jest zachowany, a Docker/WSL pool i swap usage pozostają UNKNOWN. Exact observer PS5.1 zachowuje wynik Public `CONFLICT`, Private/Supervisor `UNKNOWN`. Diagnostyka przyczyny odtworzyła dla Private i Supervisor wadę normalizacji pustego odczytu listenera. Jedno późniejsze process-only zapytanie PID `41784` udowodniło dla Public różnicę reprezentacji: właściwy `node.exe`, ten sam kanoniczny skrypt jako token absolutny zamiast manifestowego względnego, zgodne pozostałe argumenty i brak dodatkowych argumentów. Niezmieniony adapter nadal zwraca `CONFLICT / PORT_OWNERSHIP_CONFLICT`; bieżące ownership portu nie było ponownie odczytywane. Bez retry, startów lub mutacji. Nie jest to runtime PASS ani zgoda na poprawkę. Stage B pozostaje BLOCKED / NOT_AUTHORIZED. D-22 nadal ustala kolejność po właścicielskim odbiorze R04: ARKUSZE -> KOREKTA I WALIDACJA KLIENTÓW -> TYLKO NIEPRZYPISANE MAILE; wykonanie NOT_RUN. Bieżący stan znajduje się wyłącznie w §0.**
...
|---|---|
| Bieżący krok P4/B | Host22/NUP i D-23 `2/2` zachowują odbiory. Dwie udowodnione wady obserwacji usług hosta zostały naprawione razem w source `49c3f64c...` i przeszły minimalne testy całego dotkniętego przepływu; wynik pozostaje `READY_FOR_REVIEW / NOT_DEPLOYED`. Stage B ani żadna operacja hosta nie są autoryzowane |
...
| R04 | IN_PROGRESS | `R04-D21-20260923T153332Z-REMAINING-PLAN-HOST-OBS-SOURCE` / SOURCE+OFFLINE+COMPLETE PLAN READY FOR REVIEW | P3/P4-A, Host22 i NUP-01/02/03 odbiory zachowane; D-23 review `2/2`. Source `49c3f64c238e4bdb25e0d3fe9d7bb98c17677bbc` naprawia wyłącznie canonical process code-token i pełny listener snapshot/local exact-port filter; PS5.1 real-adapter/plan `53` i plan `57` PASS, produkcyjne granice `0`. Inactive draft `R04-D21-P4A-HOST-OBS-SOURCE-20260923T153332Z` pozostaje `NOT_APPROVED`; installed run01 pozostaje na starszych bajtach, Host historycznie disabled/no-trigger, warm `0/2`, Supervisor `INTENTIONALLY_STOPPED`. Pełna droga R04 jest w `R04_SINGLE_ROOT_STARTUP_PLAN.md` §8: warm CRM/Web, jedno wejście+logon/cold, dane D:/backup proof i compatibility. Następne okno `R04-P4B-USABLE-WARM` wymaga nowej zgody; Stage B/P5/D-22 nie są uruchomione |
