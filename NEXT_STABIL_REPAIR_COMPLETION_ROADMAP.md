...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. Właściciel przyjął P3 OP_ID `R04-D21-P3-CORE-SWITCH-20260917T141404Z` jako `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4/A source `8756314f51a76091a483cfc9b677a05c7f67f315` i evidence `8a156e0c738699b4bfee01fb98f0d10f01c803ba` są przyjęte jako `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B nadal pozostaje `PARTIAL_SAFE_INACTIVE`: wrapper Startup w rollbacku, Host disabled/no-trigger, pięć tasków na preimage, payload/manifest nieobecne i warm runs `0/2`. Transport natywnych argumentów w LOCAL_ONLY recepcie SHA-256 `5F310C64...1FD67` zachowuje wcześniejszy PASS, lecz bieżący exact-recipe preflight wykazał deterministyczny `BASELINE_PATH_BINDING_MISMATCH`: recepta szuka sześciu baseline records w swoim katalogu, podczas gdy zweryfikowany indeks wiąże je z katalogiem poprzedniego, skonsumowanego resume. Nie wykonano Docker/Task/HTTP preflightu, UAC ani mutacji. Status to `EXACT_RECIPE_PREFLIGHT_BLOCKED / NO_UAC / NO_MUTATION`; nie jest to odbiór P4/B. Repository draft i globalny manifest pozostają `NOT_APPROVED_FOR_START`. Supervisor pozostaje `INTENTIONALLY_STOPPED`; relokacja Qdrant/VHD/profile, backup schedule proof, live startup/reboot/rollback oraz P4-B/P5 są otwarte. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B zachowuje `PARTIAL_SAFE_INACTIVE`; transport argv zachowuje wcześniejszy PASS, ale exact recipe ma `EXACT_RECIPE_PREFLIGHT_BLOCKED / BASELINE_PATH_BINDING_MISMATCH / NO_UAC / NO_MUTATION`; globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260918T163325Z-D21-P4B-EXACT-RECIPE-PREFLIGHT-BLOCKED` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-18T16:32:03.5469632Z` |
| Aktualny wykonawca / sesja | Codex / read-only static preflight exact recipe zakończony blokadą ścieżek sześciu baseline records; bez Docker/Task/HTTP preflightu, UAC i instalacji. STOP przed nową receptą/indeksem i nową decyzją |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — przygotowany external resume ID `R04-D21-P4B-RESUME-20260918T163031Z` nie przeszedł bramki przed UAC; cały R04 nadal `IN_PROGRESS` |
...
| Ostatnia faktycznie zakończona czynność | Nowa recepta przejęła przypięte funkcje serializacji/runnera P1. Offline preimage rozpadł 7 argumentów na 56, poprawiona ścieżka zachowała 7/7; następnie wykonano dokładnie jeden niepodniesiony `docker container inspect` backendu, exit 0 |
| Potwierdzone testy bieżącego wykonania | Static exact-byte preflight: recipe/index hashes zgodne; recipe-index `17/17` i referenced input-index `29/29` present/size/hash PASS; `installer-execution` absent. Sześć ścieżek baseline oczekiwanych przez runtime recepty nie istnieje. Docker/Task/HTTP/resource preflight, produkt, Flutter/Web/backend/backup i warm runs `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY root `p4b-native-transport-20260918T152017Z` zachowany bez zmian; recepta SHA-256 `5F310C64...1FD67`, indeks `43F285C2...A2A9`. Poprawne baseline records nadal leżą w indexed previous-resume paths. Payload/manifest pozostają NOT_INSTALLED |
| Niezakończone procesy i skutki operacyjne | Brak uruchomionego instalatora i brak własnych procesów pozostawionych przez preflight. `mutation_started=false`, UAC 0, rollback 0. Zachowany wcześniejszy stan: wrapper w rollbacku, Host disabled/no-trigger, pięć tasków na preimage, helper legacy; Supervisor `INTENTIONALLY_STOPPED` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260918T163325Z-R04-D21-P4B-EXACT-RECIPE-PREFLIGHT-BLOCKED.md` |
| Zakres aktualnej zgody | Odczyt istniejących dowodów, static preflight exact recipe/index i dokumentacja. Warunkowy UAC nie został aktywowany, ponieważ preflight nie przeszedł |
| Blokada / wymagana decyzja | Exact recipe ma błędne runtime binding sześciu baseline paths. Wymaga nowej, przetestowanej recepty/indeksu i późniejszej nowej decyzji; nie wolno kopiować plików ani uruchamiać innych bajtów jako obejścia. Docker/WSL pool availability i swap-used pozostają `UNKNOWN`; Qdrant/VHD/profile, backup proof i UI są otwarte |
| Jeden następny bezpieczny krok | Przygotować i offline przetestować nową receptę/index wiążące sześć baseline records z ich exact indexed paths; dopiero później review, fresh bounded drift check i nowa jednorazowa zgoda/UAC |
...
| R04 | IN_PROGRESS | `R04-20260918T163325Z-D21-P4B-EXACT-RECIPE-PREFLIGHT-BLOCKED` / P4-A OWNER ACCEPTED + P4/B PARTIAL SAFE INACTIVE | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A source/offline/identity package jest owner-accepted jako `NOT_INSTALLED`. Transport argv w recepcie `5F310C64...1FD67` zachowuje PowerShell 5.1 offline 17/17 i pojedynczy wcześniejszy Docker readback, lecz static exact-recipe preflight wykazał brak sześciu baseline records pod ścieżkami używanymi przez `Assert-ContainersUnchanged`. Indeksowane kopie istnieją pod poprzednim resume i mają zgodne hashe, ale recepta ich nie odczytuje. UAC/Docker/Task/HTTP/mutacje `0`; wrapper pozostaje w rollbacku, Host disabled/no-trigger, istniejące taski na preimage, payload/manifest nie są zainstalowane, warm runs `0/2`, draft/global manifest `NOT_APPROVED`. Nowa operacja wymaga nowej przetestowanej recepty/indexu i osobnej decyzji. Qdrant/VHD/profile, UI, backup proof, live logon/reboot/rollback i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
