...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`, P2 `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`, DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0` jest `OFFLINE_TEST_ONLY / NOT_DEPLOYED`, a guard `0ee0ea50943578e6e552aae23ce1688595ddc262` jest `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. D21-P3 ma teraz `ROLLBACK_TOOL_PROVENANCE_RECONCILED / EVIDENCE_READY_FOR_REVIEW`: dane z punktu `RecoveryPointV2/full/CaptureOnly` w `E:\ai-lab-backup\20260917T082022Z`, manifest `2759D684...95597`, oraz izolowany drill zachowują wynik `CAPTURE_AND_ISOLATED_DATA_RESTORE_PASS / VERIFIED_LINKS_IN_TESTED_SCOPE`. Błędne identyfikatory narzędzi w raporcie sklasyfikowano jako `REPORT_TRANSCRIPTION_ERROR`; brak owner/run labels, nieautoryzowany pull `alpine:3.20` i pierwsze niedostarczone kopiowanie tmpfs pozostają jawne bez retroaktywnego PASS. Spójność nadal jest `COMPONENT_WINDOWS_RECORDED_NON_TRANSACTIONAL`. Produkcyjny manifest pozostaje `NOT_APPROVED_FOR_START`; cutover, relokacja, task install, live startup, reboot i operacyjne P3–P5 są `NOT_RUN / NOT_AUTHORIZED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. D21-P3 jest `ROLLBACK_TOOL_PROVENANCE_RECONCILED / EVIDENCE_READY_FOR_REVIEW`; wynik danych i drillu jest zachowany, a odstępstwa mają `NO_RETROACTIVE_PASS`. Draft manifestu nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T101953Z-D21-P3-ROLLBACK-PROVENANCE` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T10:19:53Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 read-only rollback tool provenance i korekta zanonimizowanych dokumentów; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P3 preparation` — `ROLLBACK_TOOL_PROVENANCE_RECONCILED / EVIDENCE_READY_FOR_REVIEW`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking `d64062261d0ff76ae5a9152e1e41803116d35692`; capture entry/tool-root `e3f2125883420a12c8721c5ba597203dcf403add`; accepted guard source `0ee0ea50943578e6e552aae23ce1688595ddc262`; accepted DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0`. Oryginalny preservation `203/203` i zastane runtime/data/junction pozostały nietknięte |
...
| Źródła runtime / release / DB | Capture potwierdził ten sam backend `9d9b46c5...615a`, image `sha256:6342b36f...d63702`, `/app=deploy-main`, `/data=C:/ai-lab-core/data`, PostgreSQL `240343eb...1226`, stable `1.0.2+29` i schema `followup_assistant_chat_history_20260829`; recovery/WIP nie jest runtime. Po cleanupie pełne ID sześciu kontenerów pozostały bez zmian. Web P2 pozostaje TEST_ONLY, Supervisor nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | Read-only reconciliation entry/evidence Git objects, raw/canonical bytes i zachowanych logów: trzy błędne „bloby”, błędny tool commit i trzy błędne SHA-256 raportu skorygowano jako `REPORT_TRANSCRIPTION_ERROR`; wynik capture/drillu zachowano |
| Potwierdzone testy bieżącego wykonania | `rev-parse`/`cat-file` potwierdziły cztery niezmienione bloby entry→evidence; `hash-object --path/--no-filters`, binary-safe SHA-256 i kontrola EOL/BOM rozdzieliły raw checkout od canonical Git. Immutable manifest nadal `2759D684...95597`; aplikacja/Docker/SQL/Flutter/Web `NOT_RUN` w tej sesji provenance |
| Niezacommitowana praca / zabezpieczenie | Guard nie jest już lokalnym WIP; pięć przetestowanych ścieżek jest w source `0ee0ea5...`. Historyczne patche SHA-256 `E043325F...EC0E3` i `78A8394E...3A4DA` pozostają dowodem pre-commit. Oryginalny worktree, snapshot `318A41...CC6C3`, Web `A99FB9...C41`, dane D:, backupy i profile pozostały bez zmian |
| Niezakończone procesy i skutki operacyjne | W sesji provenance brak nowych procesów/zasobów operacyjnych. Historyczne zasoby drillu pozostają rozliczone jako cleanup `5/3/3`, remaining `0/0/0`; backup i verify są chronione na E:. `alpine:3.20` pozostaje realnym odstępstwem pull/run z niepełnym command/mount/network evidence i bez retroaktywnego PASS. Supervisor `INTENTIONALLY_STOPPED` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T101953Z-R04-D21-P3-ROLLBACK-PROVENANCE.md`; poprzedni handoff: `docs/recovery/checkpoints/20260917T090901Z-R04-D21-P3-FRESH-ROLLBACK.md` |
| Zakres aktualnej zgody | D21-P3: lokalny read-only evidence review i sanitized correction po już wykonanym capture/drillu; bez nowych Docker/SQL/backup/restore/cleanup, zmian źródeł, produkcyjnego restore/cutoveru, instalacji, startu/restartu, zmian runtime/config/data/mount/task/flag/queue, escrow lub P4/P5 |
| Blokada / wymagana decyzja | Punkt po korekcie provenance ma `EVIDENCE_READY_FOR_REVIEW`, nie `ACCEPTED`. Produkcyjny manifest nie jest zatwierdzony; proceduralne odstępstwa pozostają; pending work 18/16/1 musi zostać zachowany; przed cutoverem trzeba ocenić zapisy po oknach punktu i uzyskać osobną zgodę operacyjną |
| Jeden następny bezpieczny krok | Review dokładnego punktu `20260917T082022Z` z rozliczonym provenance i odstępstwami; następnie osobna decyzja właściciela o jego ograniczonym odbiorze i minimalnym oknie P3 backend source-switch, bez automatycznego restore danych |
| Warunek STOP | Bez instalacji pod `C:\ai-lab-core`, uruchomienia launchera/aplikacji, zatwierdzenia manifestu startu, zmian tasków/shortcutów/mountów/config/flag, usług/kolejek/modeli, kolejnego backupu/restore, cutoveru, deploy/release i P4–P5/R05-A5/R06–R24 |
...
| R04 | IN_PROGRESS | `R04-20260917T101953Z-D21-P3-ROLLBACK-PROVENANCE` / TOOL PROVENANCE RECONCILED, EVIDENCE READY FOR REVIEW | A3 oraz D-21 P1/P2/DATA_ONLY/guard zachowują odbiory. Fresh `RecoveryPointV2/full/CaptureOnly` `20260917T082022Z` zachowuje pełny artifact hash pass i izolowany data restore pass dla PostgreSQL/storage/Qdrant/release/config/inventory/opaque n8n. Tool root `e3f212588...` i kanoniczne bloby zostały rozliczone; poprzednie identyfikatory były błędami transkrypcji. Labels/alpine/tmpfs pozostają proceduralnymi odstępstwami bez retroaktywnego PASS. Spójność `RECORDED_NON_TRANSACTIONAL / VERIFIED_LINKS_IN_TESTED_SCOPE`; pending work 18/16/1 pozostaje do zachowania. Manifest nadal `NOT_APPROVED_FOR_START`; cutover, relokacja, task install, live startup/reboot/produkcyjny restore i runtime acceptance są `NOT_RUN` |
...
3. Historyczna reguła przed D-21 dopuszczała odrębne odtwarzalne deployment roots. D-21 zastępuje ją dla stanu docelowego: `SINGLE_INSTALL_ROOT=C:\ai-lab-core`, jawne wersjonowane podkatalogi komponentów i jeden zwykły, idempotentny punkt startu. Dokładny junction `C:\ai-lab-core\data -> D:\ai-lab-data` jest świadomym wyjątkiem `ACTIVE_DATA_ONLY`; kod/skrypty/executable/CWD nie mogą z niego korzystać. P1 source/offline tests i P2 preservation/candidate są zaakceptowane jako NOT_DEPLOYED. Destination guard source `cb6e225...` z evidence `e9c17933...` jest zaakceptowany jako `OFFLINE_TEST_ONLY / NOT_DEPLOYED`; wiąże dokładny, case-sensitive kontrakt service/role/destination. Guard seed/reconcilera `0ee0ea5...` jest `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. P3 fresh point `E:\ai-lab-backup\20260917T082022Z`, manifest `2759D684...95597`, zachowuje pełny artifact hash pass i izolowany restore pass. Tool-root commit `e3f212588...`, kanoniczne bloby writer/reader/Qdrant helpera i ich raw-file hashes zostały rozliczone; błędne wcześniejsze identyfikatory są `REPORT_TRANSCRIPTION_ERROR`. Brak pełnych owner/run labels, nieautoryzowany pull/run `alpine:3.20` i pierwsze niedostarczone kopiowanie tmpfs pozostają proceduralnymi odstępstwami bez retroaktywnego PASS. Spójność nadal jest `COMPONENT_WINDOWS_RECORDED_NON_TRANSACTIONAL / VERIFIED_LINKS_IN_TESTED_SCOPE`; punkt ma `EVIDENCE_READY_FOR_REVIEW`, nie `ACCEPTED`. Produkcyjny manifest pozostaje `NOT_APPROVED_FOR_START`; instalacja, cutover, relokacja, test live startup/rollback, cleanup i operacyjne P3–P5 wymagają osobnych zgód. Produkcja nie może wykonywać recovery/staging/test WIP.
