...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. Punkt rollbacku ma ograniczony odbiór właściciela; D21-P3 jest `CORE_SWITCH_PREFLIGHT_READY / WAITING_OWNER_WINDOW / NO_CUTOVER`. Globalny manifest nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T143219Z-D21-P3-CORE-SWITCH-PREFLIGHT` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T14:32:19Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 faza A: read-only preflight, inertny payload, dokładny override i rollback; bez drugiego autora i bez mutacji produkcji |
| Aktywny pakiet / podetap | `R04 / D-21 / P3 core backend source switch` — `CORE_SWITCH_PREFLIGHT_READY / WAITING_OWNER_WINDOW / NO_CUTOVER`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `ca6a1f51cc378cb032a375c39f0ef1811dcce3bb`; accepted guard source `0ee0ea50943578e6e552aae23ce1688595ddc262`; accepted DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0`. Oryginalny preservation `203/203`, runtime/data/junction i `.git` pozostały nietknięte |
...
| Źródła runtime / release / DB | Bieżący odczyt potwierdził backend `9d9b46c5...615a`, image `sha256:6342b36f...d63702`, `/app=deploy-main`, `/data=C:/ai-lab-core/data`, PostgreSQL `240343eb...1226` i schema `followup_assistant_chat_history_20260829`; recovery/WIP nie jest runtime. Public Gateway/Web działa, `/control*` jest 404. Web P2 pozostaje TEST_ONLY, Supervisor nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | Faza A przygotowała kanoniczny payload 592 plików z zero blob mismatches, dokładny backend-only override i jeden rollback plan; Compose v5.5.1 zwalidował oba łańcuchy bez utrwalenia pełnego renderu. Nie wykonano cutoveru |
| Potwierdzone testy bieżącego wykonania | Bounded runtime/Engine/resources read, Public Gateway/Web/control GET, jedna transakcja PostgreSQL READ ONLY z ROLLBACK, source/image/schema compatibility review, `docker compose config --no-interpolate` dla kandydata i rollbacku. Aplikacja kandydata, Flutter, Web build, migracje, start/recreate i runtime smoke `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Lokalny chroniony root `...\p3-core-switch-20260917T141404Z` zawiera payload, rejected-CRLF próbę, manifesty, override i rollback plan. Payload aggregate `7A65EDFC...48B055`, override `F99BABA9...E4E86`, plan `8F5691A5...1E78`. Oryginalny backend ma osobny pełny manifest; niczego nie przeniesiono |
| Niezakończone procesy i skutki operacyjne | Brak pozostawionych procesów testowych i brak mutacji produkcji. Sześć kontenerów pozostaje w zastanym stanie, Supervisor `INTENTIONALLY_STOPPED`. Backup/junction/data/taski/harmonogramy niezmienione; historyczne odstępstwa drillu zachowane |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T143219Z-R04-D21-P3-CORE-SWITCH-PREFLIGHT.md`; poprzedni handoff: `docs/recovery/checkpoints/20260917T101953Z-R04-D21-P3-ROLLBACK-PROVENANCE.md` |
| Zakres aktualnej zgody | D21-P3 faza A: read-only preflight, chroniony inertny payload, dokładny override i plan rollbacku. Faza B — zmiana backend directory/config/mount/flags i jeden recreate — wymaga bieżącego potwierdzenia właściciela dokładnego OP_ID |
| Blokada / wymagana decyzja | `WAITING_OWNER_WINDOW`: pełna delta treści po component windows jest `DELTA_NOT_FULLY_OBSERVED`; pending work `18/16/1` ma zostać zachowany. Właściciel musi przyjąć te ograniczenia, przerwę i jawne ryzyko legacy rollbacku. Globalny manifest P1 pozostaje niezatwierdzony |
| Jeden następny bezpieczny krok | Review pakietu OP_ID `R04-D21-P3-CORE-SWITCH-20260917T141404Z` i jedno bieżące potwierdzenie fazy B; potem finalny drift check przed pierwszą mutacją |
| Warunek STOP | Do bieżącego potwierdzenia: bez zapisu w instalacji, move/copy backendu, instalacji override, stop/recreate/startu, zmian tasków/data/junction/flag/kolejek, backupu/restore, P4–P5/R05-A5/R06–R24 |
...
| R04 | IN_PROGRESS | `R04-20260917T143219Z-D21-P3-CORE-SWITCH-PREFLIGHT` / WAITING OWNER WINDOW, NO CUTOVER | A3 oraz D-21 P1/P2/DATA_ONLY/guard zachowują odbiory. Właściciel przyjął dowód danych fresh pointu z zapisanymi ograniczeniami; odstępstwa i component windows pozostają. Faza A potwierdziła runtime/DB/resources, zachowała pending 18/16/1 i przygotowała OP_ID `R04-D21-P3-CORE-SWITCH-20260917T141404Z`, payload `7A65EDFC...48B055`, override `F99BABA9...E4E86` (nieaktywny config source `f5cc96f...308b0`) oraz rollback plan `8F5691A5...1E78`. Pełna delta treści jest `DELTA_NOT_FULLY_OBSERVED`. Faza B, globalny manifest, relokacja, task install, live startup/reboot/produkcyjny restore i runtime acceptance są `NOT_RUN / NOT_AUTHORIZED` do bieżącego potwierdzenia właściciela |
