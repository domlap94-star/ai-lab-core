...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_AND_OFFLINE_TESTS_READY_FOR_REVIEW / IDENTITY_READBACK_PARTIAL / NOT_INSTALLED`; runtime name/RepoDigests `NOT_VERIFIED / BLOCKING_P4B`. Globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T225125Z-D21-P4A-IDENTITY-COLD-START` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T22:51:25Z` |
| Aktualny wykonawca / sesja | Codex / P4-A full-ID i PostgreSQL-health source/offline zamknięte; runtime identity readback nieutrwalony; zero instalacji i zero mutacji runtime; STOP przed P4-B/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-A identity and cold-start continuation` — source/offline `READY_FOR_REVIEW`, identity readback `PARTIAL`, `NOT_INSTALLED`; cały R04 nadal `IN_PROGRESS` |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T225125Z-R04-D21-P4A-IDENTITY-COLD-START.md` |
...
| Blokada / wymagana decyzja | Jeden poprawnie utrwalony readback runtime `.Name`, pełnych ID i `RepoDigests`; ponadto Qdrant backing, `OPEN_AFTER_BASE_READY`, global manifest approval, task/trigger install, P4-B operational matrix, backup schedule proof i relokacje D: pozostają otwarte |
| Jeden następny bezpieczny krok | Osobno zatwierdzony, poprawnie rejestrowany readback runtime identity sześciu przypiętych zasobów; potem owner review wynikowego draftu przed jakąkolwiek zgodą P4-B |
...
| R04 | IN_PROGRESS | `R04-20260917T225125Z-D21-P4A-IDENTITY-COLD-START` / P3 ACCEPTED LIMITED + P4-A SOURCE/OFFLINE READY, IDENTITY PARTIAL | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A full-ID/image/file-role/DB-health source i testy offline są `READY_FOR_REVIEW / NOT_INSTALLED`. Bieżąca runtime name/RepoDigests projection jest `NOT_VERIFIED / BLOCKING_P4B`, więc globalny manifest pozostaje `NOT_APPROVED`. Qdrant backing, `OPEN_AFTER_BASE_READY`, task install, live startup/reboot/rollback, backup schedule proof, relokacja i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
