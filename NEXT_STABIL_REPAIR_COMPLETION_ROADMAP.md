...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują odbiory; D-23 review pozostaje 2/2. `USABLE_WARM_ACCEPTED / LIMITED_RUNTIME_SCOPE` pozostaje w mocy. OP `R04-D21-ONE-ENTRY-COLD-COMPLETE-20260925T183020Z` ma zainstalowane przypięte launcher i recorder, ale installed approved manifest nadal wiąże ich poprzednie hashe. Dokładna, zwalidowana pochodna V2 ma SHA-256 `53CA6E98...B6972EC9`. UAC PID `49688` zweryfikował exact manifest preimage, lecz PowerShell 5.1/.NET odrzucił `[IO.File]::Replace(...,$null)` przed mutacją jako niedozwolony format ścieżki. Ograniczony readback potwierdził installed manifest nadal `33551 B / 6219F...0BBA`; Host/repeat/rollback/restart `0`. Stan to `MANIFEST_PREIMAGE_VERIFIED / ATOMIC_REPLACE_ARGUMENT_BLOCKED / NO_MANIFEST_MUTATION / NOT_READY_FOR_RESTART`; nowa decyzja jest wymagana. Supervisor pozostaje `INTENTIONALLY_STOPPED`; Docker/WSL pool i swap pozostają UNKNOWN. D-22 pozostaje `NOT_RUN`. Bieżący stan znajduje się wyłącznie w §0.**
...
| Checkpoint ID | `20260925T211335Z-R04-D21-ONE-ENTRY-COLD-MANIFEST-REPLACE-BLOCKED` |
...
| Aktualny wykonawca / sesja | Codex / ten sam ONE-ENTRY-COLD OP; UAC PID 49688 verified preimage, atomic replace blocked before mutation |
| Aktywny pakiet / podetap | R04/D-21 — `MANIFEST_PREIMAGE_VERIFIED / ATOMIC_REPLACE_ARGUMENT_BLOCKED / NO_MANIFEST_MUTATION / NO_HOST_START / NOT_READY_FOR_RESTART`; USABLE-WARM zachowuje odbiór |
...
| Ostatnia faktycznie zakończona czynność | UAC PID `49688` zweryfikował exact preimage `33551 / 6219F...0BBA`; `[IO.File]::Replace(temp,target,$null)` zwrócił niedozwolony format przed replacement, a ograniczony readback potwierdził manifest bez zmian |
...
| Niezakończone procesy i skutki operacyjne | PID `49688` rozliczony; brak final result, ale journal+manifest-only readback dowodzą no mutation/pending false. Manifest write/Host/repeat/rollback/reboot `0`; launcher+recorder nowe, manifest bindings stare; operator usunięty |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260925T211335Z-R04-D21-ONE-ENTRY-COLD-MANIFEST-REPLACE-BLOCKED.md` |
| Zakres aktualnej zgody | Exact UAC i preimage check są zużyte. Replace został odrzucony przed mutacją; limited readback zakończony. Restart niezatwierdzony |
| Blokada / wymagana decyzja | PowerShell 5.1/.NET nie zaakceptował null backup path w `File.Replace`; installed manifest pozostał na exact preimage. Nowy UAC z konkretnym same-volume backup path wymaga nowej decyzji |
