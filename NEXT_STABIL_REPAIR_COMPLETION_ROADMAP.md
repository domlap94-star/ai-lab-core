...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują dotychczasowe odbiory; D-23 review pozostaje 2/2. USABLE-WARM Stage B historycznie zainstalowała cztery exact pliki. Najnowsza owner-authorized kontynuacja `R04-D21-P4B-USABLE-WARM-CONTINUE-HOST-RECONCILE-20260924T192425Z` przeszła offline `14` scenariuszy / `66` asercji i bieżący bounded preflight. Jeden UAC/PID `56548` zakończył proces rodzica exit `22` z historycznie zachowanym `PARTIAL_PENDING_OPERATION_UNKNOWN`, ale późniejszy recorder i dozwolony post-check rozliczyły własny pierwszy Host: wynik `BASE_READY_LIMITED_CAPTURED`, Private `START_ONCE=1`, Host `Ready`/idle/LastResult `0`, sześć exact kontenerów bez zmiany, Public present, Supervisor absent i HTTP `200/200/200/404`. Jest to `WARM_RUNS_1_OF_2`; repeat/no-duplicate, logon i CRM/Web pozostają `NOT_RUN`. UAC jest zużyty; nie wykonano retry ani SAFE_INACTIVE. D-22 nadal ustala kolejność po właścicielskim odbiorze R04: ARKUSZE -> KOREKTA I WALIDACJA KLIENTÓW -> TYLKO NIEPRZYPISANE MAILE; wykonanie NOT_RUN. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. Host22 i NUP-01/02/03 zachowują odbiory, D-23 review `2/2` jest zakończony. Cztery pliki pozostają zainstalowane i niezmienione. Host-reconcile wykorzystał exact Host on-demand i uruchomił jeden recorder-backed warm run. Rodzic zachowuje historyczny wynik `PARTIAL_PENDING_OPERATION_UNKNOWN`, natomiast późniejszy exact recorder/post-check rozlicza pierwszy run jako zakończony: Private `1`, Host `Ready`/idle/LastResult `0`, sześć kontenerów bez zmiany, Supervisor absent. Warm runs `1/2`; repeat/no-duplicate, logon i CRM/Web `NOT_RUN`; bez retry i SAFE_INACTIVE. Cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-D21-P4B-USABLE-WARM-CONTINUE-HOST-RECONCILE-20260924T192425Z` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-24T20:07:48.2403427Z`: recorder zakończył pierwszy Host exit `0`, Private `START_ONCE=1`; post-check potwierdził Host exact/Ready/idle, Private present, Public present, Supervisor absent, sześć kontenerów bez zmiany i HTTP `200/200/200/404`. Drugi run/logon/CRM-Web nie zostały wykonane |
| Aktualny wykonawca / sesja | Codex / OWNER-AUTHORIZED HOST-RECONCILE CONTINUATION PARTIAL AFTER FIRST COMPLETED WARM RUN |
| Aktywny pakiet / podetap | R04/D-21/P4-B — `HOST_RECONCILE_OFFLINE_PASS / READ_ONLY_PREFLIGHT_PASS / FIRST_WARM_RECORDER_COMPLETE / WARM_RUNS_1_OF_2 / REPEAT_NOT_RUN / LOGON_NOT_CONFIGURED / CRM_WEB_NOT_OPENED / UAC_CONSUMED` |
...
| Ostatnia faktycznie zakończona czynność | Jeden dozwolony post-check o `2026-09-24T20:07:48.2403427Z`: Host `Ready`, enabled, trigger_count `0`, Running/Queued `0`, LastRun `20:06:05Z`, LastResult `0`, exact semantic/comparable `66ECF758...B938` / `17195620...BA92`; Private i Public present, Supervisor absent; sześć pinned kontenerów nadal running, PostgreSQL healthy; HTTP `200/200/200/404`. Nie wykonano drugiego runu ani dalszego zapisu |
| Potwierdzone testy bieżącego wykonania | LOCAL_ONLY host-reconcile `6C3155EE...3A690` / index `039166A4...5A0E`: PS5.1 `14` scenariuszy / `66` asercji, granice produkcyjne `0`. Preflight `12304` B / `CAB2009A05D85FA92EF8AA58C9BDDB2E309CB52962CC7AE5F38358F7DA910BEE` PASS. Parent result `1816` B / `1566C0804337174617E93D820BE559379727EE7DEF9BD5ABB71E58DC35312B2D` zachowuje partial. Recorder result `6025` B / `F252947792F5B12CADCFDB6684EC824F591F8186E3B548A94531B8DD564784B9` oraz stdout `3094` B / `A6EACD08956122702D50FDF3AE4532FA016B417BFC4DFF985A66CA829880F175` potwierdzają pierwszy warm run. Docker/WSL pool i swap pozostają UNKNOWN, nie PASS |
| Niezacommitowana praca / zabezpieczenie | Historyczne dowody pozostają chronione. Pochodna, testy i wyniki są LOCAL_ONLY pod `C:\Users\domai\AppData\Local\Temp\P4B-UWC-01`; `apply-host-reconcile01` zachowuje preflight, journal, result i post-check. Recorder evidence pozostaje pod aktywnym data root. Zainstalowanych czterech plików nie kopiowano ani nie zmieniano |
| Niezakończone procesy i skutki operacyjne | Elevated PID `56548` i recorder child PID `36168` są zakończone. Pierwszy Host run zakończył się exit `0`; Private start `1`. Drugi Host run `0`, logon `0`, shortcut/CRM-Web `0`, SAFE_INACTIVE `0`. Brak nierozliczonego własnego workera; rodzic zachowuje swój wcześniejszy partial jako historię |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260924T200945Z-R04-D21-P4B-USABLE-WARM-HOST-RECONCILE-PARTIAL.md` |
| Zakres aktualnej zgody | Jednorazowa zgoda obejmowała host-reconcile, bounded preflight, jeden UAC, dwa warm runs, logon i CRM/Web. UAC wykorzystano, lecz operacja zakończyła rodzica po pierwszym handoffie; późniejszy dowód rozliczył tylko pierwszy run. Zgoda jest zużyta i nie obejmuje drugiego runu, logon, retry ani rollbacku |
| Blokada / wymagana decyzja | Exact Host jest obecnie enabled/no-trigger/idle, a Private już present po ukończonym pierwszym runie. Zamrożony cel pozostały to jedna odrębna powtórka recorder-backed oczekująca Private start `0`, następnie logon i CRM/Web. Nie wolno użyć ponownie zużytej zgody ani sztucznie zatrzymywać Private |
| Jeden następny bezpieczny krok | Jedna nowa decyzja właściciela dla dokładnej kontynuacji od bieżącego stanu: bez ponownej rejestracji/kopiowania, jeden repeat warm run oczekujący `0` nowych startów, po sukcesie logon i read-only CRM/Web. Do tego czasu STOP |
...
| R04 | IN_PROGRESS | `R04-D21-P4B-USABLE-WARM-CONTINUE-HOST-RECONCILE-20260924T192425Z` / FIRST WARM COMPLETE, WINDOW PARTIAL | P3/P4-A, Host22 i NUP-01/02/03 odbiory zachowane; D-23 review `2/2`. Host-reconcile przeszedł offline i realny preflight. Jeden UAC/PID `56548` zakończył rodzica exit `22`; późniejszy recorder i post-check potwierdziły dokładnie jeden zakończony Host run, Private `START_ONCE=1`, Host Ready/idle/LastResult `0`, sześć kontenerów bez zmiany i Supervisor absent. Warm `1/2`; repeat/no-duplicate, logon i CRM/Web nie zostały wykonane. Bez retry/SAFE_INACTIVE; P5/D-22 nieuruchomione |
