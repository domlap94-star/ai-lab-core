...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A3 zostało zaakceptowane przez właściciela jako `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED` dla source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae`, przy zachowaniu dwóch braków dowodowych. D-20 uruchomiło R05-A1; wynik bieżący `SOURCE_PARTIAL / NOT_DEPLOYED`, bo Vision V1 nadal omija nową kontrolę. R04/R05 pozostają `IN_PROGRESS`; R06–R24 nieuruchomione |
| Checkpoint ID | `R05-20260911T133256Z-A1-SOURCE-PARTIAL` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-11T13:32:56Z` |
| Aktualny wykonawca / sesja | Codex / R05-A1 source/test: bramka Visual V2 sprawdzona syntetycznie, pozostała aktywna alternatywna granica Vision V1; bez startupu i realnego eksportu |
| Aktywny pakiet / podetap | `R05 / A1 privacy and activation` — `SOURCE_PARTIAL / NOT_DEPLOYED`; cały R05 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; parent local/remote `457a4321db835421c0d06847e2132d9623d735ae`. Cztery pliki source/test są niezacommitowanym WIP. Patch `LOCAL_ONLY`: `C:\ai-lab-core-staging\recovery\R05_A1_WIP_20260911T133256Z\r05-a1-source-wip.patch`, SHA-256 `357AF58A42C94ADC09C88DFB2F15D5C36538C6035275ECC72B79E4409C65D958` |
| Gałąź / SHA kodu objętego sprawdzeniem | accepted R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc`; evidence/start `457a4321db835421c0d06847e2132d9623d735ae`; source baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a`; R05-A1 badano jako jawnie hashowany WIP, nie jako commit SOURCE_PASS |
...
| Ostatnia faktycznie zakończona czynność | Zabezpieczono R05-A1 WIP i dowód testowy; ustalono, że `VisionProcessingService.advance()` pozostaje alternatywną granicą eksportu bez dopuszczenia dokładnych bajtów. Próba poszerzenia diffu na tę publiczną/runtime ścieżkę została zatrzymana przez bramkę bezpieczeństwa, bez obejścia |
| Potwierdzone testy bieżącego wykonania | Fail-before: `2 failed`, exit `1` — brak flagi oraz niedopuszczone piksele osiągały fake Supervisora. Pass-after Visual/Assistant: `68 passed`, exit `0`; syntetyczna DB ingestion/chat: `18 passed`, exit `0`; jedna wcześniejsza próba DB odrzucona przez guard nazwy, bez wykonania testów. `test_followup_chunk13_api_auth.py = NOT_RUN`, bo wymaga pełnego lifespan |
...
| Niezakończone procesy i skutki operacyjne | Brak uruchomionych zasobów R05. Własny syntetyczny PostgreSQL i sieć zostały usunięte po kontroli ownera/ID. Patch i log są `LOCAL_ONLY`. Aplikacja, dispatchery, Supervisor, Temporary Chat, modele i realny eksport: `NOT_RUN` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260911T133256Z-R05-A1-SOURCE-PARTIAL.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260911T095238Z-R04-A3-HANDOFF.md` |
| Zakres aktualnej zgody | D-20: polityka exact-byte, source/test Visual V2, konfiguracja/lifespan i odpowiadające call-sites wyłącznie na syntetycznej DB/filesystem z fake granicami; bez normalnego startupu, realnego Supervisora/Temporary Chat/modelu, eksportu i deploymentu |
| Blokada / wymagana decyzja | Aktywna ścieżka Vision V1 (`vision_dispatcher.py` oraz publiczne call-sites w `documents/router.py` i `technical_ai_service.py`) nadal może ominąć exact-byte approval. Jej kompatybilne objęcie/wycofanie wymaga osobnej jawnej zgody na szerszą publiczną/runtime zmianę i auth tests |
| Jeden następny bezpieczny krok | Właściciel ocenia `SOURCE_PARTIAL` i podejmuje osobną decyzję o dokładnie wskazanej migracji/containment Vision V1; do tego czasu WIP pozostaje LOCAL_ONLY, bez source commit, startupu i eksportu |
...
| R05 | IN_PROGRESS | `R05-20260911T133256Z-A1-SOURCE-PARTIAL` / EXACT-BYTE WIP | Visual V2 ma syntetycznie sprawdzoną bramkę aktywacji i exact-byte approval (`68 + 18 passed`), ale alternatywna ścieżka Vision V1 nadal omija ją. WIP pozostaje `LOCAL_ONLY / NOT_DEPLOYED`; real export i end-to-end `NOT_VERIFIED` |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
| R09 | PLANNED | — | — |
