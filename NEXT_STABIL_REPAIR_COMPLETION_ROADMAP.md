...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION jako cały pakiet. Właściciel zaakceptował R03-A4 na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`. R04-A1 jest zaakceptowane na `f4ea20c74f92c0423db087ba8d60bb8cc7f2ec99` jako source/test i pozostaje `NOT_DEPLOYED`. Source/test `R04-A2-UI06` na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` pozostaje zaakceptowane wyłącznie dla `DocumentsController`. Skorygowany, route-first scenariusz A oraz wcześniejszy scenariusz B mają `PASS / OWNER_OPERATED_RUSTDESK / TEST_ONLY`; dwa historyczne A pozostają `NOT_VERIFIED`. Nowe A potwierdziło kontrolowany stan błędu pierwszego `build()` route-local `DocumentsController` podczas outage oraz poprawny pojedynczy retry po powrocie backendu, bez `LateInitializationError`, nieskończonego ładowania, mutacji biznesowej lub duplikatu. Całe A2/R04/R16 nie jest przez Codex oznaczone `ACCEPTED`; wynik czeka na odbiór właściciela. Android runtime jest `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 nadal `WAITING_OWNER_VISUAL_EVIDENCE`. Cały R04 pozostaje IN_PROGRESS. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A1 source/test jest zaakceptowane na `f4ea20c74f92c0423db087ba8d60bb8cc7f2ec99` jako `NOT_DEPLOYED`. Source/test `R04-A2-UI06` na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` pozostaje zaakceptowane wyłącznie dla `DocumentsController`. Skorygowany route-first A i wcześniejszy B mają `PASS / OWNER_OPERATED_RUSTDESK / TEST_ONLY`; oba historyczne A pozostają `NOT_VERIFIED`. Całe A2/R04/R16 pozostają nieodebrane przez właściciela. Android `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 `WAITING_OWNER_VISUAL_EVIDENCE` |
| Checkpoint ID | `R04-20260910T201601Z-A2-UI06-A-ROUTE-FIRST` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-10T20:16:01Z` |
| Aktualny wykonawca / sesja | Codex / pojedynczy owner-operated route-first A zakończony; dowody zanonimizowane i przekazane do odbioru |
| Aktywny pakiet / podetap | `R04 / A2 UI06 A route-first` — `A_ROUTE_FIRST_LOAD_PASS / OWNER_OPERATED_RUSTDESK / TEST_ONLY`; source/test `ACCEPTED / NOT_DEPLOYED`, historyczne A `NOT_VERIFIED`, B `PASS`; cały R04 nadal `IN_PROGRESS` |
...
| Ostatnia faktycznie zakończona czynność | Po bieżącym `GOTOWE` właściciela wznowiono dokładnie zachowany zestaw A2 i wykonano jeden route-first A. Przed outage Dashboard wykonał tylko dozwolony preview `limit=6`; po zatrzymaniu dokładnego backendu wejście menu na `/documents` pokazało route-local błąd z retry. Po uruchomieniu tego samego backendu jedno kliknięcie retry wykonało dokładnie jeden udany GET `limit=50` i zwróciło jeden właściwy dokument. Nie wystąpił `LateInitializationError`, nieskończone ładowanie, mutacja biznesowa ani duplikat. Web i trzy kontenery zatrzymano, telemetry usunięto, porty zwolniono |
| Potwierdzone testy bieżącego wykonania | Statyczne powiązanie caller/provider/route: `PASS`. Route-first runtime: `A_ROUTE_FIRST_LOAD_PASS / OWNER_OPERATED_RUSTDESK / TEST_ONLY`; dwa screenshoty właściciela i skorelowane logi HTTP/DB. Monitor `157/157 PASS`; minima: Windows available `5.363 GiB`, commit reserve `33.771 GiB`, Docker/WSL pool available `13.774 GiB`, swap growth `0`. Testy aplikacyjne: `NOT_RUN` — source/test nie zmieniono. Historyczne A pozostają `NOT_VERIFIED`; B pozostaje `PASS / OWNER_OPERATED_RUSTDESK` |
...
| Niezakończone procesy i skutki operacyjne | Trzy exact-name kontenery A2 są `exited`, Web zatrzymany, monitor zakończony kodem 0, dokładna telemetry usunięta, porty `18004/18005` mają zero listenerów. Syntetyczna DB/volume/storage/cache/APK i dowody zachowane `LOCAL_ONLY`; produkcja i zasoby R03 nietknięte |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260910T201601Z-R04-A2-UI06-A-ROUTE-FIRST.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260910T191650Z-R04-A2-UI06-A-ROUTE-FIRST-START.md` |
...
| Blokada / wymagana decyzja | Route-first A ma komplet dowodu wykonania, lecz cały A2/R04/R16 nie jest samodzielnie zaakceptowany przez Codex. W-02 pozostaje osobne `WAITING_OWNER_VISUAL_EVIDENCE`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Jeden następny bezpieczny krok | Odbiór właściciela dla route-first A i połączonego zakresu A/B; bez nowej zgody nie uruchamiać Androida, W-02, dalszego R04, R05 ani naprawy `ClientsController` |
...
| R04 | IN_PROGRESS | `R04-20260910T201601Z-A2-UI06-A-ROUTE-FIRST` / A2 UI06 READY_FOR_OWNER_REVIEW | A1 source/test zaakceptowane na `f4ea20c74f92c0423db087ba8d60bb8cc7f2ec99` jako `NOT_DEPLOYED`. UI06 source/test zaakceptowane na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` wyłącznie dla `DocumentsController`; skorygowany route-first A i wcześniejszy B mają `PASS / OWNER_OPERATED_RUSTDESK / TEST_ONLY`, a dwa historyczne A pozostają `NOT_VERIFIED`. Całość A2/R04/R16 nadal wymaga odbioru właściciela. `ClientsController` nieodebrane; Android `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 `WAITING_OWNER_VISUAL_EVIDENCE`. Modele, release i produkcja poza zakresem |
| R05 | PLANNED | — | — |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
...
- `R04-A2-UI06`: po utracie i odzyskaniu transportu aplikacja pokazuje właściwy komunikat i bezpiecznie ponawia operację bez wewnętrznego wyjątku oraz bez duplikacji. Właściciel zaakceptował source/test fix na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` wyłącznie dla potwierdzonego błędu `DocumentsController`; nie jest to odbiór analogicznego ryzyka `ClientsController`, całego R16 ani deploymentu. Ręczny Web B potwierdził błąd połączenia i poprawny retry bez wewnętrznego wyjątku. Skorygowany route-first A potwierdził kontrolowany stan błędu pierwszego ładowania `DocumentsController` na `/documents` podczas outage oraz poprawny jeden retry po powrocie tego samego backendu; dokładnie jeden GET `limit=50` zwrócił jeden dokument, bez `LateInitializationError`, nieskończonego ładowania, mutacji lub duplikatu (`PASS / OWNER_OPERATED_RUSTDESK / TEST_ONLY`). Dwa wcześniejsze A zachowują historyczne `NOT_VERIFIED`. Całość A2/R04/R16 nadal oczekuje na odbiór właściciela i wymagany zakres wieloplatformowy.
