...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B zachowuje `PARTIAL_SAFE_INACTIVE`; recepta ma `EXACT_ROLLBACK_SAFE_RECIPE_ACCEPTED / SOURCE_AND_OFFLINE_SCOPE / NOT_INSTALLED`, a zastępczy odczyt `REPLACEMENT_READ_ONLY_PREFLIGHT_PARTIAL / HOST_TASK_FORMATTER_CIMCLASS_FAILURE`; globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260919T105224Z-D21-P4B-REPLACEMENT-PREFLIGHT-PARTIAL` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-19T10:52:24Z` |
| Aktualny wykonawca / sesja | Codex / jedna zastępcza kampania read-only zakończona PARTIAL po utrwaleniu 6 XML-i tasków. Bez Docker/HTTP/UAC/Install/warm runs i bez mutacji instalacji/danych. STOP przed kolejnym live odczytem |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — exact recipe odebrana tylko source/offline; replacement read-only preflight PARTIAL; cały R04 nadal `IN_PROGRESS` |
...
| Ostatnia faktycznie zakończona czynność | Krótki root przeszedł local I/O; bounded collector utrwalił XML sześciu tasków, po czym zatrzymał się na braku `CimClass` w projekcji Host. Pięć tasków exact-match preimage; Host static-match po normalizacji SID/default RunLevel. Dynamicznego stanu Host nie ponowiono |
| Potwierdzone testy bieżącego wykonania | Zachowane Windows PowerShell `5.1.26100.8894`: preimage 4/4/16, input 14/14/110, orchestration 15/15/96, workers 437/437, parser PASS i ZIP roundtrip PASS. Bieżące: local UTF-8/XML/JSON/atomic rename PASS, max path 90/220; task XML 6 zapisane, 5/5 preimage raw match, Host static semantic match; collector exit 1/no timeout. Docker/HTTP/UAC/Install/mutations `0`; warm runs `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY root `C:\ai-lab-core-staging\recovery\P4B-PF-01`; result SHA-256 `58B876CF...73B8A`, index `40A9DA46...C1A02`. Exact recipe/index/ZIP pozostają bez zmian. Payload/manifest pozostają NOT_INSTALLED |
| Niezakończone procesy i skutki operacyjne | Własny collector PID `78188` zakończył się exit 1; brak własnych procesów, instalatora i UAC. Dynamiczny Host/LastRun, dalsze host/resources, Docker i HTTP nie zostały utrwalone/wykonane. Install/mutations/rollback/warm runs 0 |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260919T105224Z-R04-D21-P4B-REPLACEMENT-PREFLIGHT-PARTIAL.md` |
| Zakres aktualnej zgody | Jedna zastępcza kampania read-only została zużyta. Exact recipe pozostaje odebrana tylko source/offline. Żadna zgoda na kolejny live read, UAC lub Install nie pozostaje aktywna |
| Blokada / wymagana decyzja | `HOST_TASK_FORMATTER_CIMCLASS_FAILURE`; dynamiczny stan Host i LastRun nie są fresh evidence. Installer output path write compatibility dla długości 252/275 pozostaje `NOT_VERIFIED`. Docker/WSL pool availability i swap-used pozostają `UNKNOWN`; Qdrant/VHD/profile, backup proof i UI są otwarte |
| Jeden następny bezpieczny krok | Review minimalnej poprawki wyłącznie read-only projekcji triggera oraz osobna decyzja na jeden nowy bounded preflight; bez UAC i Install w tej samej decyzji |
...
| R04 | IN_PROGRESS | `R04-20260919T105224Z-D21-P4B-REPLACEMENT-PREFLIGHT-PARTIAL` / P4-A OWNER ACCEPTED + P4/B PARTIAL SAFE INACTIVE | P3 backend switch pozostaje `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A pozostaje owner-accepted jako `NOT_INSTALLED`. Exact recipe `F6D3A8CC...C883F0E` pozostaje `EXACT_ROLLBACK_SAFE_RECIPE_ACCEPTED / SOURCE_AND_OFFLINE_SCOPE / NOT_INSTALLED`. Owner-authorized replacement read-only campaign pod krótkim rootem zaliczyła local I/O/path budget 90/220 i utrwaliła 6 XML-i tasków; 5/5 historycznych tasków exact-match preimage, Host static semantic match. Collector zakończył się exit 1 na `HOST_TASK_FORMATTER_CIMCLASS_FAILURE`; dynamiczny Host/LastRun nie został utrwalony ani ponowiony. Docker/HTTP/UAC/Install/warm runs/mutacje 0. Status `REPLACEMENT_READ_ONLY_PREFLIGHT_PARTIAL / NO_UAC / NOT_INSTALLED`; globalny manifest `NOT_APPROVED_FOR_START`. Qdrant/VHD/profile, UI, backup proof, live logon/reboot/rollback i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
