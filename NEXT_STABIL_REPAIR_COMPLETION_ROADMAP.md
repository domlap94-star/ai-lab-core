...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B Phase A `WINDOW_READY / WAITING_OWNER_CONFIRMATION / NOT_INSTALLED`; globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260918T091335Z-D21-P4B-WINDOW` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-18T09:13:35Z` |
| Aktualny wykonawca / sesja | Codex / P4/B Phase A wykonała tylko bounded read-only preflight i przygotowała inertny exact pakiet w chronionym stagingu; zero instalacji i zero mutacji runtime; STOP przed Phase B/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B Phase A` — OP_ID `R04-D21-P4B-WINDOW-20260918T084652Z`, `WAITING_OWNER_CONFIRMATION / NOT_INSTALLED`; cały R04 nadal `IN_PROGRESS` |
...
| Ostatnia faktycznie zakończona czynność | Zebrano świeży, ograniczony preflight sześciu kontenerów/tasków/portów/HTTP i zasobów; zachowano preimage oraz przygotowano exact proponowany manifest, osiem XML tasków, payload i changeset w stagingu. Niczego nie zainstalowano ani nie uruchomiono |
| Potwierdzone testy bieżącego wykonania | Walidacja Phase A exit `0`: parsowanie trzech PS1 w Windows PowerShell 5.1, ośmiu XML i JSON, zgodność task identities/hashów oraz semantic delta manifestu. Pure validator przed instalacją odmówił wyłącznie z oczekiwanych `FILE_MISSING/FILE_HASH_MISMATCH`, zero adapterów. Live launcher/start/logon/reboot/rollback, Flutter/Web/backend tests `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Chroniony root P3 pozostaje. P4-A effective index `3FCF1938111C9746FB9F4B5D57F1F98C5085099503B1385B862B0F68776303A9`; Phase-A window index `F99FF54E06DD9508992D9D41D38CE5C8BF54D02748814EDEA619BD046B772FF8`, proposed manifest `E66F22A7EC433183940FC3014E1B9F8C1DBE535375EC2D6580B958B55586010C`, changeset `FC0E884F531F93E922FF0C84AC87D6B6DBF34EC825B6E86796DE702ABE328F0C`. Wszystko LOCAL_ONLY / NOT_INSTALLED |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260918T091335Z-R04-D21-P4B-WINDOW.md` |
| Zakres aktualnej zgody | P4/A owner-accepted; P4/B Phase A read-only preparation i publikacja dokumentacji. Bez instalacji, importu taska, manifest approval lub startu |
| Blokada / wymagana decyzja | Phase B wymaga dokładnego bieżącego potwierdzenia właściciela dla opublikowanego OP_ID/hashów i opisanych ryzyk. Bieżący Docker/WSL pool availability oraz swap-used pozostają `UNKNOWN`; Qdrant/VHD/profile, backup proof i UI są jawnie otwarte |
| Jeden następny bezpieczny krok | Właściciel wysyła dokładne zdanie zgody Phase B dla opublikowanego pakietu albo odmawia; do tego czasu zero operacji |
...
| R04 | IN_PROGRESS | `R04-20260918T091335Z-D21-P4B-WINDOW` / P4-A OWNER ACCEPTED + P4/B PHASE A WINDOW READY | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A source/offline/identity package jest owner-accepted jako `NOT_INSTALLED`. Phase A OP_ID `R04-D21-P4B-WINDOW-20260918T084652Z` przygotowała exact pakiet i przeszła walidację offline, lecz Phase B jest `WAITING_OWNER_CONFIRMATION`. Repo draft/global manifest `NOT_APPROVED`; zero task/install/runtime mutations. Qdrant/VHD/profile, oddzielny UI, backup proof, live logon/reboot/rollback i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
