...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują odbiory; D-23 review pozostaje 2/2. `USABLE_WARM_ACCEPTED / LIMITED_RUNTIME_SCOPE` pozostaje w mocy. OP `R04-D21-ONE-ENTRY-COLD-COMPLETE-20260925T183020Z` zainstalował wyłącznie przypięty launcher i recorder z zachowaniem exact preimages. Jeden repeat zakończył się kompletnym i settled recorder result `MANIFEST_REFUSED`, exit `22`, ponieważ niezmieniony installed approved manifest nadal wiąże poprzednie hashe obu plików. Launcher events były puste, dokładnie jeden wcześniejszy klient pozostał uruchomiony, a retry/restart/logon/cold są `NOT_RUN`. Stan to `TWO_FILE_INSTALL_COMPLETE / RECORDER_COMPLETE / MANIFEST_REFUSED / NOT_READY_FOR_RESTART`; nowa decyzja jest wymagana. Supervisor pozostaje `INTENTIONALLY_STOPPED`; Docker/WSL pool i swap pozostają UNKNOWN. D-22 pozostaje `NOT_RUN`. Bieżący stan znajduje się wyłącznie w §0.**
...
| Checkpoint ID | `20260925T203531Z-R04-D21-ONE-ENTRY-COLD-MANIFEST-REFUSED` |
...
| Aktualny wykonawca / sesja | Codex / ten sam ONE-ENTRY-COLD OP; two-file install complete, one repeat settled with exact manifest refusal |
| Aktywny pakiet / podetap | R04/D-21 — `TWO_FILE_INSTALL_COMPLETE / RECORDER_COMPLETE / MANIFEST_REFUSED / NO_RETRY / NOT_READY_FOR_RESTART`; USABLE-WARM zachowuje odbiór |
...
| Ostatnia faktycznie zakończona czynność | Jeden UAC PID `43468` zainstalował exact launcher i recorder; jeden repeat utworzył kompletny settled attempt `20260925T203416504Z-7b650886`, który fail-closed zwrócił `MANIFEST_REFUSED`, exit `22`, przed startami |
...
| Niezakończone procesy i skutki operacyjne | UAC i launcher PID rozliczone; recorder complete/settled. Zainstalowane są dwa nowe przypięte pliki, lecz unchanged manifest odrzuca ich hashe. Dokładnie jeden klient PID `11924`, foreign `0`; repeat nie wykonał startów. Reboot/logon/cold `NOT_RUN` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260925T203531Z-R04-D21-ONE-ENTRY-COLD-MANIFEST-REFUSED.md` |
| Zakres aktualnej zgody | Jeden nowy UAC i jeden repeat są zużyte. Instalacja dwóch plików zakończona; runtime/manifest/task/shortcut/kontenery/dane niezmienione. Retry i restart nie są zatwierdzone |
| Blokada / wymagana decyzja | Installed approved manifest nadal wiąże poprzednie hashe launchera i recordera; nowy launcher fail-closed zwrócił exact `MANIFEST_REFUSED`. Potrzebna nowa decyzja dotycząca manifest binding albo operation-owned rollback; bez kolejnego Host startu |
| Jeden następny bezpieczny krok | Wyłącznie nowa exact decyzja właściciela dla rozliczenia manifest binding albo ograniczonego rollbacku; nie powtarzać completed preflight, instalacji ani Host entry |
