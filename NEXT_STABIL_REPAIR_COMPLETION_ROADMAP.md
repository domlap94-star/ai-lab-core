...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `STARTUP_ACTIVATION_PACKAGE_READY_FOR_REVIEW / NOT_INSTALLED`. Globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T211200Z-D21-P4A-STARTUP-ACTIVATION` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T21:12:00Z` |
| Aktualny wykonawca / sesja | Codex / P3 odbiór zapisany; P4-A nieaktywny pakiet przygotowany; zero instalacji i zero mutacji runtime; STOP przed P4-B/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-A startup activation preparation` — `READY_FOR_REVIEW / NOT_INSTALLED`; cały R04 nadal `IN_PROGRESS` |
...
| Potwierdzone testy bieżącego wykonania | Dziedziczony P3 readback: `11/11`, `9/9 false`, ta sama instancja. P4-A: parser JSON/CSV/XML/PowerShell oraz spójność hashów/referencji; metadata-only Docker/Task reads i GET `200/200/404`. Live launcher/start/logon/reboot/rollback, Flutter/Web/backend tests `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Chroniony root P3 pozostaje. P4-A LOCAL_ONLY root `...\p4-startup-activation-20260917T210051Z` ma indeks 16 plików SHA-256 `DC76525E49B77C7CA791B9724D38F9E26C2FC7FDA8513A2403B7D167481ACB72`; nie jest instalacją |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T211200Z-R04-D21-P4A-STARTUP-ACTIVATION.md` |
| Zakres aktualnej zgody | Zapis ograniczonego odbioru P3 i P4-A: read-only metadata, nieaktywne drafty/payload/rollback copies i dokumentacja. Bez instalacji, importu taska, manifest approval lub startu |
| Blokada / wymagana decyzja | Review exact P4-A package. RepoDigests/Qdrant backing, `OPEN_AFTER_BASE_READY`, global manifest approval, task/trigger install, P4-B operational matrix, backup schedule proof and D: relocations remain open |
| Jeden następny bezpieczny krok | Owner review P4-A and a separate exact P4-B decision naming files, triggers, approval state, operational window, tests and rollback |
...
| R04 | IN_PROGRESS | `R04-20260917T211200Z-D21-P4A-STARTUP-ACTIVATION` / P3 ACCEPTED LIMITED + P4-A READY FOR REVIEW | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A przygotował exact inactive package i trigger plan jako `READY_FOR_REVIEW / NOT_INSTALLED`. Globalny manifest pozostaje `NOT_APPROVED`; RepoDigests/Qdrant backing, `OPEN_AFTER_BASE_READY`, task install, live startup/reboot/rollback, backup schedule proof, relokacja i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
