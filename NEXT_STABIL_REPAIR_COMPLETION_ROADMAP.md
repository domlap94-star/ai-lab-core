...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują dotychczasowe odbiory; D-23 review pozostaje 2/2. USABLE-WARM Stage B historycznie zainstalowała cztery exact pliki. Najnowsza owner-authorized kontynuacja naprawiła lokalnie widoczność hash-pinned zależności i przeszła pełny offline path `11` scenariuszy / `57` asercji oraz realny read-only preflight `4/4`, `6/6`, PostgreSQL healthy, Public ready, Private/Supervisor absent i HTTP `200/200/200/404`. Jeden UAC/PID `37316` zakończył się exit `22`: pierwsza rejestracja Host on-demand została przekazana, lecz post-check pozostał `PENDING_UNKNOWN / TASK_POSTCHECK_NOT_CONFIRMED`. Readback wykazał Host `Ready`, enabled, bez triggera i bez Running/Queued, ale jego semantic/comparable hash nie odpowiada przypiętemu wariantowi on-demand. Warm runs `0/2`, logon `NOT_CONFIGURED`, CRM/Web `NOT_OPENED`, SAFE_INACTIVE `NOT_ATTEMPTED_PENDING_OPERATION`; zgoda/UAC są zużyte i nie ma retry. D-22 nadal ustala kolejność po właścicielskim odbiorze R04: ARKUSZE -> KOREKTA I WALIDACJA KLIENTÓW -> TYLKO NIEPRZYPISANE MAILE; wykonanie NOT_RUN. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. Host22 i NUP-01/02/03 zachowują odbiory, D-23 review `2/2` jest zakończony. Cztery pliki pozostają zainstalowane. Dependency-fix przeszedł offline i read-only preflight, lecz jedyny UAC zakończył pierwszą rejestrację Host jako `PENDING_UNKNOWN / TASK_POSTCHECK_NOT_CONFIRMED`. Readback: Host `Ready`, enabled, trigger_count `0`, Running/Queued `0`, ale hash nie odpowiada przypiętemu on-demand. Warm runs `0/2`, logon `NOT_CONFIGURED`, CRM/Web `NOT_OPENED`, SAFE_INACTIVE `NOT_ATTEMPTED_PENDING_OPERATION`; bez retry. Cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-D21-P4B-USABLE-WARM-CONTINUE-DEPS-20260924T164318Z` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-24T17:10:22.0479770Z`: po jednym UAC/PID `37316` operacja zakończyła się exit `22`; readback potwierdził zmianę Host do enabled/no-trigger/idle, ale nie do przypiętej tożsamości on-demand. Zgoda zużyta, bez retry |
| Aktualny wykonawca / sesja | Codex / OWNER-AUTHORIZED DEPENDENCY-FIX CONTINUATION STOPPED ON PENDING UNKNOWN HOST REGISTRATION |
| Aktywny pakiet / podetap | R04/D-21/P4-B — `DEPENDENCY_FIX_OFFLINE_PASS / READ_ONLY_PREFLIGHT_PASS / PARTIAL_PENDING_OPERATION_UNKNOWN / HOST_ENABLED_NO_TRIGGER_IDENTITY_MISMATCH / WARM_RUNS_0_OF_2 / LOGON_NOT_CONFIGURED / CRM_WEB_NOT_OPENED` |
...
| Ostatnia faktycznie zakończona czynność | Jeden dozwolony readback po częściowej mutacji: Host `Ready`, enabled, bez triggera, Running/Queued `0`; semantic/comparable `66ECF75806986BE7624FBC2430D14AEEC6D79BE4C348A29E072018E1F00BB938` / `171956207D309EC3B5241835F158BA5129A1685DF5FF57E4A650A0C94212BA92`, niezgodne z przypiętym on-demand. Nie wykonano dalszego zapisu ani startu |
| Potwierdzone testy bieżącego wykonania | LOCAL_ONLY dependency-fix `5F7F807A...BE4` / index `08DB292B...7446`: PS5.1 `11` scenariuszy / `57` asercji, granice produkcyjne `0`. Realny preflight `9345` B / `F95787D4DE2FF1D83E1232EC2F4D8010033B59460246030C877FFCD7865A5CC3` przeszedł pliki `4/4`, kontenery `6/6`, PostgreSQL healthy, Host/usługi/HTTP. Result `966` B / `C943F95CC27CDD6A0BC2A01969453EE77CEED4822A522F2135A87B2B847F478E`; journal `1168` B / `F908C2332408652E31F70B954E3663E30741D7C5EF8AF31D5687CC887DBF935B`. Docker/WSL pool i swap pozostają UNKNOWN, nie PASS |
| Niezacommitowana praca / zabezpieczenie | Historyczne dowody pozostają chronione. Pochodna, testy i wyniki są LOCAL_ONLY pod `C:\Users\domai\AppData\Local\Temp\P4B-UWC-01`; `apply-deps01` zachowuje preflight, journal i result. Zainstalowanych czterech plików nie kopiowano ani nie zmieniano |
| Niezakończone procesy i skutki operacyjne | Elevated PID `37316` zakończony. Journal rozlicza worker jako `WORKER_SETTLED`, ale efekt rejestracji pozostał `possible_effect=true / settled=false`; readback wykazał zmianę Host. Warm runs, Host start, Private start, logon, shortcut test i SAFE_INACTIVE wynoszą `0` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260924T171022Z-R04-D21-P4B-USABLE-WARM-DEPENDENCY-CONTINUATION-PARTIAL.md` |
| Zakres aktualnej zgody | Jednorazowa zgoda obejmowała dependency-fix, read-only preflight, jeden UAC i pełny pozostały przebieg. UAC został wykorzystany; pierwsza mutacja Host pozostała niepotwierdzona przez receptę. Zgoda jest zużyta i nie obejmuje retry, dalszego zapisu/startu ani zewnętrznego rollbacku |
| Blokada / wymagana decyzja | Bieżący Host jest enabled/no-trigger/idle, lecz nie ma przypiętej tożsamości on-demand; journal zachowuje nierozliczony handoff. Zgodnie z guardami nie wolno wykonać drugiego zapisu, warm runu ani SAFE_INACTIVE. Potrzebna jest osobna decyzja właściciela oparta na tym dokładnym stanie, nie automatyczne ponowienie |
| Jeden następny bezpieczny krok | Właścicielska decyzja o rozliczeniu aktualnej nieprzypiętej definicji Host po niezależnym review istniejących dowodów; bez nowego UAC, task write/start, retry lub rollbacku w tej sesji |
...
| R04 | IN_PROGRESS | `R04-D21-P4B-USABLE-WARM-CONTINUE-DEPS-20260924T164318Z` / UAC CONSUMED, HOST REGISTER PENDING UNKNOWN | P3/P4-A, Host22 i NUP-01/02/03 odbiory zachowane; D-23 review `2/2`. Dependency-fix przeszedł offline i realny read-only preflight. Jeden UAC/PID `37316` zakończył się exit `22` na pierwszym Host register; journal zachowuje `TASK_POSTCHECK_NOT_CONFIRMED`. Readback potwierdził Host Ready/enabled/no-trigger/idle, ale z tożsamością inną niż przypięty on-demand. Warm `0/2`, logon nie skonfigurowany, CRM/Web nie otwarte, SAFE_INACTIVE niewykonany. Retry/start/write/rollback są zatrzymane; P5/D-22 nieuruchomione |
