...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`, P2 `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`, DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0` jest `OFFLINE_TEST_ONLY / NOT_DEPLOYED`, a guard `0ee0ea50943578e6e552aae23ce1688595ddc262` jest `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. D21-P3 ma teraz `FRESH_ROLLBACK_POINT_READY_FOR_REVIEW / CAPTURE_AND_ISOLATED_DATA_RESTORE_PASS / NO_CUTOVER`: nowy `RecoveryPointV2/full/CaptureOnly` w `E:\ai-lab-backup\20260917T082022Z` ma manifest `2759D684...95597`, 9/9 artefaktów z pełną kontrolą SHA-256 oraz osobny izolowany drill PostgreSQL, storage, obu kolekcji Qdrant, release/config/inventory i opaque n8n. Spójność pozostaje `COMPONENT_WINDOWS_RECORDED_NON_TRANSACTIONAL / VERIFIED_LINKS_IN_TESTED_SCOPE`. Produkcyjny manifest pozostaje `NOT_APPROVED_FOR_START`; cutover, relokacja, task install, live startup, reboot i operacyjne P3–P5 są `NOT_RUN / NOT_AUTHORIZED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. D21-P3 jest `FRESH_ROLLBACK_POINT_READY_FOR_REVIEW / CAPTURE_AND_ISOLATED_DATA_RESTORE_PASS / NO_CUTOVER`; draft manifestu nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T090901Z-D21-P3-FRESH-ROLLBACK` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T09:09:01Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 fresh RecoveryPointV2 capture + isolated data drill; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P3 preparation` — `FRESH_ROLLBACK_POINT_READY_FOR_REVIEW / CAPTURE_AND_ISOLATED_DATA_RESTORE_PASS / NO_CUTOVER`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `e3f2125883420a12c8721c5ba597203dcf403add`; accepted guard source `0ee0ea50943578e6e552aae23ce1688595ddc262`; accepted DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0`. Oryginalny preservation `203/203` i zastane runtime/data/junction pozostały nietknięte |
...
| Ostatnia faktycznie zakończona czynność | Jeden capture `E:\ai-lab-backup\20260917T082022Z`, pełny hash-check 9 artefaktów, ValidateOnly i jeden izolowany drill dokładnie tego punktu; własne zasoby drillu usunięto po pełnej kontroli ID |
| Potwierdzone testy bieżącego wykonania | Capture exit `0`; manifest `2759D684...95597`; ValidateOnly exit `0`; artefakty/hash `PASS`; pg_restore exit `0`; 52 tabele, 113 FK/0 invalid, 114 PK; storage 6362/6362 refs, 0 mismatch; Qdrant 57 i 157 punktów, 1024/Cosine, bindingi `PASS`; release/config/inventory `PASS`; n8n opaque JSON parsed. Aplikacja/runtime/Flutter/Web `NOT_RUN` |
...
| Niezakończone procesy i skutki operacyjne | Zasoby drillu: 5 kontenerów, 3 volumes i 3 internal networks utworzone, zweryfikowane i usunięte; remaining `0/0/0`. Backup i rozpakowany verify pozostają chronione na E:. Niezamierzenie pobrany `alpine:3.20` został usunięty, remaining `0`. Supervisor `INTENTIONALLY_STOPPED / NOT_STARTED_BY_THIS_SESSION`; produkcyjne usługi nie były restartowane ani rekonfigurowane |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T090901Z-R04-D21-P3-FRESH-ROLLBACK.md`; poprzedni handoff: `docs/recovery/checkpoints/20260916T200609Z-R04-D21-P3-CURRENT-DB-ROLLBACK.md` |
| Zakres aktualnej zgody | D21-P3: jeden fresh RecoveryPointV2/full/CaptureOnly, pełna weryfikacja i jeden izolowany drill dokładnie tego punktu; bez produkcyjnego restore/cutoveru, instalacji, startu/restartu produktu, zmian runtime/config/data/mount/task/flag/queue, escrow lub P4/P5 |
| Blokada / wymagana decyzja | Fresh point ma status `READY_FOR_REVIEW`, nie `ACCEPTED`. Produkcyjny manifest nie jest zatwierdzony; pending work 18/16/1 musi zostać zachowany; przed przyszłym cutoverem trzeba ocenić zapisy po oknach punktu i uzyskać osobną zgodę operacyjną |
| Jeden następny bezpieczny krok | Review dokładnego punktu `20260917T082022Z` i osobna decyzja właściciela o minimalnym oknie P3 backend source-switch; bez automatycznego restore danych |
...
| R04 | IN_PROGRESS | `R04-20260917T090901Z-D21-P3-FRESH-ROLLBACK` / FRESH ROLLBACK READY FOR REVIEW, NO CUTOVER | A3 oraz D-21 P1/P2/DATA_ONLY/guard zachowują odbiory. Fresh `RecoveryPointV2/full/CaptureOnly` `20260917T082022Z` ma manifest `2759D684...95597`, pełny artifact hash pass i izolowany data restore pass dla PostgreSQL/storage/Qdrant/release/config/inventory/opaque n8n. Spójność `RECORDED_NON_TRANSACTIONAL / VERIFIED_LINKS_IN_TESTED_SCOPE`; pending work 18/16/1 pozostaje do zachowania. Manifest nadal `NOT_APPROVED_FOR_START`; cutover, relokacja, task install, live startup/reboot/produkcyjny restore i runtime acceptance są `NOT_RUN` |
