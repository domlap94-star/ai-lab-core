...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują odbiory; D-23 review pozostaje 2/2. `USABLE_WARM_ACCEPTED / LIMITED_RUNTIME_SCOPE` pozostaje w mocy. OP `R04-D21-ONE-ENTRY-COLD-COMPLETE-20260925T183020Z` atomowo zainstalował i zwalidował approved manifest V2, zaliczył repeat oraz dokładnie jeden restart. Realny Host logon utworzył complete attempt `20260926T102936229Z-abc29611`: exit `0`, settled, base ready, Supervisor `INTENTIONALLY_STOPPED`; dokładnie jeden nowy klient PID `3516`, foreign `0`, publiczne `/control*=404`, Supervisor listener `0`. Stan to `ONE_RESTART_COMPLETE / LOGON_COLD_PASS / WAITING_OWNER_MANUAL_CRM_CONFIRMATION`; pozostaje wyłącznie ręczne potwierdzenie listy i szczegółów klienta. Docker/WSL pool i swap pozostają UNKNOWN. D-22 pozostaje `NOT_RUN`. Bieżący stan znajduje się wyłącznie w §0.**
...
| Checkpoint ID | `20260926T103418Z-R04-D21-ONE-ENTRY-COLD-LOGON-PASS` |
...
| Aktualny wykonawca / sesja | Codex / ten sam ONE-ENTRY-COLD OP; one restart and real logon/cold PASS, waiting manual CRM confirmation |
| Aktywny pakiet / podetap | R04/D-21 — `ONE_RESTART_COMPLETE / LOGON_COLD_PASS / WAITING_OWNER_MANUAL_CRM_CONFIRMATION`; USABLE-WARM zachowuje odbiór |
...
| Ostatnia faktycznie zakończona czynność | Jeden restart; Host logon result `0`; complete settled attempt `20260926T102936229Z-abc29611`, base ready, exactly one client PID `3516`, public control 404, Supervisor absent |
...
| Niezakończone procesy i skutki operacyjne | Restart count `1/1`; Host/launcher/recorder settled; dokładnie jeden klient, foreign `0`; pending mutation false. Brak drugiego restartu, manual startu i retry |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260926T103418Z-R04-D21-ONE-ENTRY-COLD-LOGON-PASS.md` |
| Zakres aktualnej zgody | Jeden restart i post-boot readback zakończone. Pozostaje wyłącznie ręczne potwierdzenie listy klientów i szczegółów jednego klienta; brak dalszej mutacji |
| Blokada / wymagana decyzja | Wyłącznie ręczne potwierdzenie właściciela, że lista klientów i szczegóły jednego klienta otwierają się poprawnie |
| Jeden następny bezpieczny krok | Właściciel ręcznie otwiera listę klientów i szczegóły jednego klienta, po czym potwierdza wynik w tej rozmowie |
