...
| R04 | IN_PROGRESS | `R04-20260919T142238Z-D21-P4B-COLLECTOR-READBACK-PARTIAL` / P4-A OWNER ACCEPTED + P4/B PARTIAL SAFE INACTIVE | P3 backend switch pozostaje `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A pozostaje owner-accepted jako `NOT_INSTALLED`. Exact recipe `F6D3A8CC...C883F0E` pozostaje `EXACT_ROLLBACK_SAFE_RECIPE_ACCEPTED / SOURCE_AND_OFFLINE_SCOPE / NOT_INSTALLED`. Poprawiony collector przeszedł offline `13/61`; bieżący readback ma Task State `6/6`, host/resources/HTTP oraz 6/6 exact kontenerów. TaskInfo `0/6` przez błąd mapowania i bez retry, więc LastRun/LastTaskResult pozostają `NOT_VERIFIED`. Status `PREFLIGHT_EVIDENCE_PARTIAL / TASK_INFO_MAPPING_ERROR_NO_REREAD / NO_UAC / NOT_INSTALLED`; globalny manifest `NOT_APPROVED_FOR_START`. Qdrant/VHD/profile, UI, backup proof, live logon/reboot/rollback i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
