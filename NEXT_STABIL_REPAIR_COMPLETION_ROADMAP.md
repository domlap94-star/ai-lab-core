...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. Właściciel przyjął P3 OP_ID `R04-D21-P3-CORE-SWITCH-20260917T141404Z` jako `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4/A source `8756314f51a76091a483cfc9b677a05c7f67f315` i evidence `8a156e0c738699b4bfee01fb98f0d10f01c803ba` są przyjęte jako `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B nadal pozostaje `PARTIAL_SAFE_INACTIVE`: wrapper Startup w rollbacku, Host disabled/no-trigger, pięć tasków na preimage, payload/manifest nieobecne i warm runs `0/2`. Błąd transportu poprzedniej recepty został odtworzony bez Dockera i naprawiony w nowej LOCAL_ONLY recepcie SHA-256 `5F310C64...1FD67`; Windows PowerShell 5.1 zaliczył 17/17 przypadków i 120 asercji, a dokładnie jeden dozwolony `docker container inspect` potwierdził poprawny transport oraz niezmienioną tożsamość backendu. Status narzędzia to `RECIPE_NATIVE_ARGUMENT_TRANSPORT_FIXED / TESTED_ON_POWERSHELL_51 / READY_FOR_REVIEW`, nie instalacja ani odbiór P4/B. Wszystkie wcześniejsze zgody operacyjne są skonsumowane; nowego UAC nie wykonano. Repository draft i globalny manifest pozostają `NOT_APPROVED_FOR_START`. Supervisor pozostaje `INTENTIONALLY_STOPPED`; relokacja Qdrant/VHD/profile, backup schedule proof, live startup/reboot/rollback oraz P4-B/P5 są otwarte. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B zachowuje `PARTIAL_SAFE_INACTIVE`; poprawiona recepta ma `RECIPE_NATIVE_ARGUMENT_TRANSPORT_FIXED / TESTED_ON_POWERSHELL_51 / READY_FOR_REVIEW`; globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260918T153837Z-D21-P4B-NATIVE-TRANSPORT-READY` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-18T15:38:37.8656363Z` |
| Aktualny wykonawca / sesja | Codex / przygotowanie recepty i testy offline zakończone; bez UAC i instalacji. Jedyny read-only Docker inspect zakończył się PASS; STOP przed nową zgodą operacyjną/UAC/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — recepta `R04-D21-P4B-RECIPE-TRANSPORT-FIX-20260918T152017Z`, `READY_FOR_REVIEW / NOT_INSTALLED`; cały R04 nadal `IN_PROGRESS` |
...
| Potwierdzone testy bieżącego wykonania | Windows PowerShell `5.1.26100.8894`: 17/17 przypadków, 120 asercji, exit 0; parser recepty, argv, `PowerShell -File`, start failure/nonzero/timeout/incomplete, sekwencja pre/post-mutation na atrapach i 29 hashy wejściowych PASS. Jeden exact Docker readback PASS. Testy produktu/Flutter/Web/backend/backup i warm runs `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY root `p4b-native-transport-20260918T152017Z`; recepta SHA-256 `5F310C64...1FD67`, indeks `43F285C2...A2A9`, offline summary `2595FE38...03D1`, Docker verdict `FBFFA2C8...F1A43`. Payload/manifest pozostają NOT_INSTALLED |
| Niezakończone procesy i skutki operacyjne | Wszystkie własne procesy testowe zakończone. Jedno read-only Docker CLI zakończone; `mutation_started=false`, UAC 0, rollback 0. Zachowany wcześniejszy stan: wrapper w rollbacku, Host disabled/no-trigger, pięć tasków na preimage, helper legacy; Supervisor `INTENTIONALLY_STOPPED` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260918T153837Z-R04-D21-P4B-NATIVE-TRANSPORT-READY.md` |
| Zakres aktualnej zgody | Poprawka LOCAL_ONLY, testy, jeden read-only Docker inspect i publikacja dokumentacji. Brak zgody na UAC, instalację, task/trigger/startup changes, warm run lub rollback |
| Blokada / wymagana decyzja | Recepta i indeks wymagają review właściciela oraz nowej, jednorazowej decyzji operacyjnej. Przed przyszłą instalacją obowiązuje świeży bounded drift check. Docker/WSL pool availability i swap-used pozostają `UNKNOWN`; Qdrant/VHD/profile, backup proof i UI są otwarte |
| Jeden następny bezpieczny krok | Review recepty SHA-256 `5F310C64...1FD67` i indeksu `43F285C2...A2A9`; ewentualna nowa decyzja ma osobno przypiąć jedno przyszłe UAC/okno. Bez tej decyzji STOP |
...
| R04 | IN_PROGRESS | `R04-20260918T153837Z-D21-P4B-NATIVE-TRANSPORT-READY` / P4-A OWNER ACCEPTED + P4/B PARTIAL SAFE INACTIVE | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A source/offline/identity package jest owner-accepted jako `NOT_INSTALLED`. Poprzedni resume zakończył się przed mutacją; poprawiona recepta `5F310C64...1FD67` zaliczyła PowerShell 5.1 offline 17/17 i jeden exact read-only Docker inspect. Jest `READY_FOR_REVIEW`, nie zaakceptowana ani uruchomiona instalacyjnie. Wrapper pozostaje w rollbacku, Host disabled/no-trigger, istniejące taski na preimage, payload/manifest nie są zainstalowane, warm runs `0/2`, draft/global manifest `NOT_APPROVED`. Nowa operacja wymaga osobnej decyzji. Qdrant/VHD/profile, UI, backup proof, live logon/reboot/rollback i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
