...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B pozostaje `PARTIAL_AFTER_FAILURE / HOST_TASK_FAILED_22 / SAFE_INACTIVE_PARTIAL_UNKNOWN`; Host22 source i NUP-03 completion package są zaakceptowane wyłącznie SOURCE/OFFLINE / NOT_DEPLOYED. Review NUP `2/2` jest zakończony, NUP-01/02 zachowują PASS. Stage A jest `BLOCKED`; Stage B nieautoryzowany. Cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260921T084715Z-D21-P4B-NUP03-STAGE-A-BLOCKED` |
...
zależnego Stage B: exact VerifyOnly odmawia, a zachowane odczyty nie potwierdzają
pełnego runtime/preimage kontraktu. To nie otwiera ponownego review NUP-01/02/03
ani ogólnego audytu produktu i nie upoważnia do automatycznej naprawy. D-22
...
| R04 | IN_PROGRESS | `R04-20260921T084715Z-D21-P4B-NUP03-STAGE-A-BLOCKED` / P4-B PARTIAL + NUP-03 SOURCE/OFFLINE ACCEPTED + STAGE A BLOCKED | P3/P4-A i Host22 odbiory zachowane. Exact run01 pozostaje częściowy, warm `0/2`, Private start `0`, Supervisor `INTENTIONALLY_STOPPED`. Exact NUP-03 package jest `SOURCE_AND_OFFLINE_PACKAGE_ACCEPTED / NOT_DEPLOYED`. Jedyny VerifyOnly: exit `22`, `TASK_DEPENDENCY_DRIFT`, mutation `false`; niezależna projekcja wykazała dalsze mismatch i nie wykonała HTTP po błędzie formattera. `NO_STAGE_B_AUTHORIZATION_REQUESTED`; Qdrant/VHD/profile, UI, backup proof, live logon/reboot i P4-B/P5 pozostają otwarte |
