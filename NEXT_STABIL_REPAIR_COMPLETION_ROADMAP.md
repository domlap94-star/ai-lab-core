...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`, P2 `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`, DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0` jest `OFFLINE_TEST_ONLY / NOT_DEPLOYED`, a guard `0ee0ea50943578e6e552aae23ce1688595ddc262` jest `BASE_START_GUARD_SOURCE_AND_SYNTHETIC_TESTS_ACCEPTED / NOT_DEPLOYED`. Właściciel przyjął dowód danych punktu `E:\ai-lab-backup\20260917T082022Z`, manifest `2759D684...95597`, jako `ROLLBACK_POINT_DATA_EVIDENCE_ACCEPTED_WITH_RECORDED_LIMITATIONS`; odstępstwa i nieatomowe component windows pozostają zapisane. D21-P3 dla OP_ID `R04-D21-P3-CORE-SWITCH-20260917T141404Z` ma `CORE_BACKEND_SOURCE_SWITCH_READY_FOR_REVIEW / LIMITED_RUNTIME_VERIFIED`: backend `686ac376...c854` wykonuje source `0ee0ea5...` z dokładnego payloadu i override, dane/DB/pending `18/16/1` pozostały zachowane, a rollback nie był potrzebny. Późniejszy ograniczony readback `2026-09-17T19:53:26Z–19:53:27Z` potwierdził `CONTAINER_ENV_OBSERVED 11/11`, `BACKEND_PROCESS_ENV_OBSERVED 11/11` i `9/9` przełączników `false` na tej samej instancji; jest to `CURRENT_READ_ONLY_EVIDENCE`, nie odbiór właścicielski ani bezpośredni odczyt obiektu Settings. Produkcyjny globalny manifest pozostaje `NOT_APPROVED_FOR_START`; relokacja danych, task install, pełny live startup/reboot/restore i P4–P5 są `NOT_RUN / NOT_AUTHORIZED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. Punkt rollbacku ma ograniczony odbiór właściciela; D21-P3 jest `CORE_BACKEND_SOURCE_SWITCH_READY_FOR_REVIEW / LIMITED_RUNTIME_VERIFIED`, a późniejszy readback środowiska ma `BACKEND_STARTUP_ENVIRONMENT_READBACK_PASS / CURRENT_READ_ONLY_EVIDENCE`. Globalny manifest nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T195326Z-D21-P3-RUNTIME-ENV-READBACK` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T19:53:27Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 post-switch readback zakończony; zero mutacji runtime; STOP przed P4/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P3 core backend source switch` — `CORE_BACKEND_SOURCE_SWITCH_READY_FOR_REVIEW / LIMITED_RUNTIME_VERIFIED`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start bieżącego readbacku local/tracking/remote `d792eb3886cacfeb80aa08d8cf03b872319d4b8b`; accepted guard/source `0ee0ea50943578e6e552aae23ce1688595ddc262`. Manifest R00 ma `203/203`: `116` wpisów zgodnych na oryginalnych ścieżkach i `87` backendowych zgodnych w rollback; missing/mismatch `0/0`. Oryginalny HEAD pozostaje `72950657ac79b50d0afe72753632ba4cde810b95` |
...
| Źródła runtime / release / DB | Backend `686ac37663ad369f253eb91da4364aa2bd6c16c77b1205cc41d61c68d4d9c854`, image `sha256:6342b36f...d63702`, `/app=C:/ai-lab-core/backend:ro`, source `0ee0ea50943578e6e552aae23ce1688595ddc262`; `/data=C:/ai-lab-core/data:rw`. PostgreSQL `240343eb...1226` i schema `followup_assistant_chat_history_20260829` zachowane. Public Gateway/Web działa, `/control*` jest 404. Web P2 pozostaje TEST_ONLY, Supervisor nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | W ograniczonym oknie read-only potwierdzono dokładny backend i tę samą instancję przed/po; Config.Env i `/proc` istniejącego procesu dały `11/11 MATCH`, w tym `9/9` przełączników `false`; GET `/health`, `/version`, `/gateway-health`, publiczne `/control` dały odpowiednio `200/200/200/404`. Nie zmieniono runtime |
| Potwierdzone testy bieżącego wykonania | Historyczny handoff fazy B nie zawierał niezależnego readbacku flag i pozostaje prawidłowo opisany jako ówczesne `NOT_VERIFIED`. Nowy dowód `2026-09-17T19:53:26Z–19:53:27Z` potwierdza dwie warstwy wejść środowiskowych `11/11`, `9/9` bool `false`, tę samą instancję oraz health `200/200/200/404`. Nie jest to odczyt pamięci obiektu Settings ani dowód wcześniejszych cykli dispatcherów. Flutter/Web build, migracje, Supervisor, modele, eksport, P4/P5 `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Lokalny chroniony root `...\p3-core-switch-20260917T141404Z` zachowuje payload, manifesty, override, plan, dokładny stary backend i dowody fazy B. Indeks fazy B 32 plików / 180 781 B ma SHA-256 `6D6354AC...2085`; późniejsza bezpieczna projekcja readbacku jest indeksowana osobno jako `D21-P3-DIAG-052`. 203/203 chronionych wpisów są rozliczone |
| Niezakończone procesy i skutki operacyjne | Nowy backend działa; pozostałe pięć kontenerów i Public Gateway zachowały tożsamość. Supervisor `INTENTIONALLY_STOPPED`. Backup/junction/data/taski/harmonogramy niezmienione; historyczne odstępstwa drillu zachowane |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T195326Z-R04-D21-P3-RUNTIME-ENV-READBACK.md`; phase B: `docs/recovery/checkpoints/20260917T165525Z-R04-D21-P3-CORE-SWITCH-HANDOFF.md` |
| Zakres aktualnej zgody | Zużyto jednorazową zgodę na celowany odczyt 11 kluczy, jeden krótki proces kontrolny oraz cztery bezpieczne GET. Nie zmieniono flag, kodu, konfiguracji, mountów ani usług. Poprzednia zgoda na switch pozostaje skonsumowana |
| Blokada / wymagana decyzja | Właścicielski review wyniku backend switch wraz z nowym readbackiem. Globalny manifest nadal `NOT_APPROVED_FOR_START`; pełna delta treści pozostaje `DELTA_NOT_FULLY_OBSERVED`; P4/P5, full startup/reboot/rollback acceptance i cleanup wymagają osobnej zgody |
| Jeden następny bezpieczny krok | Review ograniczonego backend switch i dowodów; bez kolejnej mutacji runtime |
| Warunek STOP | Bez ponownego recreate/retry, rollbacku bez nowej przesłanki i zgody, startu Supervisora, migracji, zmiany danych/junction/tasków/harmonogramów, restore danych, P4–P5/R05-A5/R06–R24 |
...
| R04 | IN_PROGRESS | `R04-20260917T195326Z-D21-P3-RUNTIME-ENV-READBACK` / BACKEND SWITCH + ENV READBACK READY FOR REVIEW | A3 oraz D-21 P1/P2/DATA_ONLY/guard zachowują odbiory. Backend-only switch wykonał dokładny source/payload/override; runtime bytes `592/592`, DB/schema/pending `18/16/1`, junction i pięć pozostałych kontenerów zachowane. Rollback nie był potrzebny. Późniejszy readback tej samej instancji potwierdził dwie warstwy `11/11 MATCH` i `9/9` przełączników `false`; wynik to `BACKEND_STARTUP_ENVIRONMENT_READBACK_PASS / CURRENT_READ_ONLY_EVIDENCE`, bez bezpośredniego odczytu Settings. Całość pozostaje `CORE_BACKEND_SOURCE_SWITCH_READY_FOR_REVIEW / LIMITED_RUNTIME_VERIFIED`, nie ACCEPTED. Pełna delta jest `DELTA_NOT_FULLY_OBSERVED`; globalny manifest, relokacja, task install, full startup/reboot/restore i P4–P5 pozostają `NOT_RUN / NOT_AUTHORIZED` |
