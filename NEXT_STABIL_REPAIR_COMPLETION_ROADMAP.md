...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`. Właściciel przyjął P2 na evidence `f872cf9e548a7ec196289e0c1987b654b1017505` jako `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`. Dokładny junction `C:\ai-lab-core\data -> D:\ai-lab-data` jest `OWNER_APPROVED_ACTIVE_DATA_JUNCTION`; source `cb6e22506a0fecc440400566293524536847b9b0` ma status `ACTIVE_DATA_DESTINATION_GUARD_READY_FOR_REVIEW / OFFLINE_TEST_ONLY`. Produkcyjny manifest pozostaje `NOT_APPROVED_FOR_START`. Cutover, task install, live startup, reboot, relokacja niepotwierdzonych zapisów, cleanup i P3–P5 są `NOT_RUN / NOT_AUTHORIZED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`; P2 jest `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`. Junction D: jest owner-approved; destination guard jest `READY_FOR_REVIEW / OFFLINE_TEST_ONLY`, a draft manifestu `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260916T083044Z-D21-DATA-DESTINATION-GUARD` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-16T08:30:44Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 ACTIVE_DATA_ONLY destination guard review; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P2 continuation` — `ACTIVE_DATA_DESTINATION_GUARD_READY_FOR_REVIEW / OFFLINE_TEST_ONLY`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `be8ac25728e0b6c59a55b5b4d708edc0e124e726`; source commit `cb6e22506a0fecc440400566293524536847b9b0`. Oryginalny preservation `203/203` i zastane runtime/data/junction pozostały nietknięte |
...
| Źródła runtime / release / DB | Historyczny odczyt `2026-09-15T21:43:27Z`: backend ID/image i `/app` pozostają legacy main; live `/data=C:/ai-lab-core/data`, a metadata ścieżki wskazała junction do `D:/ai-lab-data`. Bieżącego runtime nie obserwowano ponownie. Qdrant volume/VHD/tablespaces/logs/profile pozostają `UNKNOWN`; nie ma `ALL_LIVE_WRITES_ON_D_PASS`. Web P2 pozostaje TEST_ONLY, frontend tree niezmieniony. Supervisor pozostaje nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | `RV-D21-DATA-01` odtworzono na preimage: `/app/app` oraz obca para `postgres/N8N_DATA` przeszły do planu. Source `cb6e225...` wiąże teraz dokładny, case-sensitive kontrakt service/role/destination; zsynchronizowano kandydata bez zmiany junctionu, danych lub backupów |
| Potwierdzone testy bieżącego wykonania | Windows PowerShell `5.1.26100.8894`: destination guard 44/44, host plan 53/53, real-adapter mapping 48/48, parser 0 błędów i JSON PASS. Fail-before obu kontrprzykładów miał exit `1`; pass-after użył tylko własnego syntetycznego junctionu i pełnych atrap. Aplikacja/UI/browser/backend/DB/modele/backup: `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Source jest w Git na `cb6e225...`; surowe logi pozostają LOCAL_ONLY pod istniejącym stagingiem P2. Oryginalny worktree, audit, wcześniejszy snapshot `318A41...CC6C3`, Web `A99FB9...C41`, dane D:, backupy i profile pozostały bez zmian |
| Niezakończone procesy i skutki operacyjne | Supervisor pozostaje `INTENTIONALLY_STOPPED / NOT_STARTED_BY_THIS_SESSION`. Nie wykonano start/stop/restart, Docker/host adapterów, zmian kontenerów, junctionu, tasków, shortcutów, mountów, flag, kolejek, harmonogramów, danych, backupu lub restore. Rzeczywiste skutki: źródła/testy Git i własny syntetyczny junction usunięty po teście |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260916T083044Z-R04-D21-DATA-DESTINATION-GUARD.md`; poprzedni handoff: `docs/recovery/checkpoints/20260916T062447Z-R04-D21-ACTIVE-DATA-JUNCTION.md` |
| Zakres aktualnej zgody | D21/P2 continuation: domknięcie service/role/destination dla `ACTIVE_DATA_ONLY`, testy offline i synchronizacja nieaktywnego kandydata; bez instalacji, startu, zmiany linku/danych/backupów lub P3–P5 |
| Blokada / wymagana decyzja | Produkcyjny manifest nie jest zatwierdzony. P3 musi rozliczyć exact ID/image/mounty, Qdrant volume i Docker/WSL VHD, external tablespaces/WAL, logi/cache/profile, schema, efektywne flagi, side effects init/reconciler, rollback i okno operacyjne. Wybór C: kontra D: nie jest już otwarty |
| Jeden następny bezpieczny krok | Review destination guard i osobna zgoda na przygotowanie dokładnego P3: `KEEP` potwierdzonych bindów na D:, read-only rozliczenie UNKNOWN, dokładna konfiguracja/mounty oraz oddzielne przełączenie wersji backendu |
...
| R04 | IN_PROGRESS | `R04-20260916T083044Z-D21-DATA-DESTINATION-GUARD` / DESTINATION GUARD SOURCE READY FOR REVIEW | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 P1 pozostaje accepted source/offline. P2 jest `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`. Source `cb6e225...` zachowuje owner-approved junction i domyka dokładny kontrakt service/role/destination jako `READY_FOR_REVIEW / OFFLINE_TEST_ONLY`; manifest nadal `NOT_APPROVED_FOR_START`. P3–P5, cutover, task install, live startup/reboot/rollback, niepotwierdzone relokacje, cleanup i runtime acceptance są `NOT_RUN` |
...
3. Historyczna reguła przed D-21 dopuszczała odrębne odtwarzalne deployment roots. D-21 zastępuje ją dla stanu docelowego: `SINGLE_INSTALL_ROOT=C:\ai-lab-core`, jawne wersjonowane podkatalogi komponentów i jeden zwykły, idempotentny punkt startu. Dokładny junction `C:\ai-lab-core\data -> D:\ai-lab-data` jest świadomym wyjątkiem `ACTIVE_DATA_ONLY`; kod/skrypty/executable/CWD nie mogą z niego korzystać. P1 source/offline tests są zaakceptowane jako NOT_DEPLOYED. P2 preservation/candidate jest zaakceptowane jako NOT_DEPLOYED, jego wcześniejszy snapshot/Web zachowują oryginalne pochodzenie, a destination guard source `cb6e225...` czeka na review. Guard wiąże dokładny, case-sensitive kontrakt service/role/destination i nie uznaje zgodności dwóch dowolnych pól manifestu za zgodę. Produkcyjny manifest pozostaje `NOT_APPROVED_FOR_START`; P3–P5, instalacja, cutover, relokacja niepotwierdzonych zapisów, test live startup/rollback i cleanup wymagają osobnych zgód. Produkcja nie może wykonywać recovery/staging/test WIP.
