...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują odbiory; D-23 review pozostaje 2/2. `USABLE_WARM_ACCEPTED / LIMITED_RUNTIME_SCOPE` pozostaje w mocy. OP `R04-D21-ONE-ENTRY-COLD-COMPLETE-20260925T183020Z` ma zainstalowane przypięte launcher i recorder, ale installed approved manifest nadal wiąże ich poprzednie hashe. Dokładna pochodna V2 zmieniająca tylko zatwierdzoną allowlistę została zachowana i przeszła niezmienione funkcje walidacyjne z errors `0`; SHA-256 `53CA6E98...B6972EC9`. Kolejny dokładnie zatwierdzony UAC instalacji manifestu również został anulowany przed utworzeniem procesu, więc manifest write, Host start, repeat, rollback i restart wyniosły `0`. Stan to `MANIFEST_DERIVATIVE_VALIDATED / NEW_UAC_CANCELLED_BEFORE_PROCESS / NO_MANIFEST_WRITE / NOT_READY_FOR_RESTART`; nowa decyzja jest wymagana. Supervisor pozostaje `INTENTIONALLY_STOPPED`; Docker/WSL pool i swap pozostają UNKNOWN. D-22 pozostaje `NOT_RUN`. Bieżący stan znajduje się wyłącznie w §0.**
...
| Checkpoint ID | `20260925T210905Z-R04-D21-ONE-ENTRY-COLD-MANIFEST-UAC-RETRY-CANCELLED` |
...
| Aktualny wykonawca / sesja | Codex / ten sam ONE-ENTRY-COLD OP; manifest V2 validated, kolejny exact UAC anulowany przed procesem |
| Aktywny pakiet / podetap | R04/D-21 — `MANIFEST_DERIVATIVE_VALIDATED / NEW_UAC_CANCELLED_BEFORE_PROCESS / NO_MANIFEST_WRITE / NO_HOST_START / NOT_READY_FOR_RESTART`; USABLE-WARM zachowuje odbiór |
...
| Ostatnia faktycznie zakończona czynność | Exact derivative installed approved manifestu `53CA6E98...B6972EC9` przeszła PS5.1 Read/Test z errors `0`; pojedynczy UAC instalacji został anulowany przed procesem, więc manifest write i Host start `0` |
...
| Niezakończone procesy i skutki operacyjne | Brak podniesionego PID i brak manifest-install result; manifest write/Host start/repeat/rollback/reboot `0`. Launcher+recorder pozostają nowe, installed manifest ma stare bindings. Candidate/preimage są zachowane w OutputRoot; temporary operator usunięty |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260925T210905Z-R04-D21-ONE-ENTRY-COLD-MANIFEST-UAC-RETRY-CANCELLED.md` |
| Zakres aktualnej zgody | Kolejny exact UAC instalacji zachowanego manifest V2 został anulowany przed procesem i jest zużyty. Pod UAC nie wykonano nawet preimage read; restart niezatwierdzony |
| Blokada / wymagana decyzja | Validated candidate nadal nie został zainstalowany, ponieważ nowy UAC anulowano przed procesem. Installed manifest ma stare bindings; kolejny UAC wymaga kolejnej exact decyzji, bez powtarzania candidate/preflight |
| Jeden następny bezpieczny krok | Wyłącznie nowa exact decyzja na jeden UAC instalacji już zwalidowanego candidate `53CA6E98...B6972EC9`; bez ponownego preflightu, walidacji, instalacji dwóch plików lub Host entry |
