...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują odbiory; D-23 review pozostaje 2/2. `USABLE_WARM_ACCEPTED / LIMITED_RUNTIME_SCOPE` pozostaje w mocy wraz z historycznymi `PARTIAL/PENDING_UNKNOWN` i późniejszymi dowodami rozliczenia. Jednorazowe okno `R04-D21-ONE-ENTRY-COLD-WINDOW-20260925T110738Z` przeszło bounded preflight, lecz instalacja zatrzymała się na kolizji parametru `ManifestPath`; jeden UAC jest zużyty, a jeden dozwolony rollback zakończył się `SAFE_INACTIVE_COMPLETE` i przywrócił 5/5 preimage. Host/manual entry/reboot/logon/cold/CRM-Web nie zostały wykonane. Status `PRE_REBOOT_INSTALL_FAILED_VALIDATION_MANIFEST_PARAMETER_COLLISION / SAFE_INACTIVE_COMPLETE / UAC_CONSUMED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`; Docker/WSL pool i swap pozostają UNKNOWN. D-22 pozostaje `NOT_RUN`. Bieżący stan znajduje się wyłącznie w §0.**
...
**Aktualizacja USABLE-WARM DEPENDENCY-FIX 2026-09-24:** LOCAL_ONLY pochodna
`5F7F807A...BE4` z indeksem `08DB292B...7446` zachowuje parametry wejściowe i
udostępnia rzeczywistym adapterom wyłącznie funkcje top-level z hash-pinned
runtime/launchera w świeżym procesie. Pełna ścieżka PS5.1 przeszła `11`
scenariuszy / `57` asercji przy `0` granicach produkcyjnych. Realny read-only
preflight przeszedł pliki `4/4`, kontenery `6/6`, PostgreSQL healthy, Public
ready, Private/Supervisor absent i HTTP `200/200/200/404`. Po jednym UAC
operacja `R04-D21-P4B-USABLE-WARM-CONTINUE-DEPS-20260924T164318Z` zapisała
`PARTIAL_PENDING_OPERATION_UNKNOWN`: rejestracja Host on-demand została
przekazana, lecz nie potwierdzono jej wyniku. Późniejszy dozwolony readback
wykazał Host `Ready`, enabled, bez triggera i bez aktywnej instancji, ale z
semantic/comparable `66ECF758...B938` / `17195620...BA92`, innymi niż
przypięty wariant on-demand. Z tego powodu nie wykonano warm runów, logon,
CRM/Web ani SAFE_INACTIVE. Zgoda i UAC są zużyte; bez retry.

**Aktualizacja USABLE-WARM HOST-RECONCILE 2026-09-24:** owner-authorized
pochodna `6C3155EE...3A690` z indeksem `039166A4...5A0E` dopuściła użycie
wyłącznie exact, bezczynnego Host on-demand. Offline PS5.1 przeszedł `14`
scenariuszy / `66` asercji przy `0` granicach produkcyjnych; bounded preflight
potwierdził pliki `4/4`, kontenery `6/6`, PostgreSQL healthy, Host exact/idle,
Public present, Private/Supervisor absent i HTTP `200/200/200/404`. Po jednym
UAC rodzic zapisał `PARTIAL_PENDING_OPERATION_UNKNOWN`, lecz recorder
`20260924T200607127Z-09f89fd4` następnie utrwalił kompletny wynik
`BASE_READY_LIMITED_CAPTURED`, exit `0`, rozliczone dziecko i Private
`START_ONCE=1`. Post-check potwierdził Host `Ready`/idle/LastResult `0`,
Private present bez drugiej instancji, Supervisor absent oraz niezmienione sześć
ID/image. Drugi warm run, jego dowód braku duplikatu, logon i CRM/Web nie zostały
wykonane. Bez retry, drugiego UAC i SAFE_INACTIVE.

**Aktualizacja USABLE-WARM REPEAT 2026-09-25:** właściciel zaakceptował pierwszy
warm run na podstawie późniejszego recordera i zatwierdził jedną kontynuację
`R04-D21-P4B-USABLE-WARM-REPEAT-20260924T230413Z`. LOCAL_ONLY pochodna
`2498FC31...99953` z indeksem `494536D6...05EE` przeszła PS5.1 offline `14`
scenariuszy / `72` asercje i bieżący read-only preflight. Jeden UAC/PID `33600`
uruchomił dokładnie jeden Host; proces rodzica zachowuje
`PARTIAL_PENDING_OPERATION_UNKNOWN`, lecz trwały recorder
`20260925T063227046Z-3e1e1246` (`D19E17A2...669E`) rozlicza to samo wykonanie:
child exit `0`, `BASE_READY_LIMITED_CAPTURED`, starty usług/kontenerów `0`,
Supervisor `0`, Host po próbie `Ready`/idle/LastResult `0`. Nie było retry ani
SAFE_INACTIVE. Późniejsza pojedyncza rejestracja logon ma journal
`PENDING_UNKNOWN`, ale readback potwierdził jeden trigger, poprawną akcję i
...
`R04-ONE-ENTRY-COLD`. Właściciel następnie zatwierdził jego kompletne
przygotowanie SOURCE/OFFLINE: exact klient i skrót odczytano, wdrożono w source
hash-bound `OPEN_AFTER_BASE_READY`, wykonano workflow testy i przygotowano jedno
przyszłe okno manual-entry + reboot/logon. Status pozostaje
`READY_FOR_REVIEW / NOT_DEPLOYED / NOT_APPROVED`; operacje hosta wymagają jednego
bieżącego potwierdzenia exact opublikowanego pakietu.

**Aktualizacja R04-ONE-ENTRY-COLD 2026-09-25:** bieżący skrót
`8B46D106...9BBDD` otwiera exact klienta Windows `1.0.2+29`
`5BD959A3...B0865`; Startup wrapper jest nieobecny, a Host/recorder pozostaje
wspólną ścieżką. Kandydat launcher/runtime/recorder implementuje otwarcie klienta
dopiero po base readiness, preserve na repeat, fail-closed identity i bezpieczny
komunikat po odmowie. Końcowe PS5.1 plan/adapters/recorder/data/P4 są PASS przy
granicach produkcyjnych `0`. Draft nadal `NOT_APPROVED`, installed run01 i skrót
niezmienione; manual entry, UAC, reboot/logon i cold-start są `NOT_RUN`.
Nieaktywny package index `9C1A60AA...BB06C` oraz review ZIP
`B8D33DD1...48876` przeszły roundtrip `13/13`; dokładne źródła launcher/runtime/
recorder mają hashe `491531FB...22D2` / `8265D4E4...B4F3` /
`45CBC5D9...63A7`. Jedno przyszłe okno ma `WINDOW_ID`
`R04-D21-ONE-ENTRY-COLD-WINDOW-20260925T110738Z` i wymaga odrębnej bieżącej
zgody obejmującej instalację, manual entry/repeat oraz jeden reboot/logon.

**Wynik okna R04-ONE-ENTRY-COLD 2026-09-25:** finalny read-only preflight
`730C0CA1...A7D5` potwierdził package, preimage, Host, maintenance, sześć
kontenerów, host services i HTTP. Deterministyczny manifest operacyjny
`6219F891...0BBA` oraz indeks `F2905BD6...2535` wiązały dokładne wejścia.
Jeden RunAs/UAC uruchomił PID `52112`; operacja zakończyła się exit `22` z
`Cannot bind argument to parameter 'ManifestPath' because it is an empty
string.` po chwilowym zapisaniu czterech plików, ale przed zmianą skrótu,
task write lub startem Host. Jeden dozwolony rollback zakończył się
`SAFE_INACTIVE_COMPLETE`. Końcowy exact readback potwierdził 4/4 zainstalowane
pliki oraz skrót w dokładnych preimage; Host pozostał Ready z jednym istniejącym
triggerem i nie został uruchomiony. Manual entry/repeat, reboot/logon, cold test
i CRM/Web są `NOT_RUN`. USABLE-WARM pozostaje przyjęty i nietknięty; zgoda/UAC
są zużyte, bez retry w tym oknie.
...
**R04-P4B-USABLE-WARM 2026-09-24 — reconciliation po partial:** jedyna
owner-authorized próba Stage B zainstalowała cztery exact pliki i zakończyła
`PARTIAL_PENDING_OPERATION_UNKNOWN / TASK_POSTCHECK_NOT_CONFIRMED` przed warm
runs. Późniejsza pojedyncza kampania read-only exact Host potwierdziła
`Disabled`, `Enabled=false`, `Triggers=0`, brak stanu Running/Queued,
LastRunTime `2026-09-19T21:07:53Z` i LastTaskResult `22`. Current XML
`AA989DF6...FAA08` / comparable `E3147E8F...D0BA7` ma wszystkie pola zgodne z
disabled targetem, lecz istniejący hash guard odrzuca różnicę deklaracji
`utf-16` versus `UTF-16`. Historyczna odpowiedź post-checku nie została
zachowana i pozostaje `NOT_CAPTURED`. Odczyt nie wykonał mutacji. Następna
decyzja może objąć wyłącznie wąską kontynuację od exact current hash do
on-demand, dwóch warm runs `Private 1 -> 0`, logon i CRM/Web; bez ponownej
rejestracji disabled, kopiowania czterech plików lub pełnego InstallAndWarm.

**R04-P4B-USABLE-WARM 2026-09-24 — skonsolidowana próba dokończenia:** po
wycofaniu wymogu diagnozowania zaginionej osłony lokalny PS5.1 preflight
poprawnego wywołania przeszedł `4/4` pliki, `6/6` exact kontenerów, PostgreSQL
healthy, Public PRESENT/ready, Private/Supervisor ABSENT i HTTP
`200/200/200/404`. Jeden potwierdzony UAC uruchomił PID `73504`, który zakończył
się `0`, ale nie pozostawił `apply`, preflightu, journalu ani `result.json`;
wejście kontynuacji i dokładny błąd są `NOT_EVIDENCED / NOT_AVAILABLE`.
Jedyny bounded post-check potwierdził niezmieniony Host
Disabled/no-trigger/idle, sześć tożsamych kontenerów, Public ready oraz
Private/Supervisor absent. Warm runs pozostają `0/2`, logon nie jest
skonfigurowany, CRM/Web nie otwarto, SAFE_INACTIVE nie uruchomiono. UAC i zgoda
są zużyte; nie ma automatycznego retry.
...
| Checkpoint ID | `20260925T115425Z-R04-D21-ONE-ENTRY-COLD-WINDOW-PARTIAL` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-25T11:54:25.5965941Z`: jedyny UAC/PID `52112` zakończył instalację exit `22` na kolizji parametru `ManifestPath`; jeden rollback zakończył się `SAFE_INACTIVE_COMPLETE`, po czym exact readback potwierdził 5/5 preimage |
| Aktualny wykonawca / sesja | Codex / R04-ONE-ENTRY-COLD WINDOW; OPERATION STOPPED AFTER SAFE ROLLBACK |
| Aktywny pakiet / podetap | R04/D-21 — `PRE_REBOOT_INSTALL_FAILED_VALIDATION_MANIFEST_PARAMETER_COLLISION / SAFE_INACTIVE_COMPLETE / UAC_CONSUMED`; USABLE-WARM zachowuje odbiór |
...
| Ostatnia faktycznie zakończona czynność | Exact post-rollback readback potwierdził launcher/runtime/recorder/manifest i skrót w 5/5 preimage; Host pozostał Ready, enabled, z jednym istniejącym triggerem, LastResult `0` i niezmienionym LastRun. Operacja zatrzymała się przed manual entry i rebootem |
| Potwierdzone testy bieżącego wykonania | Bounded preflight PASS: package, installed preimages, entry, Host, maintenance, 6/6 kontenerów, host services i HTTP `200/200/200/404/404`; LOCAL_ONLY operator parser i ValidateOnly PASS. Operacyjna walidacja po copy FAIL na pustym `ManifestPath`; rollback i exact 5/5 post-check PASS. Docker/WSL pool i swap pozostają UNKNOWN, nie PASS |
| Niezacommitowana praca / zabezpieczenie | Chronione LOCAL_ONLY dowody okna znajdują się pod istniejącym stagingiem `one-entry-cold-20260925T110216Z\window-20260925T110738Z`; zawierają preflight, deterministyczne pochodne approval/index, journal, result oraz rollback. Historyczne dowody pozostają niezmienione |
| Niezakończone procesy i skutki operacyjne | Elevated PID `52112` jest zakończony. Mutation była chwilowo rozpoczęta dla czterech plików, pending=false; rollback COMPLETE przywrócił preimage. Shortcut/task writes/Host/service/container starts `0`; manual entry/repeat/reboot/logon/cold/CRM-Web `0`. Jeden UAC i jeden rollback są zużyte |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260925T115425Z-R04-D21-ONE-ENTRY-COLD-WINDOW-PARTIAL.md` |
| Zakres aktualnej zgody | Jednorazowe okno `R04-D21-ONE-ENTRY-COLD-WINDOW-20260925T110738Z` zostało wykonane do pierwszego błędu i zatrzymane po dozwolonym rollbacku. Nie ma zgody na retry, drugi UAC, manual entry ani reboot |
| Blokada / wymagana decyzja | K1: operator walidacyjny utracił przekazany `ManifestPath` po dot-source launchera; instalacja nie może przejść do shortcut/manual/cold. Potrzebna jest nowa, exact decyzja właściciela po minimalnej korekcie i sprawdzeniu tego wywołania. Docker/WSL pool i swap pozostają UNKNOWN |
| Jeden następny bezpieczny krok | Minimalnie poprawić i offline sprawdzić kolizję `ManifestPath`, następnie przedstawić właścicielowi jeden nowy exact zakres okna; bez automatycznego retry obecnej operacji i bez ponawiania USABLE-WARM |
...
|---|---|
| Bieżący krok P4/B | Host22/NUP i D-23 `2/2` zachowują odbiory. Source `49c3f64c...` pozostaje odebrany. Stage B zainstalowało cztery exact pliki i historycznie zakończyło `PARTIAL_PENDING_OPERATION_UNKNOWN`. Jedna późniejsza exact Host reconciliation potwierdziła disabled/no-trigger/not Running-or-Queued oraz representation-only hash mismatch `utf-16`/`UTF-16`; primary historical post-check `NOT_CAPTURED`. Warm `0/2`, logon `NOT_ENABLED`, rollback niewykonany. Następny zakres wymaga właścicielskiej zgody na wąskie exact-current -> on-demand -> warm 1→0 -> logon -> CRM/Web, bez pełnego InstallAndWarm |
...
| R04 | IN_PROGRESS | `20260925T115425Z-R04-D21-ONE-ENTRY-COLD-WINDOW-PARTIAL` / PRE-REBOOT INSTALL FAILED, SAFE ROLLBACK COMPLETE | USABLE-WARM zachowuje odbiór. Exact bounded preflight przeszedł, lecz jedyny UAC/instalacja zakończyły się exit `22` na kolizji `ManifestPath`. Jeden SAFE_INACTIVE przywrócił 5/5 preimage; skrót i Host nie zostały zmienione/uruchomione. Manual entry, reboot/logon, cold i CRM/Web NOT_RUN. R04-DATA-BACKUP, relokacje i całe R04 pozostają otwarte; P5/D-22 nieuruchomione |
...
3t. Późniejsza właścicielska decyzja zachowała oryginalne bajty i dopuściła
wyłącznie LOCAL_ONLY konfigurację Stage B. Nowy manifest
`startup-set.approved-for-start.json` ma `31588` B / SHA-256
`38D7C529FD7CE37E25E32A58CC9CF46075FF5E97D7E60F3618D088653C492BDE`;
zmienia tylko `approval.status`, `installation_authorized` i
`startup_authorized`, zachowując `set_id` oraz `decision_id=D-21`. Indeks
`package-index.install-preapproval.json` ma `10908` B / SHA-256
`E298F50753BECB023A5A159F010D746046A111DFCEC2B51E89F3E8C2027C93AD`,
wiąże nowy manifest, lecz nadal ma top-level
`PROPOSED_AWAITING_SEPARATE_OWNER_OPERATIONAL_APPROVAL` i wszystkie pola
operacyjne false. Niezmienione `Test-P4BPackageIndex` oraz
`Test-StartupSetManifest` na izolowanej projekcji przeszły pod PS5.1; kontrolny
`NOT_APPROVED` zwrócił `START_NOT_APPROVED`. Przyszłe dokładne przejście sześciu
pól authorization daje wyliczony, jeszcze niezapisany index
`CF1CCA92DFE3E9A0791615F0EA22454E45D600FDA634FC9DF03D507441A7CCE7`.
Status `P4B_USABLE_WARM_STAGE_B_CONFIGURATION_PREPARED / OFFLINE_GUARDS_PASS /
OPERATION_NOT_AUTHORIZED_NOT_RUN`; następna czynność to jedna bieżąca decyzja
na exact okno, nie kolejny preflight ani review K2/K3.
3u. Właściciel udzielił dokładnej zgody, a osobny indeks operacyjny ma
`10904` B / `CF1CCA92DFE3E9A0791615F0EA22454E45D600FDA634FC9DF03D507441A7CCE7`.
Jedno `InstallAndWarm` przez jeden UAC potwierdziło instalację czterech exact
plików, po czym disabled Host register dał `PENDING_UNKNOWN /
TASK_POSTCHECK_NOT_CONFIRMED`; result `PARTIAL_PENDING_OPERATION_UNKNOWN`.
Recepta zamknęła journal, zachowała pliki i nie wykonała destrukcyjnego
rollbacku. Warm `0/2`, Host/Private starts `0`, logon i shortcut test `NOT_RUN`.
Zgoda jest zużyta; bez nowej decyzji nie wolno retry/write/start/rollback.
3v. Późniejsze exact kontynuacje doprowadziły używalny warm segment do stanu
`READY_FOR_OWNER_REVIEW`. Pierwszy recorder attempt uruchomił Private raz;
drugi, odrębny attempt `20260925T063227046Z-3e1e1246` zakończył child exit `0`
i wykazał `0` nowych startów usług/kontenerów oraz brak duplikatu. Host readback
potwierdził `Ready`/idle i jeden trigger logon z exact działaniem oraz principalem
rozstrzygniętym do przypiętego SID. Historyczny logon journal pozostaje
`PENDING_UNKNOWN`; nie było drugiego zapisu, logoffu ani rebootu. Właściciel
potwierdził ręczne read-only otwarcie CRM/Web, listy klientów i szczegółów
