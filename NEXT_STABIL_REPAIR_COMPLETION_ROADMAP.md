...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`; P2 jest `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`; DATA_ONLY source jest `ACCEPTED / OFFLINE_TEST_ONLY / NOT_DEPLOYED`. D21-P3 pozostaje `PREPARATION_PARTIAL / NO_CUTOVER`, lecz guard startupu jest `SOURCE_READY_FOR_REVIEW / SYNTHETIC_TESTS_PASS / NOT_DEPLOYED`; draft manifestu nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260916T183757Z-D21-P3-STARTUP-GUARD` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-16T18:37:57Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 blocker resolution + startup guard SOURCE/ISOLATED TEST ONLY; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P3 preparation` — `PREPARATION_PARTIAL / BASE_START_GUARD_SOURCE_READY_FOR_REVIEW / NO_CUTOVER`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `ddec6ea20e2d755354e742e324d0d9cea5e5802f`; guard source `0ee0ea50943578e6e552aae23ce1688595ddc262`; accepted DATA_ONLY source `cb6e22506a0fecc440400566293524536847b9b0`. Oryginalny preservation `203/203` i zastane runtime/data/junction pozostały nietknięte |
...
| Źródła runtime / release / DB | Exact read `2026-09-16T18:29:26Z` potwierdził backend `9d9b46c5...615a`, image `sha256:6342b36f...d63702`, `/app=C:/ai-lab-core/build/deploy-main-483f9bf8/backend` i `/data=C:/ai-lab-core/data`; recovery/WIP nie jest źródłem runtime. R02 image `sha256:4b12cf0e...37651` jest lokalny. SQL/schema/WAL/tablespaces/queue state nadal `NOT_OBSERVED`; Web P2 pozostaje TEST_ONLY, Supervisor nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | W jednej izolowanej kampanii potwierdzono fail-before i pass-after guarda, auth, `/version`, A4 API oraz compileall; source `0ee0ea50943578e6e552aae23ce1688595ddc262` został zacommitowany do review |
| Potwierdzone testy bieżącego wykonania | Preimage: `2 passed / 9 failed`, oczekiwany exit `1`. Guard: `11 passed`; auth: osiem `401` i zero product lifespan; version+A4 API: `26 passed`; compileall exit `0`. Python `3.12.13`, pytest `8.3.5`, pinned R02 image, `network=none`, read-only source/root. DB jest contract fake, nie PostgreSQL integration |
...
| Niezakończone procesy i skutki operacyjne | Jeden testowy kontener `e9a43bf2...c62c` został utworzony, zweryfikowany, zakończony i usunięty; exact read po cleanup zwrócił `No such container`. Restart/diagnostic upload/start produktu = `0`. Supervisor pozostaje `INTENTIONALLY_STOPPED / NOT_STARTED_BY_THIS_SESSION`. Nie zmieniono produkcyjnych kontenerów, junctionu, tasków, mountów, flag, kolejek, harmonogramów, danych, backupu lub restore |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260916T183757Z-R04-D21-P3-STARTUP-GUARD.md`; poprzedni handoff: `docs/recovery/checkpoints/20260916T161420Z-R04-D21-P3-STARTUP-GUARD-PARTIAL.md` |
| Zakres aktualnej zgody | D21-P3: jedna lokalna diagnoza Docker, exact resource reads i startup guard `SOURCE / ISOLATED TEST ONLY`; bez cutoveru, SQL produkcji, instalacji, startu/restartu produktu, zmian runtime/config/data/mount/task/flag/queue, backupu/restore/escrow lub P4/P5 |
| Blokada / wymagana decyzja | Current SQL/schema/WAL/tablespaces/queue state i rollback freshness pozostają nierozstrzygnięte; taski backup `267014/1/1` nie potwierdzają bieżącej świeżości. Guard oczekuje owner review, a produkcyjny manifest pozostaje niezatwierdzony |
| Jeden następny bezpieczny krok | Owner review source/evidence guarda; po jego odbiorze osobna zgoda na ograniczone rozstrzygnięcie current SQL i świeżości rollbacku przed jakimkolwiek P3 cutover |
...
| R04 | IN_PROGRESS | `R04-20260916T183757Z-D21-P3-STARTUP-GUARD` / PREPARATION + GUARD SOURCE READY FOR REVIEW, NO CUTOVER | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 P1 i P2 zachowują odbiory; DATA_ONLY source `cb6e225...` jest `ACCEPTED / OFFLINE_TEST_ONLY / NOT_DEPLOYED`. P3 exact Engine/image/resource gates przeszły, a guard seed/reconcilera source `0ee0ea5...` ma syntetyczne fail-before/pass-after, auth, version/A4 API i compileall. Przyczyna wcześniejszych timeoutów pozostaje `UNKNOWN`; current SQL i aktualność rollbacku nadal blokują operacyjny P3. Manifest nadal `NOT_APPROVED_FOR_START`; cutover, relokacja, task install, live startup/reboot/rollback, cleanup i runtime acceptance są `NOT_RUN` |
