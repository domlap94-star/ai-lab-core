...
| Checkpoint ID | `20260925T195729Z-R04-D21-ONE-ENTRY-COLD-UAC-CANCELLED` |
...
| Aktualny wykonawca / sesja | Codex / ten sam ONE-ENTRY-COLD OP; corrected preflight PASS, UAC cancelled before process |
| Aktywny pakiet / podetap | R04/D-21 — `CORRECTED_PREFLIGHT_PASS / UAC_CANCELLED_BEFORE_PROCESS / NO_INSTALL / NO_MUTATION`; USABLE-WARM zachowuje odbiór |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260925T195729Z-R04-D21-ONE-ENTRY-COLD-UAC-CANCELLED.md` |
| Zakres aktualnej zgody | Jedna HTTP-only batch zakończyła brakujący raw i corrected preflight ma PASS. Jedyny zatwierdzony UAC został anulowany przed utworzeniem procesu; instalacja i mutacje `0`. Zgoda UAC jest zużyta, bez retry |
| Blokada / wymagana decyzja | Corrected preflight jest kompletny i zgodny, ale Windows UAC został anulowany przez użytkownika przed procesem. Brak PID/result/install/mutacji; nowy UAC wymaga nowej exact decyzji. Docker/WSL pool i swap pozostają UNKNOWN |
| Jeden następny bezpieczny krok | Wyłącznie nowa exact decyzja na jeden UAC tego samego OP, bez powtarzania completed corrected preflight/HTTP i bez automatycznego retry |
