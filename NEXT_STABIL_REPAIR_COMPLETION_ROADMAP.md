...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 source `f3cbe58bed0de34cfd39a57d481abb2c61c2f4ac` nadal nie jest odebrane ani wdrożone. W ramach warunkowej operacji hostowej istniejący Public Gateway został uruchomiony raz i odpowiada na `127.0.0.1:8789`; `/control*` zwraca `404`. Supervisor nie został uruchomiony: read-only gate wykazał aktywnych producentów oraz `16 advanced_queued`, `18 document_preparation queued`, `1 assistant waiting`, `5956 vision not_evaluated`, 1 spool `UI_CHANGED` i 3 katalogi analysis incoming. Backend ASGI/CORS/API i compileall pozostają `NOT_RUN`; A4 to `SOURCE_PARTIAL / LOCAL_ONLY / PRODUCTION_PARTIAL_UNHEALTHY / NOT_DEPLOYED`. D-21 ustanawia docelowo `SINGLE_INSTALL_ROOT=C:\ai-lab-core` i jeden punkt startu, wymagane w istniejącym R04 po review A4; nie autoryzuje bieżącej migracji. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane, cały R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04 i R05 pozostają `IN_PROGRESS`. Public Gateway został przywrócony jako istniejący endpoint, ale Supervisor bezpiecznie nie wystartował z powodu aktywnych producerów i oczekujących zadań. A4 pozostaje `SOURCE_PARTIAL / LOCAL_ONLY / PRODUCTION_PARTIAL_UNHEALTHY / NOT_DEPLOYED`; R06–R24 nieuruchomione. D-21 zapisuje `SINGLE_INSTALL_ROOT / SINGLE_START_ENTRYPOINT` jako bramkę R04 wymaganą po review A4, bez obecnego przepięcia |
| Checkpoint ID | `R05-20260914T171601Z-A4-HOST-SERVICES-PARTIAL` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-14T17:16:01Z` |
| Aktualny wykonawca / sesja | Codex / R05-A4 host-services continuation: Public Gateway uruchomiony raz; Supervisor zatrzymany przed startem przez bieżącą bramkę kolejki |
| Aktywny pakiet / podetap | `R05 / A4 preview transport and decision result` — `SOURCE_PARTIAL / LOCAL_ONLY / SUPERVISOR_START_BLOCKED_PENDING_OWNER_DECISION / NOT_DEPLOYED`; cały R05 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking `e63f4d1bb1a1dfe319b7789399cae2a878ba43fb`. Cztery source/test paths nadal dokładnie odpowiadają patchowi SHA-256 `81E311E7C129FC7ACC8829BC8B2E3FB31CDFEA6F800D56EE46E16E9E58A17914` i pozostają `LOCAL_ONLY` |
| Gałąź / SHA kodu objętego sprawdzeniem | Opublikowany A4 source/test pod review `f3cbe58bed0de34cfd39a57d481abb2c61c2f4ac`; parent bieżącego checkpointu `e63f4d1bb1a1dfe319b7789399cae2a878ba43fb`; accepted R05-A2 product source `d1b0518ad9aefb5bc05cd89308de923ca54d2809`; accepted R05-A1 source `d31e105427acd40733e91c4d4b46f0412b0f95ad`; baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a` |
| Baseline commit dokumentacji | `483f9bf8b1a591ded8a42df5da87663c664ed5d4` |
| Źródła runtime / release / DB | Backend nadal ma ID `9d9b46c530412e48562b5427a0586b08c1e919ff293ec2cbd1489faffc98615a`, image `sha256:6342b36fa2cdd2501ea4e0e9fada9a9ffaa4894f0c512f19f822f009e8d63702`, `/app=C:/ai-lab-core/build/deploy-main-483f9bf8/backend`, `/data=C:/ai-lab-core/data`; recovery/WIP nie jest zamontowane. Public Gateway task `\NEXT Stabil - Public Gateway` wykonuje czysty `C:\ai-lab-core\operations\gateway\public_web_server.cjs` przez `C:\Program Files\nodejs\node.exe`. Supervisor task wskazuje czysty `C:\ai-lab-core\operations\supervisor\server.js`, ale aktywny main `vision_queue.js` nie zawiera zaakceptowanego, niewdrożonego replay guardu recovery |
| Ostatnia faktycznie zakończona czynność | Po resource gate (`7.537 GiB` Windows available; commit headroom `37.647 GiB`; docker-desktop available `14.012 GiB`) wykonano jeden dozwolony start Public Gateway. PID `41784`, listener `127.0.0.1:8789`, `/gateway-health`, `/health`, `/version` i Web `200`; `/control`, `/control/`, `/control/health` = `404`. Następnie read-only flag/DB/spool gate zatrzymał operację przed startem Supervisora |
| Potwierdzone testy bieżącego wykonania | Bez nowych testów produktu. Wykonano wyłącznie health/boundary Public Gateway i read-only Supervisor start gate. Backend ASGI/CORS/API/compileall: `NOT_RUN`, ponieważ warunek obu usług nie został spełniony. Wcześniejsze Flutter fail-before `6 pass / 4 fail`, focused `11 passed`, Documents/Auth `28 passed`, analyze PASS pozostają historycznymi wynikami niezmienionego WIP |
...
| Niezakończone procesy i skutki operacyjne | Public Gateway został uruchomiony przez istniejący task i pozostaje działający: PID `41784`, `127.0.0.1:8789`. Supervisor nie został uruchomiony; nie zmieniono flag, kolejek, spoolu, harmonogramów ani danych. Nie utworzono kontenera testowego. Docker/WSL/Windows nie restartowano ponownie |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260914T171601Z-R05-A4-HOST-SERVICES-PARTIAL.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260914T144558Z-R05-A4-DOCKER-UPDATE-PARTIAL-RECOVERY.md` |
| Zakres aktualnej zgody | Warunkowy pojedynczy start każdego z dwóch istniejących tasków. Start Public Gateway wykorzystano. Start Supervisora nie został wykonany, ponieważ warunek braku nieuprawnionego wznowienia zadań nie przeszedł. D-21 autoryzuje zapis docelowego pojedynczego rootu/startu, nie migrację ani deployment |
| Blokada / wymagana decyzja | `SUPERVISOR_START_BLOCKED_PENDING_OWNER_DECISION`: efektywne flagi producerów są aktywne, DB zawiera `16 advanced_queued`, `18 document_preparation queued`, `1 assistant waiting`, `5956 vision not_evaluated`, a spool zawiera 1 `UI_CHANGED` i 3 analysis incoming. Start istniejącego Supervisora mógłby uruchomić pracę poza zgodą. Public Gateway jest zdrowy, więc dawny `ENGINE_TIMEOUT` nie jest bieżącą blokadą |
| Jeden następny bezpieczny krok | Właściciel zatwierdza pozostawienie Supervisora jako `INTENTIONALLY_STOPPED` oraz jednorazowe wykonanie wyłącznie izolowanej kampanii A4 mimo tej świadomej bramki. Bez takiej decyzji nie uruchamiać Supervisora ani testów i nie zmieniać producerów/kolejek |
...
| R04 | IN_PROGRESS | `R04-20260911T095238Z-A3-HANDOFF` / A3 OWNER ACCEPTED; D-21 REQUIRED AFTER A4 REVIEW | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 zastępuje docelową politykę rozproszonych roots: wymagane są `SINGLE_INSTALL_ROOT=C:\ai-lab-core` i jeden zwykły start, lecz inventory/mapping/cutover/rollback wymagają osobnego zakresu R04 po review A4. Brak bieżącej zgody na przeniesienie, mount, task, cleanup lub deploy |
| R05 | IN_PROGRESS | `R05-20260914T171601Z-A4-HOST-SERVICES-PARTIAL` / A4 HOST SERVICES PARTIAL | A1/A2/A3 zachowują wąskie odbiory `NOT_DEPLOYED`; A4 pozostaje nieodebrane. Public Gateway przywrócony jednym startem i chroni `/control*`; Supervisor nie został uruchomiony, bo aktywne flagi i zadania nie spełniły bramki bezpiecznego resume. Backend ASGI/CORS/API/compileall `NOT_RUN`; cztery pliki WIP nadal `LOCAL_ONLY / NOT_DEPLOYED`. Oczekiwana decyzja: `INTENTIONALLY_STOPPED` + jawne dopuszczenie izolowanych tests, bez realnego eksportu |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
| R09 | PLANNED | — | — |
...
3. Historyczna reguła przed D-21 dopuszczała odrębne odtwarzalne deployment roots. D-21 zastępuje ją dla stanu docelowego: `SINGLE_INSTALL_ROOT=C:\ai-lab-core`, jawne wersjonowane podkatalogi komponentów i jeden zwykły, idempotentny punkt startu. Bieżący runtime/WIP pozostaje wyjątkiem do osobno zatwierdzonego inventory, mapy stare→docelowe, cutoveru, testu rollback i dopiero potem cleanupu; produkcja nie może wykonywać recovery/staging/test WIP.
