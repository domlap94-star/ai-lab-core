...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 pozostaje `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`; P2 jest `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`. Junction D: jest owner-approved; jego source jest `READY_FOR_REVIEW / OFFLINE_TEST_ONLY`, a draft manifestu `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260916T062447Z-D21-ACTIVE-DATA-JUNCTION` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-16T06:24:47Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 active-data junction source and candidate synchronization; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P2 continuation` — `ACTIVE_DATA_D_JUNCTION_SOURCE_READY_FOR_REVIEW / OFFLINE_TEST_ONLY`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `f872cf9e548a7ec196289e0c1987b654b1017505`; source commit opublikowany `e4f298a46efa2ad29921fcf7cce0ca6a04f3d0fd`. Oryginalny preservation `203/203` i zastane runtime/data/junction pozostały nietknięte |
| Gałąź / SHA kodu objętego sprawdzeniem | Aktualny source set `e4f298a46efa2ad29921fcf7cce0ca6a04f3d0fd`, tree `873d199ed43c5d6719a34f2158b0537f7e07a73b`; accepted P2 pre-junction source `2e69622bc6a0b4888427f8ae5be119377aed26d9`; P2 evidence `f872cf9e548a7ec196289e0c1987b654b1017505`; baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a` |
...
| Ostatnia faktycznie zakończona czynność | Dodano fail-closed `NEXT_STABIL_DATA_TOPOLOGY_V1` dla dokładnego junctionu DATA_ONLY, 20 asercji topologii i zachowano regresje P1; opublikowano source `e4f298a...`. Zsynchronizowano kandydat i mapę bez zmiany junctionu, danych lub backupów |
| Potwierdzone testy bieżącego wykonania | Windows PowerShell `5.1.26100.8894`: junction 20/20, host plan 53/53, real-adapter mapping 48/48, parser 0 błędów i JSON PASS. Preimage odrzucił poprawny syntetyczny junction przez `CONTAINER_BIND_REPARSE_POINT:backend`, zgodnie ze starą polityką. Testy użyły tylko własnego syntetycznego junctionu i pełnych atrap; aplikacja/UI/browser/backend/DB/modele/backup: `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Source jest w Git i remote na `e4f298a...`; surowe logi pozostają LOCAL_ONLY pod istniejącym stagingiem P2. Oryginalny worktree, audit, wcześniejszy snapshot `318A41...CC6C3`, Web `A99FB9...C41`, dane D:, backupy i profile pozostały bez zmian |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260916T062447Z-R04-D21-ACTIVE-DATA-JUNCTION.md`; poprzedni handoff: `docs/recovery/checkpoints/20260915T220132Z-R04-D21-P2-CANDIDATE.md` |
| Zakres aktualnej zgody | D21/P2 continuation: dokładny `ACTIVE_DATA_ONLY`, testy offline, mapa zapisów i nieaktywny kandydat; bez instalacji, startu, zmiany linku/danych/backupów lub P3–P5 |
...
| Jeden następny bezpieczny krok | Review source DATA_ONLY i osobna zgoda na minimalny pakiet P3: `KEEP` potwierdzonych bindów na D:, read-only rozliczenie UNKNOWN, dokładna konfiguracja/mounty oraz oddzielne przełączenie wersji backendu |
...
| R04 | IN_PROGRESS | `R04-20260916T062447Z-D21-ACTIVE-DATA-JUNCTION` / DATA_ONLY SOURCE READY FOR REVIEW | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 P1 pozostaje accepted source/offline. P2 jest `PRESERVATION_AND_CANDIDATE_ACCEPTED / NOT_DEPLOYED`. Source `e4f298a...` dopuszcza dokładny owner-approved active-data junction i ma status `READY_FOR_REVIEW / OFFLINE_TEST_ONLY`; manifest nadal `NOT_APPROVED_FOR_START`. P3–P5, cutover, task install, live startup/reboot/rollback, niepotwierdzone relokacje, cleanup i runtime acceptance są `NOT_RUN` |
