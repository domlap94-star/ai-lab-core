...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION jako cały pakiet. Właściciel zaakceptował R03-A4 na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`. R04-A1 i wąski wynik R04-A2-UI06 pozostają zaakceptowane jako `NOT_DEPLOYED`; D-19 zachowuje odbiór R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae`. D-20 obejmuje wspólną ochronę Vision V1 / Visual V2. R05-A1 na source `3a36d5b3866e277b9186028b1926debfe92209e6` osiągnęło `HANDOFF_SCOPE_TRANSACTION_FIX_READY_FOR_REVIEW / NOT_DEPLOYED`: ostatnia granica ponownie sprawdza zgodę, cancel, business scope i exact bytes; pierwszy candidate jest trwały między requestami PostgreSQL; faktyczne V1/V2 mają jeden fake handoff dla ważnej zgody i zero dla wygasłej. Real export, realny Supervisor/Temporary Chat/model, panel operatora i end-to-end pozostają `NOT_VERIFIED` i nieautoryzowane. Android runtime pozostaje `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 nadal `WAITING_OWNER_VISUAL_EVIDENCE`. Cały R04 i R05 pozostają IN_PROGRESS. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A3 pozostaje zaakceptowane jako `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED` dla source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae`. D-20 obejmuje wspólną ochronę Vision V1 / Visual V2; R05-A1 ma wynik `HANDOFF_SCOPE_TRANSACTION_FIX_READY_FOR_REVIEW / NOT_DEPLOYED` na source `3a36d5b3866e277b9186028b1926debfe92209e6`. R04/R05 pozostają `IN_PROGRESS`; R06–R24 nieuruchomione |
| Checkpoint ID | `R05-20260911T180210Z-A1-HANDOFF-SCOPE` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-11T18:02:10Z` |
| Aktualny wykonawca / sesja | Codex / R05-A1 source/test: świeża kontrola claim → submit, scope dokumentu, trwałość candidate i faktyczne V1/V2 sprawdzone syntetycznie; bez startupu produktu i realnego eksportu |
| Aktywny pakiet / podetap | `R05 / A1 privacy and activation` — `HANDOFF_SCOPE_TRANSACTION_FIX_READY_FOR_REVIEW / NOT_DEPLOYED`; cały R05 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; parent local/remote `b9508291282a8eee0a56d89575ab242630e1ebad`; przetestowany source `3a36d5b3866e277b9186028b1926debfe92209e6`. Dokumentacja/checkpoint są objęte jawną allowlistą; ich commit wynikowy odczytuje się z Git |
| Gałąź / SHA kodu objętego sprawdzeniem | accepted R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc`; wejście przeglądu `b9508291282a8eee0a56d89575ab242630e1ebad`; source po poprawce `3a36d5b3866e277b9186028b1926debfe92209e6`; baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a` |
...
| Ostatnia faktycznie zakończona czynność | Opublikowano przetestowany source `3a36d5b3866e277b9186028b1926debfe92209e6`: finalny submit ponownie sprawdza decyzję/cancel/scope/bajty, candidate-first jest trwały między requestami, a opis V2/V2 skorygowano osobnym rzeczywistym V1/V2. Logi zachowano, własne syntetyczne zasoby DB usunięto po kontroli pełnych ID |
| Potwierdzone testy bieżącego wykonania | RV05-01/02 fail-before: `1 passed, 10 failed`, exit `1`; RV05-03 fail-before: `2 passed, 1 failed`, exit `1`. Pass-after: skupiony guard `39 passed`; faktyczne V1/V2 + candidate/scope PostgreSQL `10 passed`; regresja bez DB `313 passed`; regresja PostgreSQL `58 passed`; auth `8 × 401`, product lifespan `0`; `compileall` exit `0`. Wszystkie końcowe exit `0`; szczegóły i hashe w `docs/recovery/R05_A1_PRIVACY_ACTIVATION_EVIDENCE.md` |
...
| Niezakończone procesy i skutki operacyjne | Brak uruchomionych zasobów tej sesji R05. Dokładnie nazwany syntetyczny kontener PostgreSQL, jego anonimowy wolumen i internal network usunięto po kontroli pełnych ID i owner/run. Logi są `LOCAL_ONLY`. Aplikacja, realne dispatchery, Supervisor, Temporary Chat, modele i realny eksport: `NOT_RUN` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260911T180210Z-R05-A1-HANDOFF-SCOPE.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260911T161959Z-R05-A1-V1-V2-HANDOFF.md` |
| Zakres aktualnej zgody | D-20 SOURCE/TEST ONLY: domknięcie finalnej granicy claim → submit, server-derived business scope, trwałość tras candidate/approve/revoke i test faktycznych V1/V2; wyłącznie syntetyczna DB/filesystem oraz fake granice, bez normalnego startupu, realnego Supervisora/Temporary Chat/modelu, eksportu i deploymentu |
| Blokada / wymagana decyzja | Źródłowa wspólna granica jest gotowa do review. Nadal brak odbioru realnego uploadera/workerów, używalnego panelu decyzji operatora, realnego eksportu i end-to-end Temporary Chat; R05 jako całość pozostaje `IN_PROGRESS` |
| Jeden następny bezpieczny krok | Właściciel przegląda i odbiera albo odrzuca R05-A1 source `3a36d5b3866e277b9186028b1926debfe92209e6` wraz z bieżącym evidence; każda próba realnego eksportu/end-to-end wymaga osobnej zgody i nie rozpoczyna R06 automatycznie |
...
| R05 | IN_PROGRESS | `R05-20260911T180210Z-A1-HANDOFF-SCOPE` / SOURCE READY FOR REVIEW | Source `3a36d5b3866e277b9186028b1926debfe92209e6`: finalny guard rewaliduje decyzję/cancel/business scope/exact bytes; candidate-first jest trwały pomiędzy requestami PostgreSQL; faktyczne V1/V2 mają jeden fake handoff przy ważnej zgodzie i zero przy wygasłej. Regresje `313 + 58 passed`, auth `8 × 401`, product lifespan `0`; `NOT_DEPLOYED`, real export/uploader/UI/end-to-end `NOT_VERIFIED` |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
| R09 | PLANNED | — | — |
...
`3a36d5b3866e277b9186028b1926debfe92209e6` ma status
`HANDOFF_SCOPE_TRANSACTION_FIX_READY_FOR_REVIEW / NOT_DEPLOYED`. Dowód obejmuje
świeżą kontrolę na granicy claim → submit, trwałość candidate między requestami,
wersjonowany server-derived scope oraz rzeczywiste współbieżne wejścia V1/V2.
