...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. Właściciel zaakceptował R05-A4 na source `04ab5e58cf86896ffd946cabffde13367d343f53` i evidence `8620871711321a42e62291e52865b5a668a4955d` jako `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`; operator Web runtime, Temporary Chat, remote upload, locally_redacted generation i external end-to-end pozostają `NOT_VERIFIED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. D-21 ma przygotowaną 33-elementową mapę i plan `SINGLE_INSTALL_ROOT=C:\ai-lab-core / SINGLE_START_ENTRYPOINT` jako `READY_FOR_REVIEW / NO_OPERATIONAL_CHANGES`; cutover, relokacja, cleanup i startup implementation są `NOT_RUN`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 jest przyjęte przez właściciela jako `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED` dla source `04ab5e58cf86896ffd946cabffde13367d343f53` i evidence `8620871711321a42e62291e52865b5a668a4955d`. R04/D-21 read-only inventory, mapa i plan są `SINGLE_ROOT_STARTUP_MAP_READY_FOR_REVIEW / NO_OPERATIONAL_CHANGES`; cały R04 i R05 pozostają `IN_PROGRESS`. R03 pozostaje `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R06–R24 nieuruchomione |
| Checkpoint ID | `R04-20260915T035527Z-D21-SINGLE-ROOT` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-15T03:55:27Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 read-only inventory and consolidation plan; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 single root and startup map` — `READY_FOR_REVIEW / NO_OPERATIONAL_CHANGES`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `8620871711321a42e62291e52865b5a668a4955d`, clean i staged `0`. Cztery pliki A4 są już w source `04ab5e58cf86896ffd946cabffde13367d343f53`; patch SHA-256 `81E311E7C129FC7ACC8829BC8B2E3FB31CDFEA6F800D56EE46E16E9E58A17914` pozostaje historycznym dowodem `LOCAL_ONLY`, nie aktywnym WIP |
...
| Ostatnia faktycznie zakończona czynność | Utworzono read-only inventory i 33-elementową mapę `docs/recovery/R04_SINGLE_ROOT_STARTUP_MAP.csv` oraz plan `docs/recovery/R04_SINGLE_ROOT_STARTUP_PLAN.md`. Potwierdzono rozproszony start: osobne taski Docker/Compose/gateway/Supervisor i zerwany wrapper Startup wskazujący nieistniejący `operations\runtime\start-host-services.ps1`. Nie uruchomiono żadnego launchera ani usługi |
| Potwierdzone testy bieżącego wykonania | Testy aplikacji, Flutter, A/B, CORS, browser i modele: `NOT_RUN` — źródła A4 są niezmienione i odebrane. Wykonano wyłącznie walidację Git/inventory, preservation `203/203`, CSV/JSON/references/diff/secret checks dla dokumentacji |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260915T035527Z-R04-D21-SINGLE-ROOT.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260914T184044Z-R05-A4-BACKEND-TESTS.md` |
| Zakres aktualnej zgody | Odbiór R05-A4 oraz R04/D-21 `READ-ONLY INVENTORY / PLAN / REVIEWED DOCUMENTATION`. Bez cutoveru, relokacji, cleanupu, startup implementation, deployu, testów runtime, R05-A5 i R06–R24 |
| Blokada / wymagana decyzja | D-21 mapa/plan wymagają review. Pierwszy minimalny pakiet `D21-P1` (brakujący launcher, bounded Docker timeout i testy offline) wymaga osobnej zgody source/test. P2–P5 i wszystkie działania operacyjne pozostają niezatwierdzone |
| Jeden następny bezpieczny krok | Właściciel przegląda mapę i osobno zatwierdza albo odrzuca `D21-P1`; bez automatycznego rozpoczęcia cutoveru, relokacji, cleanupu, R05-A5 lub R06 |
| Warunek STOP | Bez przenoszenia/usuwania katalogów, zmian mountów/tasków/shortcutów/config, uruchomienia launchera/usług/producentów/kolejek/modeli, backupu/restore, deploy/release i R05-A5/R06–R24 |
...
| R04 | IN_PROGRESS | `R04-20260915T035527Z-D21-SINGLE-ROOT` / D-21 MAP READY FOR REVIEW | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 ma read-only inventory, 33-elementową mapę i plan jednego startu jako `SINGLE_ROOT_STARTUP_MAP_READY_FOR_REVIEW / NO_OPERATIONAL_CHANGES`. Cutover, relokacja, cleanup, startup implementation, rollback test i pełne runtime acceptance są `NOT_RUN` |
