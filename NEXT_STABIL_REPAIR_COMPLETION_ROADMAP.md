...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. Punkt rollbacku ma ograniczony odbiór właściciela; D21-P3 jest `CORE_BACKEND_SOURCE_SWITCH_READY_FOR_REVIEW / LIMITED_RUNTIME_VERIFIED`. Globalny manifest nadal `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260917T165525Z-D21-P3-CORE-SWITCH-HANDOFF` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-17T16:55:25Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P3 faza B zakończona; ograniczony backend-only switch zweryfikowany; STOP przed P4/P5 |
...
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `78bcf70edc7ed787b377e38461e84c1b4149a29f`; accepted guard/source `0ee0ea50943578e6e552aae23ce1688595ddc262`. Manifest R00 ma `203/203`: `116` wpisów zgodnych na oryginalnych ścieżkach i `87` backendowych zgodnych w rollback; missing/mismatch `0/0`. Oryginalny HEAD pozostaje `72950657ac79b50d0afe72753632ba4cde810b95` |
...
| Ostatnia faktycznie zakończona czynność | Wykonano jeden backend-only stop/install/recreate zgodny z planem. Runtime payload porównany per-file `592/592`, brak missing/mismatch/extra. Backend/gateway odpowiadają, końcowy DB read-only zachował pending `18/16/1`. Rollback nie był potrzebny |
| Potwierdzone testy bieżącego wykonania | Exact mount/image/ID i hash zainstalowanego override, per-file runtime bytes, backend/gateway/OpenAPI/legacy auth HTTP, public `/control*` 404, startup log review, dwa read-only odczyty DB, końcowy host/WSL resource read i niezmienność pięciu pozostałych kontenerów. Niezależny runtime readback flag nie został zachowany i pozostaje `NOT_VERIFIED`. Flutter/Web build, migracje, Supervisor, modele, eksport, P4/P5 `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Lokalny chroniony root `...\p3-core-switch-20260917T141404Z` zachowuje payload, manifesty, override, plan, dokładny stary backend i dowody fazy B. Indeks 32 plików / 180 781 B ma SHA-256 `6D6354AC...2085`; 203/203 chronionych wpisów są rozliczone |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260917T165525Z-R04-D21-P3-CORE-SWITCH-HANDOFF.md`; owner window: `docs/recovery/checkpoints/20260917T163742Z-R04-D21-P3-CORE-SWITCH-OWNER-WINDOW.md` |
| Zakres aktualnej zgody | Wykorzystano zgodę na jedno przełączenie wyłącznie backendu dla dokładnego OP_ID/source/payload/override. Rollback, Supervisor, migracje, relokacja danych i P4/P5 nie zostały wykonane |
| Blokada / wymagana decyzja | Ograniczony wynik runtime oczekuje review. Globalny manifest nadal `NOT_APPROVED_FOR_START`; pełna delta treści pozostaje `DELTA_NOT_FULLY_OBSERVED`; P4/P5, full startup/reboot/rollback acceptance i cleanup wymagają osobnej zgody |
...
| R04 | IN_PROGRESS | `R04-20260917T165525Z-D21-P3-CORE-SWITCH-HANDOFF` / BACKEND SWITCH READY FOR REVIEW | A3 oraz D-21 P1/P2/DATA_ONLY/guard zachowują odbiory. Backend-only switch wykonał dokładny source/payload/override; runtime bytes `592/592`, DB/schema/pending `18/16/1`, junction i pięć pozostałych kontenerów zachowane. Rollback nie był potrzebny. Wynik to `CORE_BACKEND_SOURCE_SWITCH_READY_FOR_REVIEW / LIMITED_RUNTIME_VERIFIED`, nie ACCEPTED. Pełna delta jest `DELTA_NOT_FULLY_OBSERVED`; globalny manifest, relokacja, task install, full startup/reboot/restore i P4–P5 pozostają `NOT_RUN / NOT_AUTHORIZED` |
