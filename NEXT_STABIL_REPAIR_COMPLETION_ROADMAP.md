...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B pozostaje `PARTIAL_AFTER_FAILURE / HOST_TASK_FAILED_22 / SAFE_INACTIVE_PARTIAL_UNKNOWN`; nowa poprawka `HOST22_FINAL_OBSERVATION_CONDITIONS_AND_MANIFEST_BINDINGS_READY_FOR_REVIEW / OFFLINE_TESTS_PASS / NOT_DEPLOYED`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260920T153821Z-D21-P4B-HOST22-FINAL-CONDITIONS` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-20T15:38:21Z` |
| Aktualny wykonawca / sesja | Codex / source i testy offline bez Docker/Task/CIM/TCP/HTTP/UAC/Host; końcowe PS 5.1 `70/57/51/44/41`, candidate binding + P4 `59`, wszystkie exit `0`, granice produkcji `0` |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — Host22 OBS-01B/02B i PKG-HASH-01 gotowe do niezależnego review; installed run01 pozostaje częściowy, retry/update nieautoryzowane |
...
| Ostatnia faktycznie zakończona czynność | Opublikowano source commit `8195e5cf8dacd1976ccd9f71a1f78175c3513acc`: typowana kompletność koperty, kontrola wspólnego deadline bezpośrednio przed pozytywną gotowością i niezależny test bindingów candidate. Przygotowano nieaktywny candidate i ZIP review; instalacji nie zmieniono |
| Potwierdzone testy bieżącego wykonania | Preimage `b4269ffa...`: OBS-01B i OBS-02B `REPRODUCED`. Pass-after PS 5.1: focused `70`, start-host `57`, real adapters `51`, DATA_ONLY `44`, P4 package `41`; pełny candidate binding + P4 `59`, ZIP roundtrip binding `18`; exit `0`. Topologia 6+4, Private START_ONCE/repeat, PostgreSQL `starting -> healthy`, typed/null envelope, deadline przed gotowością i wcześniejsze state/identity guards przeszły; produkcyjne granice `0` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY `C:\Users\domai\AppData\Local\Temp\P4BH22F3-20260920T153726Z`; package index 6 166 B, SHA-256 `75A0005D4963D1FF862D53D50FC28AE3E7ECF7B93C53FF4AD85661CB8D8BAFDB`; ZIP 98 281 B, SHA-256 `D6F48B9ED1178C6362A5BF8A8C79E6B08B72D569D5C0F880FA29990939C27AEA`, roundtrip `21/21`, indeks `20` pozycji + sam indeks; wcześniejsze pakiety i run01 zachowane |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260920T153821Z-R04-D21-P4B-HOST22-FINAL-CONDITIONS.md` |
| Zakres aktualnej zgody | SOURCE/OFFLINE zakres wykonany i zużyty. Brak zgody na update zainstalowanych plików/manifestu, retry Host/Install, UAC, task writes, rollback, logon/reboot, P5 lub R06. D-22 pozostaje `NOT_RUN` |
| Blokada / wymagana decyzja | Niezależny review source `8195e5cf8dacd1976ccd9f71a1f78175c3513acc` i ZIP `D6F48B9E...39C27AEA`, następnie odrębna jednorazowa zgoda na wąski update. Installed run01 nadal używa starych hashy i Host jest disabled/no-trigger |
| Jeden następny bezpieczny krok | Review opublikowanego source/test, wykonawczych bindingów candidate i exact LOCAL_ONLY ZIP; bez UAC, Host retry lub instalacji. Ewentualny późniejszy Host musi trwale zapisać JSON code/events/details, stdout/stderr i exit |
...
| R04 | IN_PROGRESS | `R04-20260920T153821Z-D21-P4B-HOST22-FINAL-CONDITIONS` / P4-B PARTIAL + FINAL HOST22 SOURCE READY | P3/P4-A zachowują odbiory. Exact run01 pozostaje `PARTIAL_AFTER_FAILURE`; Host disabled/no-trigger, warm `0/2`, Private start `0`, Supervisor `INTENTIONALLY_STOPPED`. Source `8195e5cf8dacd1976ccd9f71a1f78175c3513acc` zachowuje wcześniejsze Health/exact-ID/state guards i domyka typowaną kompletność, deadline przed pozytywną gotowością oraz wykonawcze bindingi candidate; offline `70/57/51/44/41`, candidate+P4 `59`, produkcyjne granice `0`. `NOT_DEPLOYED / NO_RETRY_AUTHORIZED`; Qdrant/VHD/profile, UI, backup proof, live logon/reboot i P4-B/P5 pozostają otwarte |
