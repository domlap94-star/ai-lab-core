...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION jako cały pakiet. Właściciel zaakceptował R03-A4 na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`. R04-A1 i wąski wynik R04-A2-UI06 pozostają zaakceptowane jako `NOT_DEPLOYED`; D-19 zachowuje odbiór R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae`. D-20 obejmuje wspólną ochronę Vision V1 / Visual V2. R05-A1 na source `d31e105427acd40733e91c4d4b46f0412b0f95ad` osiągnęło `CLAIM_CERTAINTY_FIX_READY_FOR_REVIEW / NOT_DEPLOYED`: trwały marker rozróżnia potwierdzony brak kontaktu, możliwe rozpoczęcie kontaktu, zapisany external ID i lokalną odmowę; deterministyczne wygaśnięcie przed kontaktem pozwala na nowe jawne dopuszczenie, natomiast przerwanie po fake `create_job` blokuje revoke/retry i drugi handoff. Real export, realny Supervisor/Temporary Chat/model, panel operatora i end-to-end pozostają `NOT_VERIFIED` i nieautoryzowane. Android runtime pozostaje `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 nadal `WAITING_OWNER_VISUAL_EVIDENCE`. Cały R04 i R05 pozostają IN_PROGRESS. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A3 pozostaje zaakceptowane jako `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED` dla source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae`. D-20 obejmuje wspólną ochronę Vision V1 / Visual V2; R05-A1 ma wynik `CLAIM_CERTAINTY_FIX_READY_FOR_REVIEW / NOT_DEPLOYED` na source `d31e105427acd40733e91c4d4b46f0412b0f95ad`. R04/R05 pozostają `IN_PROGRESS`; R06–R24 nieuruchomione |
| Checkpoint ID | `R05-20260911T194028Z-A1-CLAIM-CERTAINTY` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-11T19:40:28Z` |
| Aktualny wykonawca / sesja | Codex / R05-A1 source/test: trwałe rozróżnienie lokalnej odmowy i możliwego rozpoczęcia handoffu sprawdzone na pełnych metodach V1/V2 oraz świeżych sesjach PostgreSQL; bez startupu produktu i realnego eksportu |
| Aktywny pakiet / podetap | `R05 / A1 privacy and activation` — `CLAIM_CERTAINTY_FIX_READY_FOR_REVIEW / NOT_DEPLOYED`; cały R05 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; parent local/remote `0f73eb8d5d9b88df1df33abbbdb77c7c30d876a2`; przetestowany i opublikowany source `d31e105427acd40733e91c4d4b46f0412b0f95ad`. Dokumentacja/checkpoint są objęte jawną allowlistą; ich commit wynikowy odczytuje się z Git |
| Gałąź / SHA kodu objętego sprawdzeniem | accepted R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc`; poprzedni R05-A1 source `3a36d5b3866e277b9186028b1926debfe92209e6`; wejście przeglądu `0f73eb8d5d9b88df1df33abbbdb77c7c30d876a2`; source po poprawce `d31e105427acd40733e91c4d4b46f0412b0f95ad`; baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a` |
...
| Ostatnia faktycznie zakończona czynność | Opublikowano przetestowany source `d31e105427acd40733e91c4d4b46f0412b0f95ad`: wersjonowany stan claimu jest zatwierdzany przed kontaktem, deterministyczna odmowa zwalnia tylko własny pre-contact claim, a marker `contact_may_have_started` przeżywa przerwanie po fake `create_job` i blokuje drugą wysyłkę. Logi zachowano, własne zasoby DB usunięto po kontroli pełnych ID/owner/run |
| Potwierdzone testy bieżącego wykonania | Nowe fail-before A: `2 failed, 1 passed`, exit `1` — V1/V2 mylnie dawały `HANDOFF_UNCERTAIN` po lokalnym expiry; fail-before B: `1 failed`, exit `1` — po przerwaniu powstały `2` fake `create_job`. Pass-after: skupiona regresja `20 passed`; kontrola wyniku z zapisanym external ID `5 passed`; nowy PostgreSQL crash/revoke `1 passed`; regresja bez DB `316 passed`; regresja PostgreSQL `59 passed`; auth `8 × 401`, product lifespan `0`; `compileall` exit `0`. Wszystkie końcowe exit `0`; szczegóły i hashe w `docs/recovery/R05_A1_PRIVACY_ACTIVATION_EVIDENCE.md` |
...
| Niezakończone procesy i skutki operacyjne | Brak uruchomionych zasobów tej sesji R05. Dokładnie nazwany syntetyczny kontener PostgreSQL, jego named volume i internal network usunięto po kontroli pełnych ID i owner/run; brak tych trzech nazw potwierdzono. Logi są `LOCAL_ONLY`. Aplikacja, realne dispatchery, Supervisor, Temporary Chat, modele i realny eksport: `NOT_RUN` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260911T194028Z-R05-A1-CLAIM-CERTAINTY.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260911T180210Z-R05-A1-HANDOFF-SCOPE.md` |
| Zakres aktualnej zgody | D-20 SOURCE/TEST ONLY: trwałe rozróżnienie deterministycznej lokalnej odmowy od rozpoczętego/niepewnego handoffu, zachowanie fail-closed dla historycznych claimów oraz test pełnych V1/V2 na syntetycznej DB/filesystem i fake granicy; bez normalnego startupu, realnego Supervisora/Temporary Chat/modelu, eksportu i deploymentu |
| Blokada / wymagana decyzja | Poprawka źródłowa pewności claimu jest gotowa do ograniczonego review. Nadal brak odbioru realnego uploadera/workerów, używalnego panelu decyzji operatora, realnego eksportu i end-to-end Temporary Chat; R05 jako całość pozostaje `IN_PROGRESS` |
| Jeden następny bezpieczny krok | Właściciel przegląda i odbiera albo odrzuca R05-A1 source `d31e105427acd40733e91c4d4b46f0412b0f95ad` wraz z bieżącym evidence; każda próba realnego eksportu/end-to-end wymaga osobnej zgody i nie rozpoczyna R06 automatycznie |
...
| R05 | IN_PROGRESS | `R05-20260911T194028Z-A1-CLAIM-CERTAINTY` / SOURCE READY FOR REVIEW | Source `d31e105427acd40733e91c4d4b46f0412b0f95ad`: trwałe stany `claimed_no_contact`, `contact_may_have_started`, `external_id_recorded` i `local_denied` rozdzielają bezpieczne reapproval od niepewnego kontaktu; historyczne claimy bez markera pozostają fail-closed. Regresje `316 + 59 passed`, auth `8 × 401`, product lifespan `0`; `NOT_DEPLOYED`, real export/uploader/UI/end-to-end `NOT_VERIFIED` |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
| R09 | PLANNED | — | — |
...
**Bieżący dowód wykonawczy A1:** source
`d31e105427acd40733e91c4d4b46f0412b0f95ad` ma status
`CLAIM_CERTAINTY_FIX_READY_FOR_REVIEW / NOT_DEPLOYED`. Dowód obejmuje
...
kontaktu. Historyczne claimy bez nowego markera pozostają fail-closed.
Realny uploader/UI/export/end-to-end pozostają `NOT_VERIFIED`; nie zmienia to
warunku zamknięcia całego R05 ani zakresu R06.
