...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `FOUR_SERVICE_IDENTITY_RECONCILED / CURRENT_READ_ONLY_EVIDENCE / INACTIVE_STARTUP_PACKAGE_READY_FOR_REVIEW / NOT_INSTALLED`; wszystkie 6/6 identity są przypięte po korekcie `DRAFT_TRANSCRIPTION_ERROR_PROVEN`. Globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260918T012154Z-D21-P4A-FOUR-SERVICE-IDENTITY` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-18T01:18:24Z` |
| Aktualny wykonawca / sesja | Codex / ograniczona kampania READ ONLY rozliczyła cztery role; skorygowano wyłącznie nieaktywny draft i indeks; zero instalacji i zero mutacji runtime; STOP przed P4-B/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-A identity reconciliation` — source/offline `READY_FOR_REVIEW`, identity `6/6`, `NOT_INSTALLED`; cały R04 nadal `IN_PROGRESS` |
...
| Ostatnia faktycznie zakończona czynność | Odczytano jedną listę projektu i celowane bezpieczne metadane czterech usług; potwierdzono pierwotne ID, skorygowano wyłącznie nieaktywny draft oraz nowy lokalny indeks. Niczego nie zainstalowano ani nie uruchomiono |
| Potwierdzone testy bieżącego wykonania | Dziedziczony P3 readback: `11/11`, `9/9 false`, ta sama instancja. P4-A pass-after Windows PowerShell 5.1: P1 plan `53`, real adapters `51`, DATA_ONLY `44`, P4 package `14` asercji. Kolektor reconciliation: `16/16`, po korekcie opcjonalnego `Mount.Name` `17/17`; bezpieczny readback 4/4 i łącznie identity `6/6`. Live launcher/start/logon/reboot/rollback, Flutter/Web/backend tests `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Chroniony root P3 pozostaje. P4-A LOCAL_ONLY root `...\p4-startup-activation-20260917T210051Z` zachowuje historyczne indeksy `DC76525...ACB72` i `78966E...8482`; nowy indeks 18 plików ma 5 545 B i SHA-256 `3FCF1938111C9746FB9F4B5D57F1F98C5085099503B1385B862B0F68776303A9`. Nie jest instalacją |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260918T012154Z-R04-D21-P4A-FOUR-SERVICE-IDENTITY.md` |
| Zakres aktualnej zgody | Zakończona kampania READ ONLY P4-A: proweniencja, discovery czterech ról, korekta nieaktywnego draftu/indeksu i dokumentacja. Bez instalacji, importu taska, manifest approval lub startu |
| Blokada / wymagana decyzja | Identity 6/6 nie jest już blockerem. Qdrant backing, `OPEN_AFTER_BASE_READY`, global manifest approval, task/trigger install, P4-B operational matrix, backup schedule proof i relokacje D: pozostają otwarte |
| Jeden następny bezpieczny krok | Review skorygowanego exact pakietu i osobna decyzja właściciela o P4-B z jawnym rozstrzygnięciem Qdrant/VHD oraz instalacji/aktywacji |
...
| R04 | IN_PROGRESS | `R04-20260918T012154Z-D21-P4A-FOUR-SERVICE-IDENTITY` / P3 ACCEPTED LIMITED + P4-A IDENTITY 6/6 READY FOR REVIEW | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A full-ID/image/file-role/DB-health source i testy offline są `READY_FOR_REVIEW / NOT_INSTALLED`. Kampania READ ONLY potwierdziła `DRAFT_TRANSCRIPTION_ERROR_PROVEN`; bieżące cztery role odpowiadają pierwotnemu runtime inventory, draft ma teraz 6/6 exact identity i pozostaje `NOT_APPROVED`. Qdrant backing, `OPEN_AFTER_BASE_READY`, task install, live startup/reboot/rollback, backup schedule proof, relokacja i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
