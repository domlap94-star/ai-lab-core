...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION jako cały pakiet. Właściciel zaakceptował R03-A4 na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`. R04-A1 i wąski wynik R04-A2-UI06 pozostają zaakceptowane jako `NOT_DEPLOYED`. W D-19 właściciel zaakceptował R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae` jako `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`, z zachowaniem braku stdout/exit wrappera Web i historycznego `test_followup_chunk13_api_auth.py = NOT_RUN`. D-20 zostało doprecyzowane jawną zgodą na wspólną ochronę Vision V1 / Visual V2. R05-A1 osiągnęło `V1_V2_PRIVACY_SOURCE_READY_FOR_REVIEW / NOT_DEPLOYED`: oba wejścia korzystają z jednej trwałej decyzji exact-byte i jednego claimu/handoffu, a testy syntetyczne obejmują realne metody, HTTP/auth oraz dwie sesje PostgreSQL. Real export, realny Supervisor/Temporary Chat/model, panel operatora i end-to-end pozostają `NOT_VERIFIED` i nieautoryzowane. Android runtime pozostaje `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 nadal `WAITING_OWNER_VISUAL_EVIDENCE`. Cały R04 i R05 pozostają IN_PROGRESS. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A3 pozostaje zaakceptowane jako `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED` dla source `41d2b844a7b120a79001ad5cfbe6304b30580dfc` i evidence `457a4321db835421c0d06847e2132d9623d735ae`. D-20 obejmuje teraz jawnie wspólną ochronę Vision V1 / Visual V2; R05-A1 ma wynik `V1_V2_PRIVACY_SOURCE_READY_FOR_REVIEW / NOT_DEPLOYED`. R04/R05 pozostają `IN_PROGRESS`; R06–R24 nieuruchomione |
| Checkpoint ID | `R05-20260911T161959Z-A1-V1-V2-HANDOFF` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-11T16:19:59Z` |
| Aktualny wykonawca / sesja | Codex / R05-A1 source/test: wspólna bramka Vision V1 / Visual V2, HTTP/auth, lifecycle i claim współbieżny sprawdzone syntetycznie; bez startupu produktu i realnego eksportu |
| Aktywny pakiet / podetap | `R05 / A1 privacy and activation` — `V1_V2_PRIVACY_SOURCE_READY_FOR_REVIEW / NOT_DEPLOYED`; cały R05 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; parent local/remote przed publikacją `8550a11991924d5c905813374e4630e7c38b8244`. Wynikowy source/test/docs jest objęty jawną allowlistą; commit wynikowy odczytuje się z Git. Historyczny patch preimage pozostaje `LOCAL_ONLY`: `C:\ai-lab-core-staging\recovery\R05_A1_WIP_20260911T133256Z\r05-a1-source-wip.patch`, SHA-256 `357AF58A42C94ADC09C88DFB2F15D5C36538C6035275ECC72B79E4409C65D958` |
| Gałąź / SHA kodu objętego sprawdzeniem | accepted R04-A3 source `41d2b844a7b120a79001ad5cfbe6304b30580dfc`; start `8550a11991924d5c905813374e4630e7c38b8244`; source baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a`; wynik R05-A1 identyfikuje finalny commit Git, bez wpisywania przyszłego SHA do niego samego |
...
| Ostatnia faktycznie zakończona czynność | Objęto Vision V1 i Visual V2 jedną serwerową decyzją exact-byte oraz wspólnym trwałym claimem; równoległe wejścia w dwóch sesjach PostgreSQL dały jeden fake handoff. Wykonano końcowe regresje, zachowano logi i usunięto wyłącznie własny syntetyczny kontener/sieć R05 |
| Potwierdzone testy bieżącego wykonania | Fail-before V1: `1 failed`, exit `1` — `VisionProcessingService.advance()` osiągał fake Supervisora bez zgody. Pass-after bez sieci: `301 passed`, exit `0`. Izolowany PostgreSQL/internal: `49 passed`, exit `0`, w tym równoczesny claim. Rzeczywisty skrypt auth: `8 × 401`, product lifespan start `0`, exit `0`. `compileall`: exit `0`. Skrypty Office zależne od firmowego dokumentu 770 świadomie `NOT_RUN`, nie są testem R05 |
...
| Niezakończone procesy i skutki operacyjne | Brak uruchomionych zasobów R05. Kontener `next-stabil-r05-a1-test-20260911t155338z-postgres` i sieć `next-stabil-r05-a1-test-20260911t155338z-network` usunięto po kontroli pełnego ID, owner/run, braku mountów i portów. Logi są `LOCAL_ONLY`. Aplikacja, realne dispatchery, Supervisor, Temporary Chat, modele i realny eksport: `NOT_RUN` |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260911T161959Z-R05-A1-V1-V2-HANDOFF.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260911T133256Z-R05-A1-SOURCE-PARTIAL.md` |
| Zakres aktualnej zgody | Doprecyzowane D-20: SOURCE/TEST ONLY dla wspólnej polityki exact-byte, Vision V1, Visual V2, dokumentowego HTTP/auth, dispatcherów i call-site TechnicalAiService; wyłącznie syntetyczna DB/filesystem oraz fake granice, bez normalnego startupu, realnego Supervisora/Temporary Chat/modelu, eksportu i deploymentu |
...
| Jeden następny bezpieczny krok | Właściciel przegląda i odbiera albo odrzuca R05-A1 na opublikowanym SHA; każda próba realnego eksportu/end-to-end wymaga osobnej zgody i nie rozpoczyna R06 automatycznie |
...
| R05 | IN_PROGRESS | `R05-20260911T161959Z-A1-V1-V2-HANDOFF` / SOURCE READY FOR REVIEW | Vision V1 i Visual V2 korzystają z jednej trwałej polityki exact-byte oraz claimu/handoffu. Syntetyczne testy: `301 + 49 passed`, auth `8 × 401`, product lifespan `0`, compileall `0`; źródła `NOT_DEPLOYED`, real export/uploader/UI/end-to-end `NOT_VERIFIED` |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
| R09 | PLANNED | — | — |
