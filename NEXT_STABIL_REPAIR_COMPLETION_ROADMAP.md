...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1/P2/DATA_ONLY/guard zachowują wcześniejsze odbiory. Właściciel przyjął P3 OP_ID `R04-D21-P3-CORE-SWITCH-20260917T141404Z` jako `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; backend `686ac376...c854` wykonuje source `0ee0ea5...`, payload 592/592, DB/schema/pending `18/16/1` i junction zachowano, rollback nie był potrzebny, a późniejszy readback potwierdził dwie warstwy `11/11 MATCH` i `9/9` przełączników `false`. Kontynuacja P4-A odtworzyła i naprawiła w source brak pełnego container-ID binding oraz brak PostgreSQL health-gate; P1/adapter/DATA_ONLY/P4 offline tests przeszły. Nowy poprawnie utrwalony readback dał `2/6 CURRENT_IDENTITY_MATCH`: backend i PostgreSQL są zgodne wraz z RepoDigests, natomiast przypięte pełne ID kontenerów i obrazów Qdrant/n8n/Open WebUI/Ollama zwróciły `No such container/image`. Draft oraz indeks pozostają niezmienione; wynik to `SOURCE_AND_OFFLINE_TESTS_READY_FOR_REVIEW / CURRENT_IDENTITY_READBACK_2_OF_6_PARTIAL / NOT_INSTALLED`. Globalny manifest pozostaje `NOT_APPROVED_FOR_START`; rozliczenie czterech nieobecnych tożsamości, Qdrant backing, `OPEN_AFTER_BASE_READY`, relokacja, task install, live startup/reboot/rollback, backup schedule proof i P4-B/P5 są otwarte. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_AND_OFFLINE_TESTS_READY_FOR_REVIEW / CURRENT_IDENTITY_READBACK_2_OF_6_PARTIAL / NOT_INSTALLED`; backend/PostgreSQL potwierdzone, cztery przypięte pary container/image nieobecne i `BLOCKING_P4B`. Globalny manifest `NOT_APPROVED_FOR_START`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260918T000227Z-D21-P4A-IDENTITY-READBACK-PARTIAL` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-18T00:02:27Z` |
| Aktualny wykonawca / sesja | Codex / P4-A full-ID i PostgreSQL-health source/offline zamknięte; readback utrwalony 2/6, cztery przypięte zasoby nieobecne; zero instalacji i zero mutacji runtime; STOP przed P4-B/P5 |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-A identity and cold-start continuation` — source/offline `READY_FOR_REVIEW`, identity readback `2/6 PARTIAL`, `NOT_INSTALLED`; cały R04 nadal `IN_PROGRESS` |
...
| Źródła runtime / release / DB | Backend `686ac376...c854`, image `sha256:6342b36f...d63702`, source `0ee0ea5...`, `/app=C:/ai-lab-core/backend:ro`, `/data=C:/ai-lab-core/data:rw`; pozostałe pięć kontenerów zachowane. P4-A draft wiąże bieżące IDs/images/mounts, lecz RepoDigests i Qdrant physical backing są `UNKNOWN`. Public Gateway działał; Supervisor `INTENTIONALLY_STOPPED` |
| Ostatnia faktycznie zakończona czynność | Przygotowano LOCAL_ONLY trzyplikowy payload, rollback XML pięciu tasków, kopie wrappera/skrótu, disabled host-task XML, draft manifestu i changeset. Niczego nie zainstalowano ani nie uruchomiono |
| Potwierdzone testy bieżącego wykonania | Dziedziczony P3 readback: `11/11`, `9/9 false`, ta sama instancja. P4-A pass-after Windows PowerShell 5.1: P1 plan `53`, real adapters `51`, DATA_ONLY `44`, P4 package `14` asercji. Nowy collector: parser PASS i `20/20`; bezpieczny readback `2/6 MATCH`, cztery przypięte pary nieobecne. Live launcher/start/logon/reboot/rollback, Flutter/Web/backend tests `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Chroniony root P3 pozostaje. P4-A LOCAL_ONLY root `...\p4-startup-activation-20260917T210051Z` zachowuje historyczny indeks 16 plików `DC76525...ACB72`; efektywny indeks kontynuacji ma 18 plików, 4 222 B i SHA-256 `78966E33376D4A6E118B79AFE2A844403AFEF1B0E56BB8A972B438FB55428482`. Nie jest instalacją |
| Niezakończone procesy i skutki operacyjne | Istniejący backend i Public Gateway pozostawiono bez zmian. Supervisor `INTENTIONALLY_STOPPED`. Taski, Startup wrapper, shortcut, manifest, backupy, mounty, junction i dane niezmienione |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260918T000227Z-R04-D21-P4A-IDENTITY-READBACK-PARTIAL.md` |
| Zakres aktualnej zgody | P4-A source/offline: pełny container-ID binding, image identity modes, PostgreSQL readiness, nieaktywne drafty/payload i dokumentacja. Bez instalacji, importu taska, manifest approval lub startu |
| Blokada / wymagana decyzja | Rozliczenie aktualnych pełnych ID/image identity czterech nieobecnych przypiętych zasobów bez automatycznej adopcji; ponadto Qdrant backing, `OPEN_AFTER_BASE_READY`, global manifest approval, task/trigger install, P4-B operational matrix, backup schedule proof i relokacje D: pozostają otwarte |
| Jeden następny bezpieczny krok | Osobna decyzja właściciela na ograniczone rozliczenie bieżących pełnych ID/image identity Qdrant, n8n, Open WebUI i Ollama; bez startu, tworzenia lub automatycznej adopcji po nazwie |
| Warunek STOP | Bez instalacji plików/XML, task/Startup/shortcut changes, manifest approval, launcher/service start, restart/reboot, Supervisor, backup/restore, relocation, P4-B/P5/R06 |
...
| R04 | IN_PROGRESS | `R04-20260918T000227Z-D21-P4A-IDENTITY-READBACK-PARTIAL` / P3 ACCEPTED LIMITED + P4-A SOURCE/OFFLINE READY, IDENTITY 2/6 PARTIAL | P3 backend switch jest `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`; P4-A full-ID/image/file-role/DB-health source i testy offline są `READY_FOR_REVIEW / NOT_INSTALLED`. Bieżący readback potwierdza backend i PostgreSQL 2/6 wraz z RepoDigests; cztery przypięte container/image ID są nieobecne, więc draft nie został zmieniony, P4-B jest zablokowane, a globalny manifest pozostaje `NOT_APPROVED`. Qdrant backing, `OPEN_AFTER_BASE_READY`, task install, live startup/reboot/rollback, backup schedule proof, relokacja i P4-B/P5 pozostają otwarte. Supervisor `INTENTIONALLY_STOPPED` |
