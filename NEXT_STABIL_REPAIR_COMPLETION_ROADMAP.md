...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION. R04 i R05 pozostają IN_PROGRESS. R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 source `2e69622bc6a0b4888427f8ae5be119377aed26d9` i evidence `7687cc15bb31175d19485b10a6e13dfc5945bfc2` są `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`. P2 zachowało unikalną pracę i wybrało ten sam pełny source set jako `PRESERVATION_AND_CANDIDATE_READY_FOR_REVIEW / NOT_DEPLOYED`; jego manifest pozostaje `DRAFT / NOT_APPROVED_FOR_START`. Cutover, task install, live startup, reboot, relokacja, cleanup i P3–P5 są `NOT_RUN / NOT_AUTHORIZED`. Supervisor pozostaje `INTENTIONALLY_STOPPED`. R06–R24 pozostają nieuruchomione. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R05-A4 pozostaje `SOURCE_AND_API_WIDGET_TESTS_ACCEPTED / NOT_DEPLOYED`. D-21 P1 jest `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`; P2 jest `PRESERVATION_AND_CANDIDATE_READY_FOR_REVIEW / NOT_DEPLOYED`, a draft manifestu `NOT_APPROVED_FOR_START`. Cały R04 i R05 pozostają `IN_PROGRESS`, R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260915T220132Z-D21-P2-CANDIDATE` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-15T22:01:32Z` |
| Aktualny wykonawca / sesja | Codex / R04 D-21 P2 preservation and one pinned candidate; bez drugiego autora |
| Aktywny pakiet / podetap | `R04 / D-21 / P2` — `PRESERVATION_AND_CANDIDATE_READY_FOR_REVIEW / NOT_DEPLOYED`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking/remote `7687cc15bb31175d19485b10a6e13dfc5945bfc2`, clean i staged `0`; source set `2e69622bc6a0b4888427f8ae5be119377aed26d9`. Oryginalny preservation ponownie potwierdzony `203/203`, mismatch `0`, staged `0`; original i audit niezmienione |
| Gałąź / SHA kodu objętego sprawdzeniem | P2 source set i accepted P1 source `2e69622bc6a0b4888427f8ae5be119377aed26d9`; accepted P1 evidence `7687cc15bb31175d19485b10a6e13dfc5945bfc2`; A4 accepted source `04ab5e58cf86896ffd946cabffde13367d343f53`; baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a`; audit preserved `c2e931c8b72a55f4260b546e9eba84d67040e1f4` |
| Baseline commit dokumentacji | `483f9bf8b1a591ded8a42df5da87663c664ed5d4` |
| Źródła runtime / release / DB | Odczyt `2026-09-15T21:43:27Z`: backend nadal ma pełne ID `9d9b46c530412e48562b5427a0586b08c1e919ff293ec2cbd1489faffc98615a`, image `sha256:6342b36fa2cdd2501ea4e0e9fada9a9ffaa4894f0c512f19f822f009e8d63702`, live `/app=C:/ai-lab-core/build/deploy-main-483f9bf8/backend` i `/data=C:/ai-lab-core/data`; recovery/P2 nie jest zamontowane. Kandydat nie jest załadowany. Live Web pozostaje historycznie 1.0.2+41; nowy Web P2 jest wyłącznie TEST_ONLY. Supervisor pozostaje nieuruchomiony |
| Ostatnia faktycznie zakończona czynność | Wybrano jeden source set `2e69622bc6a0b4888427f8ae5be119377aed26d9`; zapisano inertny snapshot i 1,198-file roundtrip, preservation R00/audit/external worker, jeden Web TEST_ONLY oraz draft manifestu odrzucany przez walidator P1 |
| Potwierdzone testy bieżącego wykonania | 1,198/1,198 snapshot roundtrip, mismatch 0; Git bundle verify/fetch exact HEAD PASS; dirty patch reverse-apply PASS; 7/7 external worker byte copies PASS; Web build exit 0 z pełnym stdout/stderr; draft manifest Windows PowerShell `5.1.26100.8894`: `valid=false`, `START_NOT_APPROVED`, adapters 0. P1 53/48 i A4 testy są odziedziczonym dowodem niezmienionych blobów, nie nowym runem. Aplikacja/UI/browser/modele: `NOT_RUN` |
| Niezacommitowana praca / zabezpieczenie | Oryginalny worktree: 6 modified + 197 untracked, staged 0; `203/203`, mismatch 0. Audit: bundle + dirty patch/exact bytes zachowane, worktree niezmieniony. P2 local root zawiera snapshot, build i surowe dowody LOCAL_ONLY; repo ma wyłącznie zanonimizowane indeksy/dokumentację |
| Niezakończone procesy i skutki operacyjne | Supervisor pozostaje `INTENTIONALLY_STOPPED / NOT_STARTED_BY_THIS_SESSION`. Nie wykonano start/stop/restart ani zmian kontenerów, tasków, shortcutów, mountów, flag, kolejek, spoolu, harmonogramów, danych lub runtime. Rzeczywiste skutki to lokalne kopie/archiwa, katalogi walidacyjne, cache/logi i jeden build Web TEST_ONLY |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260915T220132Z-R04-D21-P2-CANDIDATE.md`; poprzedni handoff: `docs/recovery/checkpoints/20260915T201451Z-R04-D21-P1-HOST-COEXISTENCE.md` |
| Zakres aktualnej zgody | `D21-P2 preservation/candidate` wyłącznie w jednym chronionym stagingu i dokumentacji recovery; bez instalacji, aktywacji, manifestu produkcyjnego lub P3–P5 |
| Blokada / wymagana decyzja | P2 wymaga review właściciela. Produkcyjny manifest nie jest zatwierdzony; init_database i backup reconciler wymagają kontroli skutków; ID/image/mounty, schema, efektywne flagi, external worker/profile, rollback danych i okno P3 pozostają nieodebrane. `C:\ai-lab-core\data` jest junctionem do `D:\ai-lab-data` i wymaga jawnego rozstrzygnięcia bez ukrywania zależności poza rootem. P3–P5 są niezatwierdzone |
| Jeden następny bezpieczny krok | Właściciel przegląda P2: preservation, source set, Web TEST_ONLY, draft manifestu i listę bramek; ewentualne P3 wymaga osobnej zgody operacyjnej |
...
| R04 | IN_PROGRESS | `R04-20260915T220132Z-D21-P2-CANDIDATE` / P2 PRESERVATION AND CANDIDATE READY FOR REVIEW | A3 pozostaje `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. D-21 P1 source/evidence są `SOURCE_AND_OFFLINE_TESTS_ACCEPTED / NOT_DEPLOYED`. P2 zachowało original/audit/external-worker source i wybrało source set `2e69622bc6a0b4888427f8ae5be119377aed26d9` jako `PRESERVATION_AND_CANDIDATE_READY_FOR_REVIEW / NOT_DEPLOYED`; draft manifestu jest `NOT_APPROVED_FOR_START`. P3–P5, cutover, task install, live startup/reboot/rollback, relokacja, cleanup i pełne runtime acceptance są `NOT_RUN` |
...
3. Historyczna reguła przed D-21 dopuszczała odrębne odtwarzalne deployment roots. D-21 zastępuje ją dla stanu docelowego: `SINGLE_INSTALL_ROOT=C:\ai-lab-core`, jawne wersjonowane podkatalogi komponentów i jeden zwykły, idempotentny punkt startu. Read-only inventory oraz mapa stare→docelowe są w `R04_SINGLE_ROOT_STARTUP_MAP.csv` i `R04_SINGLE_ROOT_STARTUP_PLAN.md`. P1 source/offline tests są zaakceptowane jako NOT_DEPLOYED. P2 zachowało unikalną pracę, wybrało pełny source set `2e69622...`, zbudowało jeden Web TEST_ONLY i przygotowało draft `NOT_APPROVED_FOR_START`; czeka na review. Produkcyjny manifest, P3–P5, instalacja, cutover, relokacja, test live startup/rollback i cleanup wymagają osobnych zgód. Produkcja nie może wykonywać recovery/staging/test WIP.
