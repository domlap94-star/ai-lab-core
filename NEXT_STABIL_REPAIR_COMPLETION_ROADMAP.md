...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B pozostaje `PARTIAL_AFTER_FAILURE / HOST_TASK_FAILED_22 / SAFE_INACTIVE_PARTIAL_UNKNOWN`; Host22 source jest `SOURCE_AND_OFFLINE_PACKAGE_ACCEPTED / NOT_DEPLOYED`, a skonsolidowany NUP-01/02/03 `READY_FOR_REVIEW / OFFLINE_ONLY / NOT_INSTALLED`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260920T204019Z-D21-P4B-NUP-SOURCE` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-20T20:40:19Z` — source/offline NUP-01/02/03 i paczka review, bez zmiany runtime |
...
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — zaakceptowany Host22 source pozostaje NOT_DEPLOYED; skonsolidowany NUP-01/02/03 jest gotowy do drugiego, niezależnego review D-23; installed run01 pozostaje częściowy, retry/update nieautoryzowane |
...
| Ostatnia faktycznie zakończona czynność | Odtworzono fail-before NUP-01/02/03, poprawiono recorder i LOCAL_ONLY receptę, wykonano końcową kampanię `51/16`, zbudowano i zweryfikowano roundtrip paczki review; stan Host22/run01/D-22 nie został zmieniony |
| Potwierdzone testy bieżącego wykonania | Recorder: `32` asercje i `8` własnych child cases. Skonsolidowana orkiestracja: `51` asercji / `16` scenariuszy, Host `2`, Private `1 -> 0`, Supervisor/container/5 task writes `0`, własne procesy `1`, unsettled `0`; parser/JSON/CSV/reference/secret/diff PASS; granice produkcji `0` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY `C:\Users\domai\AppData\Local\Temp\P4B-NUP-01`; package index 6 876 B SHA-256 `67B32FB8981F765A251DDC081D4FD9473B224A2BFFEE7E96CA01C1C57A288222`; review ZIP 104 965 B SHA-256 `C34B9460E1E4528932F722D93BF1E57E2F1906CE8F3179E0834CACA633957A1F`, roundtrip `28/28`; wcześniejsze pakiety i run01 zachowane |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260920T204019Z-R04-D21-P4B-NUP-SOURCE.md` |
| Zakres aktualnej zgody | SOURCE / LOCAL FILE READ / OFFLINE TEST ONLY dla NUP-01/02/03 oraz dokumentacja. Brak zgody na update zainstalowanych plików/manifestu, retry Host/Install, UAC, task writes, rollback, logon/reboot, P5 lub R06. D-22 pozostaje `NOT_RUN` |
| Blokada / wymagana decyzja | Drugie, niezależne review D-23 exact source `d3435afcfb89d02d91f2db3d1eb55be17fd790bd`, recepty `0B051C2F...B9EF`, indexu `67B32FB8...8222` i ZIP `C34B9460...7A1F`. Dopiero po tym właściciel może osobno rozważyć exact jednorazową zgodę operacyjną |
| Jeden następny bezpieczny krok | Niezależny review 2/2 wyłącznie NUP-01/02/03 i regresji tej poprawki; bez dodawania K2/K3, UAC, Host retry, instalacji lub rollbacku |
...
|---|---|
| Bieżący krok P4/B | Host22 source/offline package jest odebrany. Pierwszy zbiorczy review D-23 wskazał wyłącznie NUP-01/02/03 (`K0/K0/K1`); skonsolidowany diff jest `READY_FOR_REVIEW / OFFLINE_ONLY / NOT_INSTALLED`, a cykl pozostaje `1/2`. Następny review weryfikuje ten diff i jego regresje, bez dodawania K2/K3. Dopiero po review 2/2 właściciel może osobno rozważyć wąski update i dwa kontrolowane warm runs. Brak operacyjnej zgody jest stanem `WAITING_APPROVAL`, nie wadą `K0/K1` |
