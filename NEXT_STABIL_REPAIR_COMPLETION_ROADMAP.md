...
| Stan rejestracji | P3 `CORE_BACKEND_SOURCE_SWITCH_ACCEPTED / LIMITED_RUNTIME_SCOPE`. P4-A `SOURCE_OFFLINE_AND_IDENTITY_PACKAGE_ACCEPTED / NOT_INSTALLED`. P4/B pozostaje `PARTIAL_AFTER_FAILURE / HOST_TASK_FAILED_22 / SAFE_INACTIVE_PARTIAL_UNKNOWN`; nowa poprawka `HOST22_OBSERVATION_COMPLETENESS_DEADLINE_AND_STATE_SOURCE_READY_FOR_REVIEW / OFFLINE_TESTS_PASS / NOT_DEPLOYED`; cały R04/R05 `IN_PROGRESS`; R03 `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Checkpoint ID | `R04-20260920T144559Z-D21-P4B-HOST22-OBS` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-20T14:45:59Z` |
| Aktualny wykonawca / sesja | Codex / source i testy offline bez Docker/Task/CIM/TCP/HTTP/UAC/Host; końcowe PS 5.1 `51/57/51/44/40`, wszystkie exit `0`, stderr `0`, granice produkcji `0` |
| Aktywny pakiet / podetap | `R04 / D-21 / P4-B` — Host22 OBS-01–03 source gotowe do niezależnego review; installed run01 pozostaje częściowy, retry/update nieautoryzowane |
...
| Ostatnia faktycznie zakończona czynność | Opublikowano source commit `b4269ffa7bacc95b4d1441bb196e572a34a4ec43`: fail-closed kompletność koperty natywnej, jeden malejący deadline obserwacji/readiness i exact `state_status=running` dla pinned gotowości. Przygotowano nieaktywny candidate i ZIP review; instalacji nie zmieniono |
| Potwierdzone testy bieżącego wykonania | Preimage `ed961d...`: OBS-01/02/03 `REPRODUCED`. Pass-after PS 5.1: focused `51`, plan `57`, real adapters `51`, DATA_ONLY `44`, P4 package `40`; exit `0`, stderr `0`. Topologia 6+4, Private START_ONCE/repeat, PostgreSQL `starting -> healthy` przed backendem, truncated/missing envelope, deadline i paused/restarting pinned state przeszły; produkcyjne granice `0` |
| Niezacommitowana praca / zabezpieczenie | LOCAL_ONLY `C:\Users\domai\AppData\Local\Temp\P4B-H22-OBS-01`; docelowy istniejący staging odmówił zapisu bez elewacji, której nie użyto. Package index SHA-256 `CE373406DA49B35B01B20F4E8039A5F6F0060C54237C1B2D91193867B83B5763`; ZIP SHA-256 `4EB0706A365C2D46AD7F63047AE2DB253D1841C7E372A45F4F1947CA212ADBD9`, roundtrip `12/12` + index; wcześniejsze `P4B-H22-SRC-01`, `P4B-E22-01` i run01 zachowane |
...
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260920T144559Z-R04-D21-P4B-HOST22-OBS.md` |
...
| Blokada / wymagana decyzja | Niezależny review source `b4269ffa7bacc95b4d1441bb196e572a34a4ec43` i ZIP `4EB0706A...12ADBD9`, następnie odrębna jednorazowa zgoda na wąski update. Installed run01 nadal używa starych hashy i Host jest disabled/no-trigger |
| Jeden następny bezpieczny krok | Review opublikowanego source/test i exact LOCAL_ONLY ZIP; bez UAC, Host retry lub instalacji. Ewentualny późniejszy Host musi trwale zapisać JSON code/events/details, stdout/stderr i exit |
...
| R04 | IN_PROGRESS | `R04-20260920T144559Z-D21-P4B-HOST22-OBS` / P4-B PARTIAL + HOST22 OBS SOURCE READY | P3/P4-A zachowują odbiory. Exact run01 pozostaje `PARTIAL_AFTER_FAILURE`; Host disabled/no-trigger, warm `0/2`, Private start `0`, Supervisor `INTENTIONALLY_STOPPED`. Source `b4269ffa7bacc95b4d1441bb196e572a34a4ec43` zachowuje Health/exact-ID i domyka fail-closed kompletność koperty, jeden deadline etapu oraz exact pinned running state; offline `51/57/51/44/40` PASS, produkcyjne granice `0`. `NOT_DEPLOYED / NO_RETRY_AUTHORIZED`; Qdrant/VHD/profile, UI, backup proof, live logon/reboot i P4-B/P5 pozostają otwarte |
