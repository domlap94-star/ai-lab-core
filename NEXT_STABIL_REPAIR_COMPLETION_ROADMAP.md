...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A3 pozostaje zaakceptowane jako `SOURCE_INTEGRATION_ACCEPTED / TEST_STAGING_RECORDED / NOT_DEPLOYED`. R05-A1/A2/A3 zachowują wcześniejsze wąskie odbiory `NOT_DEPLOYED`. R05-A4 source `f3cbe58bed0de34cfd39a57d481abb2c61c2f4ac` jest nieodebrane. Engine odzyskał obserwowalność po zgłoszonej przez właściciela aktualizacji Docker, lecz produkcja wróciła tylko częściowo; A4 pozostaje `SOURCE_PARTIAL / LOCAL_ONLY / PRODUCTION_PARTIAL_UNHEALTHY / NOT_DEPLOYED`, cały R05 `IN_PROGRESS`, R06–R24 nieuruchomione |
| Checkpoint ID | `R05-20260914T144558Z-A4-DOCKER-UPDATE-PARTIAL-RECOVERY` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-14T14:45:58Z` |
| Aktualny wykonawca / sesja | Codex + właściciel/operator / R05-A4: jeden zatwierdzony restart wykonany; późniejsza aktualizacja Docker zgłoszona przez właściciela; Engine odczytywalny, usługi częściowo niesprawne, testy zatrzymane |
| Aktywny pakiet / podetap | `R05 / A4 preview transport and decision result` — `SOURCE_PARTIAL / LOCAL_ONLY / ENGINE_RECOVERED / PRODUCTION_PARTIAL_UNHEALTHY / NOT_DEPLOYED`; cały R05 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; start local/tracking `4df09305b870626795036a3ea370929f03e92ce5`. Cztery source/test paths nadal dokładnie odpowiadają patchowi SHA-256 `81E311E7C129FC7ACC8829BC8B2E3FB31CDFEA6F800D56EE46E16E9E58A17914` i pozostają `LOCAL_ONLY` |
| Gałąź / SHA kodu objętego sprawdzeniem | Opublikowany A4 source/test pod review `f3cbe58bed0de34cfd39a57d481abb2c61c2f4ac`; dokumentacyjny parent bieżącego wznowienia `4df09305b870626795036a3ea370929f03e92ce5`; accepted R05-A2 product source `d1b0518ad9aefb5bc05cd89308de923ca54d2809`; accepted R05-A1 source `d31e105427acd40733e91c4d4b46f0412b0f95ad`; baseline `origin/main@483f9bf8b1a591ded8a42df5da87663c664ed5d4`; rescue `5cd8f86e63e1ab829692ca2601096fd0c0d9d53a` |
| Baseline commit dokumentacji | `483f9bf8b1a591ded8a42df5da87663c664ed5d4` |
| Źródła runtime / release / DB | Bieżący read-only runtime po aktualizacji: pełny backend ID `9d9b46c530412e48562b5427a0586b08c1e919ff293ec2cbd1489faffc98615a`, image ID `sha256:6342b36fa2cdd2501ea4e0e9fada9a9ffaa4894f0c512f19f822f009e8d63702`, `/app=C:/ai-lab-core/build/deploy-main-483f9bf8/backend`, `/data=C:/ai-lab-core/data`, started `2026-09-14T14:41:11Z`. Brak mountu recovery/WIP potwierdzony. Compose `C:\ai-lab-core\backend` nie jest używany jako dowód. R04-A3 pozostaje `NOT_DEPLOYED`; zachowane zestawy R03/A2 nie były użyte |
| Ostatnia faktycznie zakończona czynność | Engine Docker Desktop `4.91.0` / `29.8.0` został odczytany przez formalnie zatwierdzoną ścieżkę. Potwierdzono pełne ID/mounty/stany działających kontenerów, brak starego `next-stabil-r05-a4-20260912t121418z-regression-final` oraz dostępność obrazu R02. Health: backend/n8n/Open WebUI `200`; Supervisor `8787` i public gateway `8789` nieosiągalne. Zatrzymano pracę przed pomiarem bramek i testem |
| Potwierdzone testy bieżącego wykonania | Bez nowych testów produktu. Wcześniejsze, zweryfikowane dla niezmienionego WIP: Flutter fail-before `6 pass / 4 fail`, focused `11 passed`, Documents/Auth `28 passed`, analyze PASS. Backend ASGI/CORS, API regression i compileall: `NOT_RUN / PRODUCTION_PARTIAL_UNHEALTHY`; historyczne `78/319` nie są używane jako bieżący wynik |
...
| Niezakończone procesy i skutki operacyjne | Wykonano dokładnie jeden zatwierdzony Restart Docker Desktop; zgłoszona późniejsza aktualizacja Docker była czynnością właściciela, nie Codexa. Drugi restart, `wsl --shutdown`, kill/reset/prune, compose/recreate i zmiany mountów/config: `NOT_RUN`. Sześć AI-Lab containers i obcy `postgres:10` działają od około `14:41Z`; Supervisor/public gateway pozostają niedostępne. Nie utworzono nowego kontenera testowego |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260914T144558Z-R05-A4-DOCKER-UPDATE-PARTIAL-RECOVERY.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260912T152351Z-R05-A4-BACKEND-RESUME.md` |
| Zakres aktualnej zgody | Właściciel dopuścił jeden standardowy Restart Docker Desktop mimo `CURRENT_BACKEND_MOUNT=UNKNOWN`, bez resetu danych, drugiego restartu, WSL/Windows restartu, compose/recreate lub zmian konfiguracji. Restart wykorzystano. Późniejsza aktualizacja Docker nie była wykonywana przez Codexa. Source A4 nadal obejmuje wyłącznie cztery wskazane pliki i izolowane testy po pełnym odzyskaniu usług |
| Blokada / wymagana decyzja | `PRODUCTION_PARTIAL_UNHEALTHY`: Engine i mounty są odczytywalne, lecz Supervisor `127.0.0.1:8787` oraz public gateway `127.0.0.1:8789` nie odpowiadają. Zgodnie z procedurą nie wolno przechodzić do CORS tests, dopóki wcześniej działające usługi nie zostaną rozliczone. Bieżące kolejki/zadania nadal `UNKNOWN`; pomiar RAM/commit/WSL po aktualizacji nie został zaliczony, bo STOP nastąpił wcześniej |
| Jeden następny bezpieczny krok | Właściciel/operator zatwierdza osobno minimalne przywrócenie dokładnych usług Supervisor i public gateway albo potwierdza ich zamierzone wyłączenie. Dopiero po ich health, nowym bounded resource gate i braku nieoczekiwanych skutków wolno uruchomić zatwierdzony kontener backend tests; bez R06 |
...
| R05 | IN_PROGRESS | `R05-20260914T144558Z-A4-DOCKER-UPDATE-PARTIAL-RECOVERY` / A4 SOURCE PARTIAL | Właściciel zaakceptował A1/A2/A3 w ich wąskich zakresach `NOT_DEPLOYED`; A4 pozostaje nieodebrane. Po jednym zatwierdzonym restarcie i zgłoszonej przez właściciela aktualizacji Docker Engine `29.8.0` odpowiada. Runtime potwierdził backend `/app=C:/ai-lab-core/build/deploy-main-483f9bf8/backend`, brak mountu recovery/WIP i nieobecność starego kontenera A4. Supervisor i public gateway pozostają nieosiągalne; backend ASGI/CORS/API/compileall `NOT_RUN / PRODUCTION_PARTIAL_UNHEALTHY`, source nadal `LOCAL_ONLY / NOT_DEPLOYED`. Operator Web runtime, Temporary Chat mode, remote upload i external end-to-end pozostają `NOT_VERIFIED`; cały R05 nadal `IN_PROGRESS` |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
| R09 | PLANNED | — | — |
