...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION; R04 i R05 pozostają IN_PROGRESS. D-21 P1/P2/DATA_ONLY/guard, ograniczony P3, P4/A, Host22 i NUP-01/02/03 zachowują odbiory; D-23 review pozostaje 2/2. `USABLE_WARM_ACCEPTED / LIMITED_RUNTIME_SCOPE` pozostaje w mocy. OP `R04-D21-ONE-ENTRY-COLD-COMPLETE-20260925T183020Z` atomowo zainstalował i zwalidował approved manifest V2 `53CA6E98...B6972EC9`; same-directory backup exact preimage usunięto dopiero po PASS. Jeden świeży repeat przy zastanym jednym kliencie zakończył się nowym kompletnym attemptem `20260926T101258980Z-f60e3fd8`: `BASE_READY_LIMITED_CAPTURED`, launcher/recorder exit `0`, settled, complete streams, stderr empty, base ready, Supervisor `INTENTIONALLY_STOPPED`, zero nowych startów. Finalnie Host `Ready`/result `0`, dokładnie jeden klient PID `11924`, foreign `0`, aktywne backup/import/maintenance `0`, pending mutation false. Stan to `MANIFEST_ATOMIC_INSTALL_PASS / REPEAT_PASS / RECORDER_COMPLETE / READY_FOR_OWNER_RESTART_CONFIRMATION`; restart jeszcze niezatwierdzony. Docker/WSL pool i swap pozostają UNKNOWN. D-22 pozostaje `NOT_RUN`. Bieżący stan znajduje się wyłącznie w §0.**
...
| Checkpoint ID | `20260926T101446Z-R04-D21-ONE-ENTRY-COLD-READY-FOR-RESTART` |
...
| Aktualny wykonawca / sesja | Codex / ten sam ONE-ENTRY-COLD OP; manifest/repeat complete, paused only at owner restart confirmation |
| Aktywny pakiet / podetap | R04/D-21 — `MANIFEST_ATOMIC_INSTALL_PASS / REPEAT_PASS / RECORDER_COMPLETE / READY_FOR_OWNER_RESTART_CONFIRMATION`; USABLE-WARM zachowuje odbiór |
...
| Ostatnia faktycznie zakończona czynność | UAC PID `26228` atomowo zainstalował/zwalidował manifest V2; jeden repeat utworzył complete settled attempt `20260926T101258980Z-f60e3fd8`, exit `0`, base ready, zero nowych startów |
...
| Niezakończone procesy i skutki operacyjne | PID `26228` i launcher child rozliczone; manifest pending false; Host Ready/result 0; dokładnie jeden klient; backup/import/maintenance 0. Reboot/logon/cold `NOT_RUN`; resume point zachowany |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260926T101446Z-R04-D21-ONE-ENTRY-COLD-READY-FOR-RESTART.md` |
| Zakres aktualnej zgody | Manifest installation i jeden repeat zakończone. Restart nie jest jeszcze zatwierdzony; brak zgody na dalsze mutacje przed bieżącym potwierdzeniem właściciela |
| Blokada / wymagana decyzja | Wyłącznie bieżące potwierdzenie właściciela gotowości do dokładnie jednego restartu Windows w tym samym OP |
| Jeden następny bezpieczny krok | Po exact bieżącym potwierdzeniu wykonać jeden restart Windows i kontynuować ten sam OP bez ręcznego startu przed oceną logon/cold |
