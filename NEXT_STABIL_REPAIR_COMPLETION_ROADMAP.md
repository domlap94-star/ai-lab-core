...
**Status rejestracji: R00–R02 ACCEPTED; R03 pozostaje WAITING_APPROVAL / WAITING_ESCROW_DECISION jako cały pakiet. Właściciel zaakceptował R03-A4 na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`. R04-A1 jest zaakceptowane na `f4ea20c74f92c0423db087ba8d60bb8cc7f2ec99` jako source/test i pozostaje `NOT_DEPLOYED`. Source/test `R04-A2-UI06` na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` pozostaje zaakceptowane wyłącznie dla `DocumentsController`. B ma funkcjonalny `PASS / OWNER_OPERATED_RUSTDESK`; A pozostaje `NOT_VERIFIED / PREFETCH_CONTAMINATED`. W powtórzeniu zleconym przez właściciela świeże przeładowanie przy wyłączonym backendzie zatrzymało się na wcześniejszym `auth/me`, a po przywróceniu sesji Dashboard sam pobrał `/api/v1/documents?...limit=6` przed wejściem operatora do repozytorium. Nie wykonano kolejnego outage ani prób do skutku. Całość pozostaje `WEB_AB_PARTIAL / TEST_ONLY`. Android runtime jest `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 nadal `WAITING_OWNER_VISUAL_EVIDENCE`. Cały R04 pozostaje IN_PROGRESS. Bieżący stan znajduje się wyłącznie w §0.**
...
| Stan rejestracji | R03-A4 pozostaje zaakceptowane na `92cc3aa38f0a81ff40e3798981970bd2e45b6206` dla manifestu `8F20A7845473097EE74019966583EC3F121139C4B562265F9A7391FAFAF6BE4B`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION`. R04-A1 source/test jest zaakceptowane na `f4ea20c74f92c0423db087ba8d60bb8cc7f2ec99` jako `NOT_DEPLOYED`. Source/test `R04-A2-UI06` na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` pozostaje zaakceptowane wyłącznie dla `DocumentsController`. B `PASS / OWNER_OPERATED_RUSTDESK`; powtórzone A `NOT_VERIFIED / PREFETCH_CONTAMINATED`: najpierw zadziałał auth gate, a następnie Dashboard wykonał odczyt listy z limitem 6 przed wejściem operatora do repozytorium. Całość `WEB_AB_PARTIAL / TEST_ONLY`. `ClientsController`, całe A2/R04/R16 pozostają nieodebrane. Android `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 `WAITING_OWNER_VISUAL_EVIDENCE` |
| Checkpoint ID | `R04-20260910T175215Z-A2-UI06-A-REPEAT` |
| Ostatnia aktualizacja operacyjna UTC | `2026-09-10T17:52:15Z` |
| Aktualny wykonawca / sesja | Codex + właściciel przez istniejący RustDesk / zakończone powtórzenie R04-A2 UI06 A |
| Aktywny pakiet / podetap | `R04 / A2 UI06` — `SOURCE_ACCEPTED / WEB_AB_PARTIAL`; A `NOT_VERIFIED / PREFETCH_CONTAMINATED`, B `PASS`, source/test `NOT_DEPLOYED`; cały R04 nadal `IN_PROGRESS` |
| Potwierdzony lokalny worktree | `C:\ai-lab-core-recovery`, branch `recovery/next-stabil-repair-completion`; clean local/remote start powtórzenia `25c30e7dd9e3d451772eee812bdef190665e1b8d`; accepted source commit `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` |
...
| Ostatnia faktycznie zakończona czynność | Na polecenie właściciela `powtorz A` wykonano jedną nową próbę A. Po zalogowaniu i pełnym przeładowaniu przy zatrzymanym backendzie aplikacja pokazała wcześniejszy błąd sprawdzenia sesji; zachowano screenshot. Po uruchomieniu tego samego backendu operator potwierdził Dashboard, lecz log wykazał `GET /api/v1/documents?link_state=ALL&skip=0&limit=6` przed wejściem do repozytorium. Ścisły warunek braku wcześniejszego pobrania listy nie przeszedł, więc nie wykonano następnego outage ani kliknięcia Dokumenty. Stack/Web zatrzymane, telemetria usunięta, porty wolne |
| Potwierdzone testy bieżącego wykonania | Testy aplikacyjne: `NOT_RUN` — source/test nie zmieniono. Powtórzenie A: `NOT_VERIFIED / AUTH_GATE_THEN_DASHBOARD_PREFETCH`; screenshot auth gate i log żądania limit=6 są `LOCAL_ONLY`. Monitor: `68/68` próbek PASS; minimum Windows `4.539 GiB`, commit reserve `33.645 GiB`, pool `13.808 GiB`, swap maksymalnie `1.9 MiB`. Stan historycznego B pozostaje `PASS / OWNER_OPERATED_RUSTDESK`; nie wykonywano go ponownie |
...
| Niezakończone procesy i skutki operacyjne | Trzy exact-name kontenery A2 są `exited`; proces Flutter Web zatrzymany; telemetry `b40aae3a...` usunięta po kontroli ID/owner/run; porty `18004/18005` mają 0 listenerów. Syntetyczna DB/volume/storage, cache, APK i dowody pod `...\owner-ui06-a-repeat-20260910T173258Z` pozostają `LOCAL_ONLY`; produkcja i zasoby R03 nietknięte. Testowe okno InPrivate zamyka właściciel |
| Najnowsza notatka przekazania | `docs/recovery/checkpoints/20260910T175215Z-R04-A2-UI06-A-REPEAT.md`; poprzedni checkpoint: `docs/recovery/checkpoints/20260910T152711Z-R04-A2-UI06-OWNER-HANDOFF.md` |
| Zakres aktualnej zgody | Wiadomość właściciela `powtorz A` wykorzystano wyłącznie do jednej dodatkowej próby A na tym samym zaakceptowanym `DocumentsController` i zachowanym zestawie. Nie użyto automatyzacji browsera i nie zmieniono produktu/testów/security tools. Zgoda nie rozszerzyła się na kolejne próby, selektywny proxy/interceptor, `ClientsController`, Android, produkcję, modele, escrow, rescue adoption, pozostałe R16 ani R05–R24 |
| Blokada / wymagana decyzja | A pozostaje `NOT_VERIFIED`: pełny reload przy outage nie przechodzi auth gate, a poprawnie odtworzona sesja na Dashboardzie wykonuje listę dokumentów z limitem 6 przed wejściem operatora do repozytorium. Ścisłego braku wcześniejszego odczytu nie da się uzyskać tym samym ręcznym przebiegiem bez deterministycznej selektywnej izolacji lub zmiany harnessu. W-02 nadal osobne `WAITING_OWNER_VISUAL_EVIDENCE`; cały R03 nadal `WAITING_APPROVAL / WAITING_ESCROW_DECISION` |
| Jeden następny bezpieczny krok | Właściciel i ChatGPT oceniają zachowany wynik B oraz dwukrotnie nieweryfikowalne A; ewentualny selektywny test blokujący tylko endpoint dokumentów wymaga osobnej, dokładnej zgody i nie może zostać uruchomiony automatycznie |
...
| R04 | IN_PROGRESS | `R04-20260910T175215Z-A2-UI06-A-REPEAT` / A2 UI06 WEB_AB_PARTIAL | A1 source/test zaakceptowane na `f4ea20c74f92c0423db087ba8d60bb8cc7f2ec99` jako `NOT_DEPLOYED`. UI06 source/test zaakceptowane na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` wyłącznie dla `DocumentsController`; ręczny Web B `PASS`. Powtórzenie A nadal `NOT_VERIFIED / AUTH_GATE_THEN_DASHBOARD_PREFETCH`; całość `WEB_AB_PARTIAL / TEST_ONLY`. `ClientsController` nieodebrane; Android `DEFERRED_BY_OWNER / NOT_TESTED`; W-02 `WAITING_OWNER_VISUAL_EVIDENCE`. Modele, release i produkcja poza zakresem |
| R05 | PLANNED | — | — |
| R06 | PLANNED | — | — |
| R07 | PLANNED | — | — |
| R08 | PLANNED | — | — |
...
- `R04-A2-UI06`: po utracie i odzyskaniu transportu aplikacja pokazuje właściwy komunikat i bezpiecznie ponawia operację bez wewnętrznego wyjątku oraz bez duplikacji. Właściciel zaakceptował source/test fix na `48fbecae0a76edb25f60e9dd314bb8d65bfbae4b` wyłącznie dla potwierdzonego błędu `DocumentsController`; nie jest to odbiór analogicznego ryzyka `ClientsController`, całego R16 ani deploymentu. Ręczny Web B potwierdził błąd połączenia i poprawny retry bez wewnętrznego wyjątku (`PASS / OWNER_OPERATED_RUSTDESK`). A pozostało `NOT_VERIFIED / PREFETCH_CONTAMINATED`; dodatkowe powtórzenie pokazało wcześniejszy auth gate przy pełnym reloadzie oraz dashboardowy `GET /api/v1/documents?...limit=6` po przywróceniu sesji, zanim operator wszedł do repozytorium. Nie wykonano kolejnego outage ani prób do skutku. Całość ma status `WEB_AB_PARTIAL / TEST_ONLY`; R16 nadal wymaga deterministycznego rozstrzygnięcia A i odbioru wieloplatformowego.
