...
function batchScriptArgs(projectDir, mode, schedules) {
  const script = path.join(projectDir, 'operations', 'hardening', 'reconcile-backup-schedule-task.ps1');
  const payload = Buffer.from(JSON.stringify(schedules), 'utf8').toString('base64');
  return [
    '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass',
    '-File', script, '-Mode', mode, '-RepositoryRoot', projectDir,
    '-SchedulesPayloadBase64', payload,
  ];
}
...
async function previewSchedules(projectDir, items, spawnImpl = spawn) {
  ...
  return runPowerShell(batchScriptArgs(projectDir, 'PreviewBatch', schedules), spawnImpl);
}

async function reconcileSchedules(projectDir, items, spawnImpl = spawn) {
  const schedules = validateScheduleList(items);
  const result = await runPowerShell(batchScriptArgs(projectDir, 'ApplyBatch', schedules), spawnImpl);
  if (result.prune && Array.isArray(result.prune.unmanaged) && result.prune.unmanaged.length) {
    throw new Error('backup_scheduler_unmanaged_task_detected');
  }
  return result;
}
